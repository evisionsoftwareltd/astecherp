...
public class SemiFgFindWindow extends Window {
	...
	public SemiFgFindWindow(SessionBean sessionBean,TextField txtItemId){
		...
		this.setCaption("SEMI FINISH GOODS FIND :: "+sessionBean.getCompany());
	...
	private void partyDataLoad(String type) {
		...
		String sql="select distinct a.partyCode,(select partyName from tbPartyInfo where partyCode=a.partyCode) from tbSemiFgInfo a where productionTypeId like '"+type+"'";
	...
	private void semiFgLoad(String type) {
		...
		String sql="select distinct semiFgCode,semiFgName,color from tbSemiFgInfo a where productionTypeId like '"+type+"' and semiFgCode is not null and semiFgCode !='null'";
		Iterator iter=dbService(sql);
		while(iter.hasNext()){
			Object element[]=(Object[]) iter.next();
	...
	private void productionTypeLoad() {
		String sql="select distinct productionTypeId,(select productTypeName from tbProductionType where productTypeId=productionTypeId) from tbSemiFgInfo where productionTypeId !='null'";
		Iterator iter=dbService(sql);
		while(iter.hasNext()){
			Object element[]=(Object[]) iter.next();
	...
	private void tableDataLoad(String type,String semiFg){
		...
		String sql="select semiFgCode,semiFgName,unit,stdWeight,date,color from tbSemiFgInfo" +
				" where productionTypeId like '"+type+"'  and semiFgCode like '"+semiFg+"' " +
				" and productionTypeId!='null' and semiFgCode is not null and semiFgCode!='null'";
	...
	private void tableRowAdd(int ar){
		...
		table.addItem(new Object[]{tbSl.get(ar),tbSemiFgId.get(ar),tbSemiFgName.get(ar),tbUnit.get(ar),tbColor.get(ar),tbStdWeight.get(ar),tbDate.get(ar)}, ar);
	}
}
