...
public class SemiFgSubFindWindow extends Window {
	...
	public SemiFgSubFindWindow(SessionBean sessionBean,TextField txtItemId){
		...
		this.setCaption("FINISH GOODS(Printing/Labeling) FIND :: "+sessionBean.getCompany());
	...
	private void SemiFgDataLoad() {
		...
		String sql="select distinct semiFgId,semiFgName from tbSemiFgSubInformation";
		Iterator iter=dbService(sql);
		while(iter.hasNext()){
			Object element[]=(Object[]) iter.next();
			cmbSemiFgName.addItem(element[0]);
			String caption=element[1].toString();
			cmbSemiFgName.setItemCaption(element[0], caption);
		}
	}

	private void semiFgSubLoad(String SemiFg) {
		cmbSemiFgSubName.removeAllItems();
		String sql="select distinct semiFgSubId,semiFgSubName from tbSemiFgSubInformation where semiFgId like '"+SemiFg+"' and Status='Active'";
		Iterator iter=dbService(sql);
		while(iter.hasNext()){
			Object element[]=(Object[]) iter.next();
	...
	private void btnFindEvent() {
		...
		tableDataLoad( semiFg, semiFgSub);
	}
	...
	private void tableDataLoad(String semiFg,String semiFgSub){
		for(int a=0;a<tbSemiFgSubId.size();a++){
			tbSemiFgSubId.get(a).setValue("");
			tbSemiFgSubName.get(a).setValue("");
			tbDate.get(a).setValue(null);
		}
		String sql="select distinct semiFgSubId,semiFgSubName,date from tbSemiFgSubInformation where semiFgId like '"+semiFg+"' and semiFgSubId like '"+semiFgSub+"'";
		...
		while(iter.hasNext()){
			...
			tbDate.get(a).setValue(element[2]);
			a++;
		}
	}
	...
	private AbsoluteLayout buildMainLayout(){
		...
		table.setColumnWidth("Fg Name",230);
		...
		mainLayout.addComponent(lblSemiFgSubName,"top:90.0px;left:30.0px;");
		mainLayout.addComponent(cmbSemiFgSubName, "top:87.0px;left:150.0px;");
	...
	private void tableRowAdd(int ar){
		...
		table.addItem(new Object[]{tbSl.get(ar),tbSemiFgSubId.get(ar),tbSemiFgSubName.get(ar),tbDate.get(ar)}, ar);
	}
}
