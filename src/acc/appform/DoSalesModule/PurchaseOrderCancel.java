...
public class PurchaseOrderCancel extends Window
{
	...
	public PurchaseOrderCancel(SessionBean sessionBean)
	{
	...
	public void partyNameData()
	{
		...
		try
		{
			...
			if(isFind==false){
			sql ="select distinct partyId,partyname from tbDemandOrderInfo order by partyname";}
			else{
	...
	private void findCancelledData()
	{
		...
		try
		{
			String sql = "";
				...
				sql = "SElect vPoNo,dPoDate,mTotalQty,mDeliveredQty,mBalanceQty from tbPurchaseOrderCancel where vPartyId like  '"+cmbPartyName.getValue().toString()+"'  and dPoDate between '"+dateF.format(dFromDate.getValue())+"' and '"+dateF.format(dToDate.getValue())+"'";
