...
public class DeleteFestivalBonus_CHO extends Window {
		...
		public DeleteFestivalBonus_CHO(SessionBean sessionBean) 
		{
			this.sessionBean= sessionBean;
			this.setCaption("DELETE FESTIVAL BONUS CHO:: " +sessionBean.getCompany());
		...
		private void cmbSectionData(String DepartmentID) 
		{
			...
			try
			{
				String query="select distinct vsectionID,vSectionName,vDepartmentName from tbEmployeeBonus " +
						"where vDepartmentID like '"+DepartmentID+"' and  DateName(MM,dBonusDate)='"+vMonthName+"' " +
						"and year(dBonusDate)='"+yearName+"' order by vSectionName";
				List <?> list=session.createSQLQuery(query).list();	
