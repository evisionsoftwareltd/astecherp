...
public class FestivalBonusCHO extends Window
{
	...
	private Label lblOccassion;
	private ComboBox cmbOccassion;	
	...
	public FestivalBonusCHO(SessionBean sessionBean)
	{
		
		this.sessionBean= sessionBean;
		this.setCaption("FESTIVAL BONUS CHO:: " +sessionBean.getCompany());
	...
	private void formValidation()
	{
		if(dBonusDate.getValue()!=null)
		{
			if(cmbDepartment.getValue()!=null || chkDepartmentAll.booleanValue())
			{
				...
				{
					if(!cmbOccassion.getValue().toString().isEmpty())
					{
						generateAction();
					}
					else
					{
						showNotification("Warning!","Provide CHo Bonus Occasion!!!",Notification.TYPE_WARNING_MESSAGE);
						cmbOccassion.focus();
					}
				}
				else
		...
		{
			showNotification("Warning!","Select CHO Bonus Date!!!",Notification.TYPE_WARNING_MESSAGE);
			dBonusDate.focus();
		}
	}
	...
	private boolean bonusChk()
	{
		...
		String bonusQuery = " select * from tbEmployeeBonus where MONTH(dBonusDate)= MONTH('"+dateFormat.format(dBonusDate.getValue())+"') " +
			"AND YEAR(dBonusDate)= YEAR('"+dateFormat.format(dBonusDate.getValue())+"') "
			...
			+ "and vOccasion='"+cmbOccassion.getValue()+"'";
		System.out.print("bonusChk: "+bonusQuery );
	...
	private void generateBonus()
	{
		...
		try
		{
			String query = "exec prcEmployeeBonus '"+dateFormat.format(dBonusDate.getValue())+"'," +
					...
					"'"+cmbOccassion.getValue().toString()+"','"+sessionBean.getUserName()+"'," +
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		// lblOccassion
		lblOccassion = new Label("Occasion :");
		lblOccassion.setImmediate(false);
		lblOccassion.setWidth("-1px");
		lblOccassion.setHeight("-1px");
		mainLayout.addComponent(lblOccassion, "top:140.0px;left:20.0px;");

		// cmbOccassion
		cmbOccassion = new ComboBox();
		cmbOccassion.setWidth("240px");
		cmbOccassion.setImmediate(true);
		cmbOccassion.setHeight("25px");
		cmbOccassion.addItem("Eid-Ul-Fitr");
		cmbOccassion.addItem("Eid-Ul-Azaha");
		mainLayout.addComponent(cmbOccassion, "top:138.0px;left:130.0px;");
