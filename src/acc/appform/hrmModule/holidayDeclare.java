...
public class holidayDeclare extends Window{
	...
	int index=0;
	public holidayDeclare(SessionBean sessionBean)
	{
	...
	private void tableValueChange()
	{
		...
		String sqlvalue="select dDate,ISNULL(vOccasion,'') from tbHoliday where year(ddate) = '"+year.format(date.getValue())+"' and month(ddate) = '"+month.format(date.getValue())+"'";
	...
	private void tableRowAdd(final int ar)
	{
		...
		Del.get(ar).setIcon(new ThemeResource("../icons/closeGreen.png"));
		...
		cButton.btnSave.addListener(new ClickListener()
		{
			...
			{	
				...
				if(!chkempty)
					...
					showNotification("Warning","Please enter vOccasion Name!!!",Notification.TYPE_WARNING_MESSAGE);
			}
		});
	...
	private void updateData()
	{
		if(sessionBean.isUpdateable())
		{
			...
			try
			{
				...
				if(!chkempty)
				{
					session.createSQLQuery("DELETE FROM tbHoliday WHERE Month(dDate) = '"+month.format(date.getValue())+"' and year(dDate) = '"+year.format(date.getValue())+"'").executeUpdate();
					for(int in=0;in<=holidaydate.size()-1;in++)
					{
						if(holidaydate.get(in).getValue()!=null)
						{
							holidaydate.get(in).setReadOnly(false);
							holidate=new SimpleDateFormat("yyyy-MM-dd").format(holidaydate.get(in).getValue());
							holidaydate.get(in).setReadOnly(true);
							session.createSQLQuery("INSERT INTO tbHoliday(dDate,vOccasion,userId,userIp,entryTime) VALUES('"+holidate+"','"+txtOccasion.get(in).getValue().toString()+"','"+sessionBean.getUserId()+
			...
			try
			{
				...
				for(int in=0;in<=holidaydate.size()-1;in++)
				{
					if(holidaydate.get(in).getValue()!=null)
					{
						...
						holidate=new SimpleDateFormat("yyyy-MM-dd").format(holidaydate.get(in).getValue());
						holidaydate.get(in).setReadOnly(true);
						session.createSQLQuery("INSERT INTO tbHoliday(dDate,vOccasion,userId,userIp,entryTime) VALUES('"+holidate+"','"+txtOccasion.get(in).getValue().toString().replaceAll("'", "''")+"','"+sessionBean.getUserId()+
								"','"+sessionBean.getUserIp()+"',CURRENT_TIMESTAMP)").executeUpdate();
					}
				}
				tx.commit();
	...
	private void calReGen()
	{
		...
		{
			this.getParent().showNotification("calReGen",exp+"",Notification.TYPE_ERROR_MESSAGE);
		}
		finally{session.close();}
