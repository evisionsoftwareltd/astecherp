...
public class EmployeeInformation_CHO extends Window
{
	...
	public EmployeeInformation_CHO(SessionBean sessionBean) 
	{
		this.sessionBean = sessionBean;
		this.setCaption("EMPLOYEE INFORMATION CHO :: "+sessionBean.getCompany());
	...
	private void addEmployeeSectionData(String departmentID)
	{
		...
		try
		{
			String query="select distinct ein.vSectionId, si.SectionName from tbEmployeeInfo ein inner join " +
					...
					"and iStatus=1 and ISNULL(ein.vProximityID,'')!='' order by si.SectionName";
			List <?> lst=session.createSQLQuery(query).list();
			if(!lst.isEmpty())
			{
				for(Iterator <?> itr=lst.iterator();itr.hasNext();)
...
/*					selectEmployeeInfo(RadioBtnGroup.getValue().toString());
					...
					*/
					...
					{
						firstTab.btnBirthPreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnTINNOPreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnCirclePreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnZonePreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnNidPreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnApplicationPreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnJoinPreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
					...
					{
						firstTab.btnConPreview.setCaption("Select a jpg/pdf file");
					}
					else
					{
	...
	private void selectEmployeeInfo(String radio)
	{
		...
		try
		{
			...
			sql = "select ein.vEmployeeId,ein.employeeCode,ein.vProximityId,ein.vEmployeeName,ein.vEmployeeNameBan,ein.vReligion,ein.vContact," +
					...
					"physicallyDisable,ISNULL(vMobileBankFlag,'') vMobileBankFlag,vTinNo,vTinImageLocation,vCircle,vCircleImageLocation,vZone,vZoneImageLocation " +
	...
	private void reportShow()
	{
		...
		try
		{
			String query="select vEmployeeID,employeeCode,vProximityId,vEmployeeName,(select din.designationName from tbDesignationInfo "
					...
					+ " vUserName,dDateTime,vPcIp,mHouseRent,OtStatus,mMedicalAllowance,mConAllowance,mClinical,mSpecial,mOthersAllowance,"
					+ " mProvidentFund,bankId,bankName,bankBranchId,branchName,accountNo,FridayStatus,FridayLunchFee,'Old' vUDFlag from "
					+ " tbUDEmployeeInfo uein where vEmployeeId = '"+firstTab.txtEmployeeId.getValue().toString().trim()+"' and vDepartmentID='"+cmbDepartmentName.getValue()+"' order by "
					+ " vEmployeeName,vEmployeeID,vUDFlag desc,dDateTime desc";
	...
	private void insertDataToEmployeeInfo()
	{
		...
		try
		{
			...
			query="insert into tbEmployeeInfo (vEmployeeId,employeeCode,iFingerID,vProximityId,vEmployeeName,"
					...
					+ " vCircleImageLocation,vZone,vZoneImageLocation) values (" +
		...
		try
		{
			if(status==1 || status==2 || status==3 || status==4 || status==5)
			{
				...
				String updatequery="INSERT INTO tbUDEmployeeInfo(vEmployeeId,employeeCode,iFingerID,vProximityId," +
						...
						"vZone,vZoneImageLocation) select vEmployeeId,employeeCode,iFingerID,vProximityId,vEmployeeName,vEmployeeNameBan," +
						...
						"vTinNo,vTinImageLocation,vCircle,vCircleImageLocation,vZone,vZoneImageLocation from tbEmployeeInfo where " +
	...
	private void addCmbDepartmentData()
	{
		...
		String query = "select DISTINCT vDepartmentID,vDepartmentName from tbDepartmentInfo where vDepartmentName = 'CHO'";
		Session session=SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		try
		{
	...
	private AbsoluteLayout buildMainLayout() 
	{
		...
		mainLayout.addComponent(RadioBtnGroup, "top:5.0px;left:600.0px;");
	...
	private void textClear()
	{
		...
		firstTab.btnTINNOPreview.setCaption("Select a jpg/pdf file");
		firstTab.btnCirclePreview.setCaption("Select a jpg/pdf file");
		firstTab.btnZoneNO.setCaption("Select a jpg/pdf file");
