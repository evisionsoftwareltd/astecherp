...
public class IncrementProcess extends Window
{
	...
	private CheckBox chkAllEmp,chkAllEmpType;
	...
	private AmountField txtIncrementRnge;
	...
	public IncrementProcess(SessionBean sessionBean) 
	{
		this.sessionBean=sessionBean;
		this.setCaption("INCREMENT PROCESS:: "+sessionBean.getCompany());
	...
	private void componentIni(boolean b)
	{
		...
		txtIncrementRnge.setVisible(false);
		txtIncrementRnge.setEnabled(!b);
	...
	private void setEventAction()
	{
		...
		cmbSection.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				...
				txtIncrementRnge.setValue("");
				if(cmbSection.getValue() != null)
				{
					String section=cmbSection.getValue().toString();
					String empType=cmbEmpType.getValue()!=null?cmbEmpType.getValue().toString():"%";
					addEmployeeName(section,empType);
					//tableValueAdd();
		...
		cmbEmpType.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				...
				if(cmbSection.getValue() != null)
				{					
					if(cmbEmpType.getValue() != null ||chkAllEmpType.booleanValue())
					{
						String section=cmbSection.getValue().toString();
						String empType=cmbEmpType.getValue()!=null?cmbEmpType.getValue().toString():"%";
						addEmployeeName(section,empType);
		...
		chkAllEmpType.addListener(new ValueChangeListener() {
			public void valueChange(ValueChangeEvent event) {
				if(chkAllEmpType.booleanValue()){
					if(cmbSection.getValue()!=null)
					{
						tableClear();
						tableComponentIni();
					...
					if(cmbSection.getValue()!=null)
					{
						if(cmbEmpType.getValue()!=null || chkAllEmpType.booleanValue())
						{
							if(cmbEmployee.getValue()!=null)
							{
								...
								empType = cmbEmpType.getValue()!=null?cmbEmpType.getValue().toString():"%";
								
								tableValueAdd(empId,empType);
							}
						}
					}
				}
		...
		cmbIncrement.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				...
				if(cmbIncrement.getValue() != null)
				{
					...
					if(cmbIncrement.getValue().toString()!="INC4")
					{
						txtIncrementRnge.setVisible(true);
						txtIncrPercentage.setVisible(true);
		...
		btnGroupIncrementType.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(!txtIncrementAmount.getValue().toString().trim().isEmpty() || !txtIncrPercentage.getValue().toString().trim().isEmpty() || !txtIncrementRnge.getValue().toString().trim().isEmpty())
		...
		txtIncrementRnge.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(!txtIncrementRnge.getValue().toString().trim().isEmpty())
				{
					double val = 0;
					String numstring2="";
					cButton.btnSave.focus();
					...
					txtIncrementAmount.setValue("");
					btnGroupIncrementType.setVisible(false);
					if(Double.parseDouble(txtIncrementRnge.getValue().toString().trim())>0)
					{
						for(int ind=0;ind<tbLblAutoEmpID.size();ind++)
						{
							if(tbLblGross.get(ind).getValue().toString().contains(","))
							{
								String numString="";
								String[] numbers = tbLblGross.get(ind).getValue().toString().split("\\s*,\\s*");
								  for(int i = 0; i<numbers.length; i++) {
	                                   String numString1=numbers[0];
	                                   if(i==1)
	                                   {
	                                	   numstring2 =numString1+numbers[1];
	                                	   System.out.println(numstring2);
	                                   }
	                                	   
									}
								  
								val=Double.parseDouble(numstring2);
							    double range=  Double.parseDouble(txtIncrementRnge.getValue().toString());
								
								
							    if( val < range )
								{
									double incrementAmount=  range-val;
									System.out.println(incrementAmount);
									if(!tbLblAutoEmpID.get(ind).getValue().toString().trim().isEmpty())
									{
										tbtxtIncrementAmount.get(ind).setValue(incrementAmount);
									}
								}		
							}
							else
							{
								val=Double.parseDouble(tbLblGross.get(ind).getValue().toString().isEmpty()?"0.0":tbLblGross.get(ind).getValue().toString());
							    double range=  Double.parseDouble(txtIncrementRnge.getValue().toString());
	...
	private void addSectionData()
	{
		...
		{ 
			...
			String query=" SELECT distinct ein.vSectionId,sein.SectionName from tbSectionInfo sein inner join"
					+ " tbEmployeeInfo ein on ein.vSectionID=sein.vSectionID where ein.vDepartmentID='"+cmbDepartment+"' and "
					+ "sein.SectionName!='CHO' order by sein.SectionName";
	...
	private void addEmployeeName(String Section,String empType)
	{
		Session session=SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		cmbEmployee.removeAllItems();
		...
		try
		{
			String query = "SELECT distinct vEmployeeId,employeeCode from tbEmployeeInfo " +
					...
					+ "and vEmployeeType like '"+empType+"' "
	...
	private void tableValueAdd(String emp,String empType)
	{
		Session session=SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		try
		{
			String query="select distinct ein.vEmployeeId,ein.employeeCode,ein.vProximityId,ein.vEmployeeName,"
					+ "ein.vDesignationId,din.designationName,ein.vEmployeeType,ein.dInterviewDate,"
					...
					+ "ein.mMonthlySalary,ein.mHouseRent,ein.mConAllowance,ein.mMedicalAllowance,mOthersAllowance "
					+ "from tbEmployeeInfo ein "
					+ "inner join tbDesignationInfo din on ein.vDesignationId=din.designationId "
					+ "where ein.vSectionId='"+cmbSection.getValue()+"' "
					+ "and ISNULL(ein.vProximityId,'')!='' "
					+ "and ein.iStatus=1 and "
					+ "ein.vEmployeeId like '"+emp+"' "
					+ "and vEmployeeType like '"+empType+"' "
					+ "and vSectionId='"+cmbSection.getValue()+"'";
			...
			if(!lst.isEmpty())
			{
				for(Iterator <?> itr=lst.iterator();itr.hasNext();)
				{
					...
					for(int j=0;j<tbLblAutoEmpID.size();j++)
					{
						if(tbLblAutoEmpID.get(j).getValue().toString().trim().isEmpty())
						{
							i=j;
							t=true;
							break;
						...
						if(!tbLblAutoEmpID.get(j).getValue().toString().trim().isEmpty())
						{
							if(tbLblAutoEmpID.get(j).getValue().toString().equals(element[0].toString()))
							{
								t=false;
								break;
							}
					...
					if(t)
					{
						...
						tblblEmployeeType.get(i).setValue(element[6]);
						tblblInterviewDate.get(i).setValue(dateFormat.format(element[7]));
						tblblServiceOfLength.get(i).setValue(element[8].toString()+"y "+element[9].toString()+"m "+element[10].toString()+"d");
						tbtLblBasic.get(i).setValue(commaSeparator.setComma(Double.parseDouble(element[11].toString())));
						tbLblHouseRent.get(i).setValue(commaSeparator.setComma(Double.parseDouble(element[12].toString())));
						tbLblConveyance.get(i).setValue(commaSeparator.setComma(Double.parseDouble(element[13].toString())));
						tbLblMedicalAllowance.get(i).setValue(commaSeparator.setComma(Double.parseDouble(element[14].toString())));
						tbLblGross.get(i).setValue(commaSeparator.setComma(Double.parseDouble(element[15].toString())));
						i++;
	...
	private void updateBntEvent()
	{
		...
		txtIncrementRnge.setEnabled(true);
		txtIncrementRnge.setVisible(true);
	...
	session.createSQLQuery("delete from tbSalaryIncrement where vIncrementType = '"+cmbIncrement.getItemCaption(cmbIncrement.getValue().toString())+"' and  Day(dDate)=Day('"+dFormat.format(dDate.getValue())+"') and Month(dDate)=MONTH('"+dFormat.format(dDate.getValue())+"')"
	...
	private void insertData(Session session,String showNotify)
	{
		...
		for(int i=0; i<tbLblAutoEmpID.size();i++)
		{
			if(!tbLblAutoEmpID.get(i).getValue().toString().isEmpty())
			{
				...
				else if(txtIncrementRnge.getValue()!="" )
				{
					incrementStatus="By Range";
				}
				else
				{
	...
	{
		Window win = new IncrementProcessFind(sessionBean,IncrementDate,SectionID,IncrementType,EmployeeName);
	...
	private AbsoluteLayout buildlMainLayout()
	{
		...
		lblStar.setWidth("-1px");
		lblStar.setHeight("-1px");
		mainLayout.addComponent(lblStar, "top:70.0px;left:18.0px;");

		//lblEmpType
		lblEmpType=new Label("Employee Type : ");
		mainLayout.addComponent(lblEmpType, "top:70.0px;left:25.0px;");
		...
		mainLayout.addComponent(cmbEmpType, "top:68.0px;left:117.0px;");
		
		
		chkAllEmpType=new CheckBox("All");
		chkAllEmpType.setImmediate(true);
		mainLayout.addComponent(chkAllEmpType, "top:68.0px;left:320.0px");
		...
		mainLayout.addComponent(opgEmployee, "top:95.0px;left:95.0px;");
		...
		LblOr.setWidth("-1px");
		LblOr.setHeight("-1px");
		mainLayout.addComponent(LblOr, "top:40.0px;left:1015.0px;");
		
		txtIncrementRnge = new AmountField("");
		txtIncrementRnge.setImmediate(true);
		txtIncrementRnge.setWidth("110px");
		mainLayout.addComponent(new Label("Range  : "), "top:40.0px; left:1050.0px;");
		mainLayout.addComponent(txtIncrementRnge, "top:38.0px; right:5.0px;");
		...
		mainLayout.addComponent(table, "top:150.0px; left:25.0px;");
		mainLayout.addComponent(cButton, "top:590.0px; left:365.0px;");
		...
		mainLayout.addComponent(lblComment, "top:590.0px; left:30.0px;");
