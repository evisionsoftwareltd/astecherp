...
public class individualLeaveEntitlement extends Window 
{
	...
	public individualLeaveEntitlement(SessionBean sessionBean)
	{
		this.sessionBean = sessionBean;

		this.setCaption("INDIVIDUAL LEAVE ENTITLEMENT :: "+sessionBean.getCompany());
	...
	private void tableDataAdd()
	{
		...
		try
		{
			String query ="SELECT ein.vEmployeeId, ein.employeeCode, ein.vEmployeeName, ein.vDesignationId, din.designationName , "
					...
					+ "and ein.vEmployeeID like '"+(cmbEmployeeName.getValue()!=null?cmbEmployeeName.getValue().toString():"%")+"' order by employeeCode";
	...
	private void findInitialize(String EmpID)
	{
		...
		{	
			...
			System.out.println("DataFindByDidar: "+findQuery);
				...
				if(!lbEmployeeID.get(i).getValue().toString().isEmpty())
				{
							...
							!amtAL.get(i).getValue().toString().isEmpty())
					{
						insertQuery = "insert into tbLeaveBalanceNew (currentYear,vAutoEmployeeId,vEmployeeId,vProximityId,"
								...
								+ " iClEnjoyed,iSlEnjoyed,iAlEnjoyed,iMlEnjoyed,iflag,vEncashment,vPartial,userName,entryTime,userIp,vDepartmentID) "
								+ "values "
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		lblEmployeeName.setValue("Employee :");
