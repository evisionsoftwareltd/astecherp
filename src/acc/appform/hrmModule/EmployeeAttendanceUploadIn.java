...
public class EmployeeAttendanceUploadIn extends Window 
{
	...
	private void saveDataToHm()
	{
		...
		try
		{
			...
			List <?> list = session.createSQLQuery("select ei.vEmployeeId,ei.employeeCode,ei.vProximityId,ei.vEmployeeName," +
					...
					" ei.vSectionId,si.SectionName from tbEmployeeInfo ei left join tbDesignationInfo di on" +
					" ei.vDesignationId=di.designationId left join tbSectionInfo si on ei.vSectionId=si.vSectionID inner " +
					" join tbDepartmentInfo dept on dept.vDepartmentID=ei.vDepartmentID " +
					"  where ei.iStatus = 1 and dept.vDepartmentId != 'DEPT10' or ei.vSectionId != 'SEC-5' order by " +
					" CAST(SUBSTRING(vEmployeeId,5,LEN(vEmployeeId)) as int)").list();
			/********************Edit End By Mezbah****************************/
	...
	public void saveAttendanceFileToDirectory(String path,String attUloadDateTime,String dworkdate,String type,String s,String id,
			String ip,String deviceType,String actionType){

		File attendanceFileUpload=new File(path+attUloadDateTime+" DateOfAttendance ("+dworkdate+")-"+type+" "+id+" "+ip+" "+deviceType+" "+actionType+".txt");
	...
	private void saveDataToDirectory()
	{
		...
		try
		{
			...
			String dworkdate=dFormatDDMMYY.format(dWorkingDate.getValue()).toString().trim();
			DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss");
			Date date = new Date();
			String path="D:/Tomcat 7.0/webapps/report/astecherp/employee/application/save/MultipleDevice/";
			...
			ip=ip.replace(":", ".");
			String type="";
			String deviceType="multiple device"; //single and multiple device
			String actionType="Save";

			if(opgType.getValue().toString().equals("IN"))
			{
				type="IN";
			}
			else if(opgType.getValue().toString().equals("OUT"))
			{
				type="OUT";
			}
			//---------------File Save To Directory--------------------//
			...
			while((s = br.readLine())!=null)
			{
				...
				//---------------File Save To Directory By Didar--------------------//
				saveAttendanceFileToDirectory(path,attUloadDateTime,dworkdate,type,s,id,ip,deviceType,actionType);
				//---------------File Save To Directory By Didar--------------------//
	...
	private void showData()
	{
		...
		try
		{
			...
			DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss");
			Date date = new Date();
			String path="D:/Tomcat 7.0/webapps/report/astecherp/employee/application/show/MultipleDevice/";
			String attUloadDateTime=dateFormat.format(date);
			String id=sessionBean.getUserName().toString();
			String ip=sessionBean.getUserIp().toString();
			ip=ip.replace(":", ".");
			...
			while((s = br.readLine())!=null)
			{
				...
				saveAttendanceFileToDirectory(path,attUloadDateTime,workdate,type,s,id,ip,deviceType,actionType);
				//---------------File Save To Directory ShowAction By Didar--------------------//
				...
				if(str[0].equals(opgType.getValue().toString()))
				{
					if(strdate.equals(dworkdate) || strdate.equals(dworkdate2))
					{
						if(hmEmployeeName.get(str[1])!=null)
						{
							...
							if(!(hmDepartmentId.get(proxId).equals("DEPT10")|| hmSectionId.get(proxId).equals("SEC-5")||hmSectionId.get(proxId).equals("SEC-42")))
							{
		...
		try
		{
			for(int i = 0;i<lblEmpID.size();i++)
			{
				if(!lblEmpID.get(i).getValue().toString().isEmpty())
				{
					...
					String chkQuery = "select * from tbEmployeeAttendance"+opgType.getValue().toString()+" where vEmployeeID = '"+lblEmpID.get(i).getValue().toString().trim()+"' and " +
							"dAtt"+opgType.getValue().toString()+"Time = '"+dFormat.format(lblAttDate.get(i).getValue())+" "+time+"'";
					List <?> chkList = session.createSQLQuery(chkQuery).list();
					if(chkList.isEmpty())
					{
