...
public class LeaveInfoFind extends Window 
{
	...
	public LeaveInfoFind(SessionBean sessionBean, TextRead txtAuto, TextRead txtFindDate, String typeOfSearch)
	{		
	...
	private void DepartmentDataAdd()
	{
		...
		try
		{
			String query = "select el.vDepartmentID,dept.vDepartmentName from tbEmployeeLeave el "
					+ "inner join tbDepartmentInfo dept on dept.vDepartmentId=el.vDepartmentID "
					+ "where el.dApplicationDate between '"+dFormat.format(dFromDate.getValue())+"' "
	...
	private void findButtonEvent()
	{
		...
		try
		{
			String Findquery = " SELECT EL.iAutoId,EL.vAutoEmployeeId,EL.vEmployeeId,"
					+ "(select vEmployeeName from tbEmployeeInfo ein where ein.vEmployeeId=EL.vAutoEmployeeId) EmployeeName,"
					+ "EL.dApplicationDate,D.designationName from tbEmployeeLeave as EL "
					+ "inner join tbDesignationInfo as D on EL.vDesignationId=D.designationId "
