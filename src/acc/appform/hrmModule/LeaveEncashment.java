...
public class LeaveEncashment extends Window
{
	...
	public LeaveEncashment(SessionBean sessionBean) 
	{
	...
	private void tableValueAdd()
	{
		...
		try
		{
			String check = "SELECT * from tbLeaveEncashment where YEAR(dLeaveYear)=('"+cmbLeaveYear.getValue()+"')  " +
					...
					"and vEmployeeID like '"+(cmbEmployeeID.getValue()!=null?cmbEmployeeID.getValue().toString():"%" )+"' and vDepartmentID!='DEPT10'";
			List <?> checkList = session.createSQLQuery(check).list();

			if(checkList.isEmpty())
			{
				String query="select distinct ln.vAutoEmployeeId,ln.vEmployeeId,(select vEmployeeName from tbEmployeeInfo ei where ei.vEmployeeId = ln.vAutoEmployeeId)EmployeeName," +
						"(select designationName from tbDesignationInfo where designationId=(select distinct vDesignationId from tbEmployeeInfo where vEmployeeId=ln.vAutoEmployeeId))designation,ln.vSectionId,(select SectionName from tbSectionInfo si where" +
				...
				System.out.println("TableDataLoadByDidar: "+query);
			...
			{
				showNotification("Warning", "No Employee Found or Data already Encash!!!", Notification.TYPE_WARNING_MESSAGE);
			}
		}
		catch (Exception exp)
		{
