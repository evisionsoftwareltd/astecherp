...
public class TourApplicationFormFind extends Window
{
	...
	private List <?> lstOpgType = Arrays.asList(new String [] {"Employee ID","Proximity ID","Employee Name"});
	private OptionGroup opgTypeGroup;
	...
	public TourApplicationFormFind(SessionBean sessionBean,TextField txtTransactionID)
	{
	...
	{	
		...
		opgTypeGroup.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				cmbEmployeeName.removeAllItems();
	...
	private void employeeValueAdd()
	{
		...
		try
		{
			...
			if(opgTypeGroup.getValue()=="Proximity ID")
			{
				query = "select vEmployeeID,vProximityID,vEmployeeCode from tbTourApplication where " +
					"dTourFrom>='"+dbDateFormat.format(dTourFrom.getValue())+"' " +
					"and dTourTo<='"+dbDateFormat.format(dTourTo.getValue())+"' order by vEmployeeCode";
			...
			{
				query = "select vEmployeeID,vEmployeeName,vEmployeeCode from tbTourApplication where " +
					"dTourFrom>='"+dbDateFormat.format(dTourFrom.getValue())+"' " +
					"and dTourTo<='"+dbDateFormat.format(dTourTo.getValue())+"' order by vEmployeeCode";
	...
	private void tableDataAdd()
	{
		...
		try
		{
			String employeeCode = "%";
			if(cmbEmployeeName.getValue()!=null)
				employeeCode=cmbEmployeeName.getValue().toString();
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(new Label("Tour From : "), "top:20.0px; left:20.0px;");
		mainLayout.addComponent(dTourFrom, "top:18.0px; left:120.0px;");
		...
		mainLayout.addComponent(new Label("Tour To : "), "top:45.0px; left:20.0px;");
		mainLayout.addComponent(dTourTo, "top:43.0px; left:120.0px;");

		opgTypeGroup = new OptionGroup("",lstOpgType);
		opgTypeGroup.setImmediate(true);
		opgTypeGroup.setStyleName("horizontal");
		opgTypeGroup.setValue("Employee ID");
		mainLayout.addComponent(opgTypeGroup, "top:70.0px; left:80.0px;");
		
		lblEmployeeName = new Label("Employee ID : ");
		mainLayout.addComponent(lblEmployeeName, "top:95.0px; left:20.0px");

		cmbEmployeeName = new ComboBox();
		cmbEmployeeName.setImmediate(true);
		cmbEmployeeName.setWidth("250.0px");
		mainLayout.addComponent(cmbEmployeeName, "top:93.0px; left:120.0px;");
