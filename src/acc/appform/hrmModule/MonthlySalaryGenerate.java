...
public class MonthlySalaryGenerate extends Window
{
	...
	public MonthlySalaryGenerate(SessionBean sessionBean) 
	{
	...
	private void generateAction()
	{
		...
		if(chkSalary(empQuery))
		{
			query = "select * from tbSalary where vMonthName= " +
					...
					"and autoEmployeeID like '"+employee+"'";
			if(!chkSalary(query))
			{
	...
	private void salaryGenerate()
	{
		...
		try
		{
			...
			String insertQuery = "exec prcCalcMonthlySalary '"+dFormat.format(dGenerateDate.getValue())+"','"+dFormat.format(dSalaryMonth.getValue())+"','"+employee+"','"+Department+"','"+section+"','"+txtRevenueStamp.getValue()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"'";
