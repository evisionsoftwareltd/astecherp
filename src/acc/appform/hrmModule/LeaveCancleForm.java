...
public class LeaveCancleForm extends Window
{
...
//s	private SimpleDateFormat dateFormatForm = new SimpleDateFormat("dd-MM-yyyy");
	...
	public LeaveCancleForm(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("LEAVE CANCEL FORM::"+sessionBean.getCompany());
	...
	private void setBtnAction()
	{
		...
		cmbApplicationDate.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				...
				{
					LeaveAllDataAddd(cmbApplicationDate.getValue().toString());
				}
			}
		});
	...
	private void dApplicationDateDataAdd(String autoEmpID)
	{
		...
		try
		{
			String query = "select distinct iAutoId,dApplicationDate from tbEmployeeLeave where " +
					"vAutoEmployeeId='"+autoEmpID+"'";
			List <?> list = session.createSQLQuery(query).list();
			...
			for (Iterator <?> iter = list.iterator(); iter.hasNext();)
			{
				...
				cmbApplicationDate.setItemCaption(element[0],element[1].toString());
	...
	private void employeeSetData(String employeeId)
	{
		...
		try
		{
			String query = "select ein.vEmployeeName,el.vProximityID from tbEmployeeLeave el inner join tbEmployeeInfo ein " +
					"on el.vAutoEmployeeID=ein.vEmployeeID where el.vAutoEmployeeId='"+employeeId+"'";
	...
	{
		String query="delete from tbEmployeeLeave where iAutoId='"+cmbApplicationDate.getValue()+"' and"
	...
	private void UpdateLeaveBalanceNew(Session session)
	{
		...
		String query="update tbLeaveBalanceNew set i"+txtLeaveType.getValue().toString()+"" +
				...
				"iAutoId='"+cmbApplicationDate.getValue()+"' and  "
			   + "dApplicationDate='"+cmbApplicationDate.getItemCaption(cmbApplicationDate.getValue())+"'  and " +
	...
	private void insertData()
	{
		...
		try
		{
			...
			insertquery = " insert into tbLeaveCancel(dCancelDate,vRefNo,vAutoEmployeeId,vEmployeeID,vProximityID,dApplicationDate,vLeaveTypeID," +
						  ...
						  " '"+cmbApplicationDate.getItemCaption(cmbApplicationDate.getValue())+"'," +					  						 
