...
public class Delete_Monthly_Salary extends Window
{
	...
	public Delete_Monthly_Salary(SessionBean sessionBean) 
	{
		this.sessionBean= sessionBean;
		this.setCaption("DELETE_MONTHLY_SALARY :: " +sessionBean.getCompany());
	...
	private void cmbSalaryMonthData()
	{
		...
		try
		{
			String query="select distinct vMonthName,iYear,dDate from tbAuditSalary order by dDate desc";
	...
	private void cmbEmployeeNameDataAdd(String SectionID)
	{
		...
		try
		{
			...
			String query="select vEmployeeID,vEmployeeCode from tbAuditSalary where vMonthName='"+vMonthName+"' and iYear='"+yearName+"' and vSectionID like '"+SectionID+"'";
	...
	private void deleteSalary()
	{
		...
		{
			query = "select * from tbAuditSalary where vMonthName= '"+vMonthName+"' AND iYear= '"+yearName+"' AND vDepartmentID like '"+(cmbDepartment.getValue()!=null?cmbDepartment.getValue():"%")+"' and vSectionID like '"+section+"' and vEmployeeID like '"+employee+"'";
			if(chkSalary(query))
			{
				clickCount=0;
					...
					public void buttonClicked(ButtonType buttonType)
					{
						if(buttonType == ButtonType.YES)
						{
							if(clickCount==0)
							{
								...
								try
								{
									String deleteQuery="delete from tbAuditSalary where vMonthName='"+vMonthName+"' AND iYear='"+yearName+"' and vDepartmentID like '"+department+"' AND vSectionID like '"+section+"' and vEmployeeID like '"+employee+"'";
	...
	private void cmbDepartmentData() 
	{
		...
		try
		{
			String query="select distinct vDepartmentID,vDepartmentName from tbAuditSalary where vMonthName='"+vMonthName+"' and iYear='"+yearName+"' order by vDepartmentName";
			List <?> list=session.createSQLQuery(query).list();	
	...
	private void cmbSectionData(String DepartmentID) 
	{
		...
		try
		{
			String query="select distinct vSectionID,vSectionName,vDepartmentName from tbAuditSalary "
					+ "where vDepartmentID like '"+DepartmentID+"' and  vMonthName='"+vMonthName+"' "
							+ "and iYear='"+yearName+"' order by vSectionName";
			List <?> list=session.createSQLQuery(query).list();	
