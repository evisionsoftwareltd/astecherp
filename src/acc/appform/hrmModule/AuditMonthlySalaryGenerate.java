...
public class AuditMonthlySalaryGenerate extends Window
{
	...
	public AuditMonthlySalaryGenerate(SessionBean sessionBean) 
	{
		this.sessionBean= sessionBean;
		this.setCaption("SALARY GENERATE :: " +sessionBean.getCompany());
	...
	private void cmbEmployeeNameDataAdd(String SectionID)
	{
		...
		try
		{
			...
			String query="select vEmployeeId, employeeCode from tbEmployeeInfo where vDepartmentID like '"+DepartmentID+"' and  vSectionID like '"+SectionID+"' and iStatus=1 and ISNULL(vProximityID,'')!=''";
	...
	private boolean chkQuery(String query)
	{
		Session session=SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		try
	...
	private void generateAction()
	{
		...
		empQuery = " select * from tbEmployeeInfo where vDepartmentId like " +
				...
				"and vSectionId like '"+section+"' and iStatus=1 and ISNULL(vProximityID,'')!=''";

		if(chkQuery(empQuery))
		{
			query = "select * from tbAuditSalary where vMonthName = " +
					"DateName(MM,'"+dFormat.format(dSalaryMonth.getValue())+"') AND " +
					"iYear= YEAR('"+dFormat.format(dSalaryMonth.getValue())+"') AND " +
					"vDepartmentID like '"+(cmbDepartment.getValue()!=null?cmbDepartment.getValue().toString():"%")+"' and " +
					"vSectionID like '"+section+"' and vEmployeeID like '"+employee+"'";
	...
	private void salaryGenerate()
	{
		...
		try
		{
			String insertQuery = "exec prcCalcAuditMonthlySalary '"+dFormat.format(dGenerateDate.getValue())+"','"+dFormat.format(dSalaryMonth.getValue())+"','"+employee+"','"+Department+"','"+section+"','"+txtRevenueStamp.getValue()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"'";
