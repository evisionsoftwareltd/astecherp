...
public class DepartmentFindWindow extends Window
{
	...
	public DepartmentFindWindow(SessionBean sessionBean,TextField txtReceiptSupplierId,String frmName)
	{
		this.txtReceiptSupplierId = txtReceiptSupplierId;
		this.sessionBean=sessionBean;
		this.frmNam=frmName;
		this.setCaption("DEPARTMENT FIND WINDOW :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("420px");
	...
	private void tableDataDepartment(String department)
	{
		...
		try
		{
			String query ="select vDepartmentId,vDepartmentName from tbDepartmentInfo where" +
					" vDepartmentName like '"+department+"' and vDepartmentName!= 'CHO'";
		...
		{
			showNotification("tableDataDepartment", ex.toString(), Notification.TYPE_ERROR_MESSAGE);
		}
		finally
		{
			session.close();
