...
public class LeaveApplication extends Window
{
	...
	public LeaveApplication(SessionBean sessionBean)
	{
	...
	private void deleteWork()
	{
		...
		String sqlCheck=" select * from tbSalary where "
				...
				+ " and autoEmployeeID='"+cmbEmployeeName.getValue()+"' vDepartmentID!='DEPT10'";
	...
	public void deleteLeave()
	{
		...
		try{
			...
			" and dApplyFrom='"+sessionBean.dfDb.format(dLeaveFrom.getValue())+"' and dApplyTo='"+sessionBean.dfDb.format(dLeaveTo.getValue())+"' and vDepartmentID!='DEPT10'";
	...
	{
		Window win = new LeaveInfoFind(sessionBean, txtAutoId, txtFindDate, typeOfSearch);
	...
	private void saveBtnAction()
	{
		...
		if(isUpdate)
		{
			...
			String sqlCheck=" select * from tbSalary where "
					+ " vMonthName=DateName(MONTH,'"+sessionBean.dfDb.format(dLeaveFrom.getValue())+"') and year=YEAR('"+sessionBean.dfDb.format(dLeaveFrom.getValue())+"') "
					+ " and vMonthName=DateName(MONTH,'"+sessionBean.dfDb.format(dLeaveTo.getValue())+"') and year=YEAR('"+sessionBean.dfDb.format(dLeaveTo.getValue())+"') "
					+ " and autoEmployeeID='"+cmbEmployeeName.getValue()+"' and vDepartmentID!='DEPT10'";
			List<?> list=session.createSQLQuery(sqlCheck).list();
			if(list.isEmpty())
			{
