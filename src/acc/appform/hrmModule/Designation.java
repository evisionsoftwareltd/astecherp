...
public class Designation extends Window {
	...
	private Label lblDesignationSL;
	private NumberField txtDesignationSl;
	...
	private void DesignationAlreadryExistCheck()
	{
		Session session=SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		try
		{
			String query="select designationName from tbDesignationInfo where designationName='"+txtDesignationName.getValue().toString().trim()+"'";
			List <?> lst=session.createSQLQuery(query).list();
			if(!lst.isEmpty() && !isFind)
			{
				txtDesignationName.setValue("");
				showNotification("Warning","Designation "+txtDesignationName.getValue().toString()+" is already exists!!!", Notification.TYPE_WARNING_MESSAGE);
			}
		}
		catch(Exception exp)
		{
			showNotification("DesignationAlreadryExistCheck", exp.toString(), Notification.TYPE_ERROR_MESSAGE);
		}
		finally
		{
			session.close();
	...
	private void DesignationSLAlreadryExistCheck()
	{
		Session session=SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		try
		{
			String query="select designationSerial from tbDesignationInfo where designationSerial='"+txtDesignationSl.getValue().toString().trim()+"'";
			List <?> lst=session.createSQLQuery(query).list();
			if(!lst.isEmpty() && !isFind)
			{
				txtDesignationSl.setValue("");
				showNotification("Warning","Designation Serial "+txtDesignationName.getValue().toString()+" is already exists!!!", Notification.TYPE_WARNING_MESSAGE);
			}
		}
		catch(Exception exp)
		{
			showNotification("DesignationSerialAlreadryExistCheck", exp.toString(), Notification.TYPE_ERROR_MESSAGE);
		}
		finally
		{
			session.close();
	...
	{		
		txtDesignationName.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(!txtDesignationName.getValue().toString().trim().isEmpty())
				{
					DesignationAlreadryExistCheck();
				}
			}
		});

		txtDesignationSl.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(!txtDesignationName.getValue().toString().trim().isEmpty())
				{
					DesignationSLAlreadryExistCheck();
				}
			}
		});
	...
	private void findInitialise(String degId)
	{
		...
		try
		{
			...
			if(lst.iterator().hasNext()){
				...
				txtDesignationSl.setValue(element[3]);
			}
		}
		catch(Exception exp)
		{
	...
	private void saveBtnAction(ClickEvent e)
	{
		...
		{	
			...
			if(txtDesignationSl.getValue().toString().trim().isEmpty())
			{
				txtDesignationSl.focus();
				showNotification("Warning", "Please Enter Designation Serial!!!", Notification.TYPE_WARNING_MESSAGE);
			}
			else
			{
				if(isUpdate)
	...
	private void insertData()
	{
		...
		try
		{
			String query = "insert into tbDesignationInfo values('"+txtDesignationId.getValue().toString().trim()+"','"+txtDesignationName.getValue().toString().trim()+"'," +
					"'"+txtDesignationSl.getValue().toString().trim()+"','Not Need','"+sessionBean.getUserIp()+"','"+sessionBean.getUserName()+"',CURRENT_TIMESTAMP)";
	...
	private void updateData()
	{
		...
		try
		{
			String query=" update tbDesignationInfo set designationName='"+txtDesignationName.getValue().toString().trim()+"',designationSerial='"+txtDesignationSl.getValue().toString().trim()+"' " +
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(lblDesignationId,
				"top:30.0px; left:35.0px;");
		...
		mainLayout.addComponent(txtDesignationId,
				"top:30.0px; left:180.0px;");
		...
		mainLayout.addComponent(lblDesignation,
				"top:55.0px; left:35.0px;");
		...
		mainLayout.addComponent(txtDesignationName,
				"top:55.0px; left:180.0px;");

		lblDesignationSL=new Label("Designation Serial :");
		lblDesignationSL.setImmediate(false);
		mainLayout.addComponent(lblDesignationSL, "top:80.0px; left:35.0px;");

		txtDesignationSl=new NumberField();
		txtDesignationSl.setImmediate(true);
		txtDesignationSl.setWidth("220.0px");
		txtDesignationSl.setHeight("-1px");
		mainLayout.addComponent(txtDesignationSl, "top:80.0px; left:180.0px;");


		mainLayout.addComponent(lblline,"top:100.0px; left: 35.0px");

		mainLayout.addComponent(button, "top:125.0px;left:30.0px;");
