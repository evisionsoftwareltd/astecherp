...
public class DeleteMonthlySalaryCHO extends Window
{
	...
	public DeleteMonthlySalaryCHO(SessionBean sessionBean) 
	{
		this.sessionBean= sessionBean;
		this.setCaption("DELETE MONTHLY SALARY (CHO) :: " +sessionBean.getCompany());
...
/*		chkDepartmentAll.addListener(new ValueChangeListener()
	...
	private void cmbSalaryMonthData()
	{
		...
		try
		{
			String query="select distinct vMonthName,year,dDate from tbSalary where vDepartmentName ='CHO' order by dDate desc";
	...
	private void cmbEmployeeNameDataAdd(String SectionID)
	{
		...
		try
		{
			...
			String query="select autoEmployeeID, empID from tbSalary where vMonthName='"+vMonthName+"' and year='"+yearName+"' and SectionID like '"+SectionID+"'";
...
/*		if(chkDepartmentAll.booleanValue())
		...
		{
			query = "select * from tbSalary where vMonthName= '"+vMonthName+"' AND year= '"+yearName+"' AND vDepartmentID like '"+(cmbDepartment.getValue()!=null?cmbDepartment.getValue():"%")+"' and SectionId like '"+section+"' and autoEmployeeID like '"+employee+"'";
			if(chkSalary(query))
			{
				clickCount=0;
					...
					public void buttonClicked(ButtonType buttonType)
					{
						if(buttonType == ButtonType.YES)
						{
							if(clickCount==0)
							{
								...
								try
								{
									String prcDeleteSalary = "exec prcDeleteMonthlySalary '"+cmbSalaryMonth.getValue()+"','"+department+"','"+section+"','"+employee+"'"; 
									
									System.out.println("Procedure :"+prcDeleteSalary);
									session.createSQLQuery(prcDeleteSalary).executeUpdate();

									String deleteTracking="insert into tbUDsalary(year,vMonthName,vAutoEmployeeID,empId,empCode,empName," +
											"shiftId,shiftName,empType,designation,SectionID,Section,joinDate,totalDaysofMonth," +
											"totalWorkingDay,Friday,present,absentDay,leaveDay,holiday,casualLeave,sickLeave," +
											"EarnedLeave,officialTour,Gross,basicSalary,houseRent,Conveyance,Medical,perDay,AttBonus," +
											"FridayAllowance,Subtotal,salaryCutAbsent,advanceSalary,incomeTax,Insurance,ProvidentFund," +
											"totalDeduction,Adjust,Less,payableAmount,otHour,otRate,totalOtTaka,userId,userIP,entryTime," +
											"vLoanNo,vLoanTransactionID,UDFlag,mDearnessAllowance,mFireAllowance,mRevenueStamp,vDesignationID," +
											"iDesignationSerial,vBankAccountNo,dDate,vDepartmentID,vDepartmentName) select year,vMonthName," +
											"autoEmployeeID,empId,empCode,empName,shiftId,shiftName,empType,designation,SectionID,Section,joinDate," +
											...
											"FridayAllowance,Subtotal,salaryCutAbsent,advanceSalary,incomeTax,Insurance,ProvidentFund," +
											"totalDeduction,Adjust,Less,payableAmount,itotalOTHour,otRate,totalOtTaka,userId,userIP,entryTime," +
											"vLoanNo,vLoanTransactionID,'DELETE',mDearnessAllowance,mFireAllowance,mRevenueStamp,vDesignationID,"+
											"iDesignationSerial,vBankAccountNo,dDate,vDepartmentID,vDepartmentName from tbsalary where vMonthName= '"+vMonthName+"' " +
											"AND year= '"+yearName+"' and vDepartmentID like '"+department+"' AND SectionId like '"+section+"' and " +
											"autoEmployeeID like '"+employee+"'";
									session.createSQLQuery(deleteTracking).executeUpdate();

									String deleteQuery="delete from tbSalary where vMonthName='"+vMonthName+"' AND year='"+yearName+"' and vDepartmentID like '"+department+"' AND SectionId like '"+section+"' and autoEmployeeID like '"+employee+"'";
	...
	private void cmbDepartmentData() 
	{
		...
		try
		{
			String query="select distinct vDepartmentID,vDepartmentName from tbSalary where vMonthName='"+vMonthName+"' and year='"+yearName+"' and vDepartmentName='CHO' order by vDepartmentName";
			List <?> list=session.createSQLQuery(query).list();	
	...
	private void cmbSectionData(String DepartmentID) 
	{
		...
		try
		{
			String query="select distinct sectionID,Section,vDepartmentName from tbSalary where vDepartmentID like '"+DepartmentID+"' and  vMonthName='"+vMonthName+"' and year='"+yearName+"' and section ='CHO' order by Section";
			List <?> list=session.createSQLQuery(query).list();	
