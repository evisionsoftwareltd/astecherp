...
public class FindLeaveBalance extends Window 
{
	...
	private Label lbDeptName = new Label("Dept. Name :");
	private Label lbSecName = new Label("Sec. Name :");
	private ComboBox cmbSecName = new ComboBox();
	private ComboBox cmbDepartName = new ComboBox();
	...
	private ArrayList<Label> amtML = new ArrayList<Label>();

	public FindLeaveBalance(SessionBean sessionBean, TextRead trIdDept)
	{
	...
	private void tableInitialize()
	{
		...
		table.setColumnWidth("Employee Name",192);
		...
		table.addContainerProperty("ML", Label.class , new Label());
	...
	private void SectiondataAdd()
	{
		...
		try
		{
			String query = "select distinct ein.vSectionId,sein.SectionName from tbSectionInfo sein inner join " +
					"tbEmployeeInfo ein on sein.vSectionId=ein.vSectionId where ein.vDepartmentID='"+cmbDepartName.getValue()+"' and (vEmployeeType='Permanent' " +
					"or vEmployeeType='Provisionary') order by sein.SectionName";
			List <?> list = session.createSQLQuery(query).list();
			...
			public void itemClick(ItemClickEvent event)
			{
				if(cmbDepartName.getValue()!=null)
				{
					if(event.isDoubleClick())
					{
						...
						receiptDeptID = cmbDepartName.getValue().toString();

						trIdDept.setValue(receiptempID+"^"+receiptDeptID);
						windowClose();
					}
				}
				else
	...
	private void tableDataAdding()
	{
		...
		try
		{
			String query = " select LB.vAutoEmployeeId,LB.vEmployeeId,EI.vEmployeeName,LB.iClyBalance,LB.iSlyBalance,LB.iAlyBalance,LB.iMlyBalance" +
					" from tbLeaveBalanceNew LB inner join tbEmployeeInfo EI on LB.vAutoEmployeeId=EI.vEmployeeId " +
					" where LB.vDepartmentId = '"+cmbDepartName.getValue().toString()+"' and LB.vSectionId = '"+cmbSecName.getValue().toString()+"' " +
					"and YEAR(LB.currentYear)=YEAR('"+dateformat.format(dYear.getValue())+"') and (vEmployeeType='Permanent' or vEmployeeType='Provisionary') " +
					"order by LB.vEmployeeId";
			List <?> list = session.createSQLQuery(query).list();
			...
			if(!list.isEmpty())
			{
				...
				{						  
					...
					amtML.get(i).setValue(element[6]);
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(lbYear, "top:10.0px;left:20.0px;");
		...
		mainLayout.addComponent(dYear, "top:08.0px;left:65.0px;");
		...
		mainLayout.addComponent(lbDeptName, "top:10.0px;left:200.0px;");
		...
		mainLayout.addComponent(cmbDepartName, "top:08.0px;left:320.0px;");
		...
		mainLayout.addComponent(lbSecName, "top:35.0px;left:200.0px;");
		...
		mainLayout.addComponent(cmbSecName, "top:33.0px;left:320.0px;");

		mainLayout.addComponent(table, "top:65.0px;left:20.0px;");
		return mainLayout;
	}
}
