...
public class LcClosWindowNew extends Window
{
	...
	public LcClosWindowNew(SessionBean sessionBean,TextField txtLcNo,String frmName)
	{
		this.txtLcNoForFind = txtLcNo;
		this.setCaption("MATERIAL RECEIPT L/C FIND :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("700px");
	...
	public void tableRowAdd(final int ar)
	{
		...
		table.addItem(new Object[]{lblSl.get(ar),tbLblLedgerId.get(ar),tbLblLcNo.get(ar),tbLblMrrNo.get(ar),
				tblblVoucherNo.get(ar),tbLblDate.get(ar),tbLblLcValueUSD.get(ar),tbLblLcValueBDT.get(ar)},ar);
	}
	...
	private void lcNoLoad(){
		String query = "select a.lcNo,b.Ledger_Name from tbLcCloseInfo a  "
				+ "inner join tbLedger b on a.lcNo=b.Ledger_Id order by date";
		try{
	...
	private void tableDataAdding()
	{
		...
		try
		{
			Session session = SessionFactoryUtil.getInstance().getCurrentSession();
			session.beginTransaction();
			query= "select CONVERT(varchar(12),a.date,105) date,a.voucherNo,a.mrrNo,a.lcNo,b.Ledger_Name,a.LcValueUsd,a.LcValueBdt from tbLcCloseInfo a " 
				   +"inner join tbLedger b on a.lcNo=b.Ledger_Id where a.lcNo =  '"+cmbLcNo.getValue().toString()+"' "
				   +"and (select tbRawPurchaseInfo.VoucherNo from tbRawPurchaseInfo where tbRawPurchaseInfo.MrrNo=a.mrrNo)='' order by date ";
			
/*			query ="  select CONVERT(varchar(12),a.date,105) date,a.voucherNo,a.mrrNo,a.lcNo,b.Ledger_Name,a.LcValueUsd,a.LcValueBdt from tbLcCloseInfo a "
					+ " inner join tbLedger b on a.lcNo=b.Ledger_Id where a.lcNo =  '"+cmbLcNo.getValue().toString()+"' order by date";
*/
			...
			if(!list.isEmpty())
			{
				...
				{						  
					...
					tblblVoucherNo.get(i).setValue(element[1]);
					tbLblMrrNo.get(i).setValue(element[2]);
					tbLblLedgerId.get(i).setValue(element[3]);
					tbLblLcNo.get(i).setValue(element[4]);
					tbLblLcValueUSD.get(i).setValue(cm.setComma(Double.parseDouble(element[5].toString())));
					tbLblLcValueBDT.get(i).setValue(cm.setComma(Double.parseDouble(element[6].toString())));
	...
	private void compInit()
	{
		...
		table.addContainerProperty("L/C Value USD", TextRead.class, new TextRead(1));
		table.setColumnWidth("L/C Value USD",100);
		
		table.addContainerProperty("L/C Value BDT", TextRead.class, new TextRead(1));
		table.setColumnWidth("L/C Value BDT",100);
		

		table.setColumnAlignments(new String[] { Table.ALIGN_CENTER, Table.ALIGN_CENTER,Table.ALIGN_CENTER, Table.ALIGN_LEFT, Table.ALIGN_CENTER, 
