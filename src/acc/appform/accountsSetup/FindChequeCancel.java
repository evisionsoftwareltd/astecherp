...
public class FindChequeCancel extends Window 
{
	...
	public FindChequeCancel(SessionBean sessionBean, TextRead id) 
	{
	...
	private void findButtonEvent()
	{
		...
		try
		{
			...
			sql = "select dCancelDate,vBankName,vBookNo,vFolioNo,iAutoId from tbChequeCancel where dCancelDate between" +
					" '"+dF.format(fromDate.getValue())+"' and '"+dF.format(toDate.getValue())+"' ";
			...
			if(!lst.isEmpty())
			{
				...
				for (Iterator iter = lst.iterator(); iter.hasNext();)
				{
					...
					table.addItem(new Object[] { dateFormat.format(element[0]), element[1] , element[2], element[3], element[4] }, new Integer(i));

					lblId.get(i).setValue(element[4]);
	...
	private void compInit()
	{
		...
		table.addContainerProperty("Cancel Date", Label.class, new Label());
		table.setColumnWidth("Cancel Date",70);
		...
		table.addContainerProperty("Folio No", Label.class, new Label());
		table.setColumnWidth("Folio No",150);
