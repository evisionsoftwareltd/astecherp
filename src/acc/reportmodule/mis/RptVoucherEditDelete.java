...
public class RptVoucherEditDelete extends Window {
	...
	public RptVoucherEditDelete(SessionBean sessionBean)
	{
		this.sessionBean = sessionBean;
		this.setCaption("Voucher Edit Delete :: "+this.sessionBean.getCompany());
	...
	private void addComboData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			List list=session.createSQLQuery(" select userId,name from tbLogin ").list();
	...
	private void previewBtnAction()
	{
		...
		try
		{
			...
			{
				all = cmbUser.getItemCaption(cmbUser.getValue().toString());
				userName = cmbUser.getItemCaption(cmbUser.getValue().toString());
			}

			queryAll =  " select duv.Voucher_No,duv.Date,l.Ledger_Name,duv.Narration,cc.costCentreName," +
						" duv.TransactionWith,duv.DrAmount,duv.CrAmount,duv.UserName,duv.userIp,duv.DUDtime,duv.TType" +
						" from tbDeleteUpdateVoucher duv inner join tbCostCentre cc on duv.costId=cc.id inner join" +
						" tbLedger l on duv.Ledger_Id=l.Ledger_Id where duv.Date between" +
						" '"+dateFormat.format(dFromDate.getValue())+"' and '"+dateFormat.format(dToDate.getValue())+"'" +
						" and duv.UserName like '"+all+"' ";
			...
			if(queryValueCheck(queryAll))
			{
				...
				Window win = new ReportViewer(hm,"report/account/rptVoucherEditDelete.jasper",
