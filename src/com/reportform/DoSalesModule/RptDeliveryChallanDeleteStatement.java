...
public class RptDeliveryChallanDeleteStatement extends Window
{
	...
	public RptDeliveryChallanDeleteStatement(SessionBean sessionBean)
	{
		this.sessionBean = sessionBean;
		this.setCaption("DELIVERY CHALLAN DELETE STATEMENT :: "+sessionBean.getCompany());
	...
	private void reportPreview()
	{
		...
		try
		{
			...
			String query = "select dEntryTime,dChallanDate,vChallanNo,vPartyName,vProductName,vProductUnit,mChallanQty,vUserName,"
					...
					+ " where convert(date, dEntryTime,105) between '"+df.format(dFromDate.getValue())+"' "
							+ "and '"+df.format(dToDate.getValue())+"' and"
					+ " vUserName like '"+UserName+"' and vPartyId like '"+Party+"' order by dEntryTime,iAutoId";
