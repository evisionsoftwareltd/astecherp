...
public class RptSalarySertificate extends Window
{
	...
	public RptSalarySertificate(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("SALARY CERTIFICATE :: "+sessionBean.getCompany());
	...
	private void reportView()
	{
		...
		try
		{
			String query = "select vEmployeeName,employeeCode,b.designationName,convert(varchar,dJoiningDate,113) dJoiningDate,mMonthlySalary,mHouseRent," +
					"mMedicalAllowance,mConAllowance,mOthersAllowance,convert(varchar,convert(date,CURRENT_TIMESTAMP),113) as Curdate " +
					"from tbEmployeeInfo a " +
					"inner join tbDesignationInfo b on a.vDesignationId=b.designationId " +
					"where vEmployeeID='"+cmbDynamic.getValue().toString()+"'";
			...
			if(queryCheckValue(query))
			{
				...
				Window win = new ReportViewer(hm,"report/account/hrmModule/RptSalarySertificate.jasper",
