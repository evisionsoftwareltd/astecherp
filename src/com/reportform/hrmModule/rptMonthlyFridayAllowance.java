...
public class rptMonthlyFridayAllowance extends Window
{
	...
	public rptMonthlyFridayAllowance(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("MONTHLY FRIDAY ALLOWANCE :: "+sessionBean.getCompany());
	...
	private void reportShow()
	{
		...
		try
		{
			...
			if(cmbDepartment.getValue()!=null)
			{
				strDepartment=cmbDepartment.getValue().toString();
			}
			...
			String query=" select ts.vMonthName,ts.year,ts.autoEmployeeID,ts.empId,ts.empCode,ts.empName,ts.vDesignationID," +
					...
					" ts.vDepartmentID like '"+strDepartment+"' and ts.FridayAllowance>0 and ts.vDepartmentID not like 'DEPT10' and ein.iStatus='1' order by Section,ts.empId,ts.iDesignationSerial";
			...
			if(queryValueCheck(query))
			{
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					String query1= "SELECT distinct SectionID,Section,ts.vDepartmentID,vDepartmentName from tbSalary ts "+
							...
							"SectionID like '"+strSection+"' and ts.vDepartmentId = '"+strDepartment+"' and ts.vDepartmentID not like 'DEPT10'  and " +
					...
					for(Iterator <?> itr1=lst1.iterator();itr1.hasNext();)
					{
						...
						detailQuery[countInd]=" select ts.empId,ts.empCode,ts.empName,ts.designation,cast(ts.Gross as float) Gross,ts.FridayAllowance," +
								...
								" ts.vDepartmentID like '"+cmbDepartment.getValue()+"' and ts.FridayAllowance>0  and ein.iStatus='1'  order by Section,ts.empId,ts.iDesignationSerial";
						countInd++;
					}
