...
public class RptDepartmentWiseCoinAnalysis extends Window
{
	...
	private void reportShow(String Department,String Section)
	{
		...
		try
		{
			query="select * from dbo.funSectionWiseCoinAnalysis('"+Department+"','"+Section+"','"+monthformat.format(dMonth.getValue())+"','"+yearformat.format(dMonth.getValue())+"') where vDepartmentName != 'CHO'  order by vDepartmentName";
