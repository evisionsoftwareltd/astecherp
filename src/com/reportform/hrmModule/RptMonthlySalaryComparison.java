...
public class RptMonthlySalaryComparison extends Window
{
	...
	public RptMonthlySalaryComparison(SessionBean sessionBean)
	{
	...
	public void cmbSectionAddData(String DepartmentID)
	{
		...
		try
		{
			String query="select distinct SectionID,vDepartmentName,Section from tbSalary where vDepartmentID like '"+DepartmentID+"' and " +
					"MONTH(dDate)=MONTH('"+dFormat.format(dSalaryMonth.getValue())+"') and " +
					"YEAR(dDate)=YEAR('"+dFormat.format(dSalaryMonth.getValue())+"') and Section !='CHO' order by vDepartmentName,Section";
			List <?> list=session.createSQLQuery(query).list();
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
	...
	private void reportShow()
	{
		...
		try
		{
			...
			String query="select iYear,vMonthName,employeeCode,vProximityID,vEmployeeName,vDesignationID,vDesignation,vDepartmentID," +
					...
					"funMontlySalaryComparison('"+dFormat.format(dSalaryMonth.getValue())+"','"+Department+"','"+Section+"') " +
					" where vDepartmentName!='CHO' order by vDepartmentName,vSectionName,employeeCode";
