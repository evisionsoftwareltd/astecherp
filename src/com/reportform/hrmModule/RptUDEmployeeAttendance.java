...
public class RptUDEmployeeAttendance extends Window
{
	...
	public void cmbDepartmentAddData()
	{
		...
		try
		{
			String query="select distinct vDepartmentId,vDepartmentName from tbUDEmployeeAttendance " +
	...
	public void cmbSectionAddData()
	{
		...
		try
		{
			String query="select distinct vSectionId,vSectionName from tbUDEmployeeAttendance " +
					"where vDepartmentId='"+cmbDepartment.getValue()+"' and vDepartmentId!="+CHO+" and dDate between " +
	...
	private void EmployeeDataAdd(String Section)
	{
		...
		try
		{
			String query = "select vEmployeeId,vEmployeeCode from tbUDEmployeeAttendance where vDepartmentID = '"+cmbDepartment.getValue()+"' " +
					"and vSectionId like '"+Section+"' and vDepartmentId!="+CHO+" and dDate between '"+dFormat.format(dFrom.getValue())+"' and '"+dFormat.format(dTo.getValue())+"' " +
					"order by vEmployeeCode";
			lblEmpID.setValue("Employee ID :");
			
			if(opgEmployee.getValue()=="Employee Name")
			{
				query = "select vEmployeeId,vEmployeeName from tbUDEmployeeAttendance where vDepartmentID = '"+cmbDepartment.getValue()+"' " +
						"and vSectionId like '"+Section+"' and vDepartmentId!="+CHO+" and dDate between '"+dFormat.format(dFrom.getValue())+"' and '"+dFormat.format(dTo.getValue())+"' " +
						"order by vEmployeeCode";
			...
			{
				query = "select vEmployeeId,vProximityID from tbUDEmployeeAttendance where vDepartmentID = '"+cmbDepartment.getValue()+"' " +
						"and vSectionId like '"+Section+"' and vDepartmentId!="+CHO+" and  dDate between '"+dFormat.format(dFrom.getValue())+"' and '"+dFormat.format(dTo.getValue())+"' " +
						"order by vEmployeeCode";
	...
	private void reportShow()
	{
		...
		try
		{
			...
			query="select vEmployeeCode,vProximityId,vEmployeeName,vDesignation,vDepartmentID,vDepartmentName,vSectionId,vSectionName,dAttInTime,dAttOutTime,"+
					"permittedBy,vReason,udFlag,vPAFlag,vUserId,vUserIP,dEntryTime from tbUDEmployeeAttendance where vDepartmentID " +
					"like '"+cmbDepartment.getValue()+"' and vSectionId like '"+section+"' and vDepartmentId!="+CHO+" and vEmployeeId like '"+employee+"' and " +
					"dAttDate between '"+dFormat.format(dFrom.getValue())+"' and '"+dFormat.format(dTo.getValue())+"' " +
					"order by vDepartmentID,vSectionId,vEmployeeCode,dAttDate";
