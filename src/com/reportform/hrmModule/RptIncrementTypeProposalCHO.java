package com.reportform.hrmModule;
	...
	public class RptIncrementTypeProposalCHO extends Window
	{
		...
		public RptIncrementTypeProposalCHO(SessionBean sessionBean)
		{
		...
		public void cmbSectionAddData()
		{
			...
			try
			{
				String query="select distinct vSectionId,vSectionName from tbSalaryIncrement where "
						+ " YEAR(dDate) = '"+yearFormat.format(dYear.getValue())+"' and vDepartmentId like '"+cmbDepartment.getValue()+"' and vSectionName = 'CHO' order by vSectionName";
				List <?> list=session.createSQLQuery(query).list();
				for(Iterator <?> iter=list.iterator();iter.hasNext();)
				{
					Object[] element = (Object[]) iter.next();
			...
			try
			{
				...
				if(queryValueCheck(query))
				{
					...
					else
					{
						...
						Window win = new ReportViewer(hm,"report/account/hrmModule/RptIncrementTypeProposalCho.jasper",
