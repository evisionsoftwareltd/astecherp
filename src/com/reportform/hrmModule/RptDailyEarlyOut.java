...
public class RptDailyEarlyOut extends Window
{
	...
	public RptDailyEarlyOut(SessionBean sessionbean)
	{
		this.sessionbean=sessionbean;
		this.setCaption("DAILY EARLY OUT REPORT :: "+sessionbean.getCompany());
		this.setWidth("475px");
	...
	private void cmbSectiondataload()
	{
		...
		try
		{
			String sql="select distinct vSectionId,vDepartmentName,vSectionName from tbEmployeeMainAttendance " +
					...
					"vDepartmentID='"+cmbDepartment.getValue()+"' and vDepartmentId!="+CHO+" order by vDepartmentName,vSectionName";
			Iterator <?> itr=session.createSQLQuery(sql).list().iterator();
			while(itr.hasNext())
			{
				Object[] element=(Object[])itr.next();
	...
	private void reportShow()
	{
		...
		{
			String Section="%";
			shiftTimeEnd=hour.getValue().toString();

			shiftEnd=hour.getValue().toString();

			if(txtAMPM.getValue().toString().equals("PM"))
				shiftTimeEnd=Integer.toString(12+Integer.parseInt(hour.getValue().toString()));
			if(Integer.parseInt(minute.getValue().toString())>=Integer.parseInt(txtLateInMinutes.getValue().toString().trim()))
			{
				min=Integer.parseInt(minute.getValue().toString())-Integer.parseInt(txtLateInMinutes.getValue().toString().trim());
				shiftTimeEnd+=":"+Integer.toString(min);
				if(min<=9)
					shiftEnd+=":0"+Integer.toString(min);
				else
					shiftEnd+=":"+Integer.toString(min);
			}
			else
			{
				h=Integer.parseInt(shiftEnd)-1;
				shiftTimeEnd=Integer.toString(h);
				if(h<=10)
					shiftEnd="0"+Integer.toString(h);
				else
					shiftEnd=Integer.toString(h);
				min=60-Integer.parseInt(txtLateInMinutes.getValue().toString().trim());
				shiftTimeEnd+=":"+Integer.toString(min);
				if(min<=9)
					shiftEnd+=":0"+Integer.toString(min);
				else
			...
			shiftTimeEnd+=":"+second.getValue().toString();
			shiftEnd+=":"+second.getValue().toString();
			...
			int diffhour=Integer.parseInt(session.createSQLQuery("select datediff(hh,'14:00:00','"+shiftTimeEnd+"')").list().iterator().next().toString());

			if(diffhour<0)
			{
				query="select *,convert(time,convert(varchar(20),DATEDIFF(SS,Convert(time,dOutTimeOne),'"+shiftTimeEnd+"')/3600)+':'+" +
						"convert(varchar(20),DATEDIFF(SS,Convert(time,dOutTimeOne),'"+shiftTimeEnd+"')%3600/60)+':'+" +
						"convert(varchar(20),DATEDIFF(SS,Convert(time,dOutTimeOne),'"+shiftTimeEnd+"')%60)) Early from funDailyEmployeeAttendance('"+dateFormat.format(dDate.getValue())+"','"+dateFormat.format(dDate.getValue())+"','%','"+cmbDepartment.getValue()+"','"+Section+"')" +
						" where (ISNULL(dInTimeOne,'')!='' and ISNULL(dOutTimeOne,'')!='') and convert(time,dOutTimeOne)<'"+shiftTimeEnd+"' and vDepartmentId!="+CHO+" order by vDepartmentName,vSectionName,vEmployeeCode";
			}
			else
			{
				query="select *,convert(time,convert(varchar(20),DATEDIFF(SS,Convert(time,dOutTimeOne),'"+shiftTimeEnd+"')/3600)+':'+" +
						"convert(varchar(20),DATEDIFF(SS,Convert(time,dOutTimeOne),'"+shiftTimeEnd+"')%3600/60)+':'+" +
						"convert(varchar(20),DATEDIFF(SS,Convert(time,dOutTimeOne),'"+shiftTimeEnd+"')%60)) Early from funDailyEmployeeAttendance('"+dateFormat.format(dDate.getValue())+"','"+dateFormat.format(dDate.getValue())+"','%','"+cmbDepartment.getValue()+"','"+Section+"')" +
						" where (ISNULL(dInTimeOne,'')!='' and ISNULL(dOutTimeOne,'')!='') and (CONVERT(time,dOutTimeOne)>'14:00:00' and convert(time,dOutTimeOne)<'"+shiftTimeEnd+"') and vDepartmentId!="+CHO+" order by vDepartmentName,vSectionName,vEmployeeCode";
			}
			...
			if(!lst.isEmpty())
			{
				...
				hm.put("shiftTime", shiftEnd+" "+txtAMPM.getValue().toString());
				hm.put("sql", query);

				Window win = new ReportViewer(hm,"report/account/hrmModule/rptDailyEarlyOut.jasper",
						this.getWindow().getApplication().getContext().getBaseDirectory()+"".replace("\\","/")+"/VAADIN/rpttmp",
						this.getWindow().getApplication().getURL()+"VAADIN/rpttmp",false,
						this.getWindow().getApplication().getURL()+"VAADIN/applet",RadioBtn.Radio);

				win.setCaption("Section Wise Daily Early Out Report");
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(chkAll, "top:70.0px;left:415.0px;");
		...
		mainLayout.addComponent(new Label("Shift End Time : "), "top:100.0px;left:30.0px;");
		mainLayout.addComponent(hour, "top:98.0px;left:150.0px;");
		...
		mainLayout.addComponent(new Label(":"), "top:98.0px;left:183.0px;");
		mainLayout.addComponent(minute, "top:98.0px;left:186.0px;");
		...
		mainLayout.addComponent(new Label(":"), "top:98.0px;left:219.0px;");
		mainLayout.addComponent(second, "top:98.0px;left:222.0px;");
		...
		mainLayout.addComponent(txtAMPM, "top:96.0px;left:258.0px;");

		txtLateInMinutes=new TextField();
		txtLateInMinutes.setImmediate(true);
		txtLateInMinutes.setWidth("40px");
		mainLayout.addComponent(new Label("Early Out Time Limit : "), "top:130.0px; left:30.0px");
		mainLayout.addComponent(txtLateInMinutes, "top:130.0px;left:150.0px;");
		mainLayout.addComponent(new Label("mins."), "top:130.0px;left:200.0px;");

		reportType=new OptionGroup("",lstType);
		reportType.setStyleName("horizontal");
		reportType.setImmediate(true);
		reportType.select("PDF");
		mainLayout.addComponent(reportType, "top:160.0px;left:150.0px;");
