...
public class RptLeaveEncashment extends Window 
{
	...
	public RptLeaveEncashment(SessionBean sessionBean) 
	{
	...
	private void cmbSectionDataAdd()
	{
		...
		try
		{
			String str = "select distinct vSectionID,vSectionName from tbLeaveEncashment where YEAR(dLeaveYear)='"+cmbYear.getValue()+"' and vSectionName!='CHO' order by vSectionID ";
	...
	private void reportShow()
	{
		...
		try
		{
			...
			Query="select le.vSectionID,le.vSectionName,le.vEmployeeCode,le.vEmployeeName,le.vDesignation,le.mBasic,le.iClTotalDays,le.iClEnjoyedDays,le.iClBalance,le.mClAllowance,le.iSlTotalDays,"
					...
					+ "and le.vEmployeeID like '"+(cmbEmployeeName.getValue()!=null?cmbEmployeeName.getValue().toString():"%")+"'"
					+ " and YEAR(le.dLeaveYear)=YEAR('"+cmbYear.getValue()+"')  and vSectionName!='CHO' and em.istatus like '"+ActiveValue+"' order by le.vEmployeeCode";
			//rptName="rptMonthlySalary.jasper";
			System.out.println("From Report :"+Query);
			...
			if(queryValueCheck(Query))
			{
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					String reportName = "Section Name:"+(cmbSectionName.getItemCaption(cmbSectionName.getValue())!=null?cmbSectionName.getItemCaption(cmbSectionName.getValue()):"All")+"    Leave  Year:"+(cmbYear.getValue());
					String detailQuery[]=new String[1];
					String GroupQuery[]=new String[1];
