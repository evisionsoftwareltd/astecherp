...
public class RptSectionWiseDailyAttendance extends Window 
{
	...
	public void addDepartmentName()
	{
		...
		try
		{
			String query="select distinct vDepartmentId,vDepartmentName from tbEmployeeMainAttendance " +
					"where dDate='"+dfYMD.format(dDate.getValue())+"' and vDepartmentId!="+CHO+" order by vDepartmentName";
	...
	public void addSectionName(String  DepartmentID)
	{
		...
		try
		{
			String query="select distinct vSectionId,vDepartmentName,vSectionName from tbEmployeeMainAttendance " +
					"where vDepartmentID like '"+DepartmentID+"'  and vDepartmentId!="+CHO+" and dDate='"+dfYMD.format(dDate.getValue())+"' " +
	...
	private void reportShow(String DepartmentID,String SectionId)
	{
		...
		try
		{
			String query="select * from funDailyEmployeeAttendance('"+dfYMD.format(dDate.getValue())+"','"+dfYMD.format(dDate.getValue())+"','%','"+DepartmentID+"','"+SectionId+"')" +
					" where ISNULL(dInTimeOne,'')!='' and ISNULL(dOutTimeOne,'')!='' and dTotalWorkingHour>='07:00:00'  and vDepartmentId!="+CHO+" order by vDepartmentName,vSectionName,vEmployeeCode";
			...
			if(queryValueCheck(query))
			{
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					String query1="SELECT distinct vDepartmentID,vDepartmentName,vSectionID,vSectionName" +
							" from tbEmployeeMainAttendance where vSectionID like '"+SectionId+"'" +
							"and dDate='"+dfYMD.format(dDate.getValue())+"'  and vDepartmentId!="+CHO+" order by vSectionName";
					...
					for(Iterator <?> itr1=lst1.iterator();itr1.hasNext();)
					{
						...
						detailQuery[countInd]="select vEmployeeCode,vProximityID,vEmployeeName,vDesignationName,vShiftName,CONVERT(date,dInTimeOne) inDate,"+
								...
								"subString(convert(varchar(50),dTotalWorkingHour),1,8) totalHrs,subString(Convert(varchar(50),dOtHour),1,8) OTHrs  from funDailyEmployeeAttendance" +
								"('"+dfYMD.format(dDate.getValue())+"','"+dfYMD.format(dDate.getValue())+"','%','"+element[0].toString()+"','"+element[2].toString()+"') " +
								"where ISNULL(dInTimeOne,'')!='' and ISNULL(dOutTimeOne,'')!='' and dTotalWorkingHour>='07:00:00'  and vDepartmentId!="+CHO+" order by vDepartmentName,vSectionName,vEmployeeCode";
						countInd++;
					}
