...
public class RptEmployeeBonusCHO extends Window 
{
	...
	public RptEmployeeBonusCHO(SessionBean sessionBean) 
	{
			...
			public void buttonClick(ClickEvent event) 
			{
				if(cmbDepartment.getValue()!=null)
				{
					if(cmbSection.getValue()!=null)
					{
						...
						{
							reportShow(cmbSection.getValue().toString());
						}
						else
						{
	...
	public void addSectionName()
	{
		...
		try
		{
			List <?> list = session.createSQLQuery(" SELECT distinct vSectionID,vDepartmentName,vSectionName from tbEmployeeBonus " +
					...
					"order by vDepartmentName,vSectionName").list();
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
	...
	private void reportShow(Object sectionName)
	{
		...
		String query = " select dGenerateDate,dBonusDate,vEmployeeID,vEmployeeCode,vProximityID,vEmployeeName,vDesignationID," +
				...
				"YEAR(dBonusDate)=YEAR('"+dFormat.format(dYear.getValue())+"') and vDepartmentID like '"+(cmbDepartment.getValue()!=null?cmbDepartment.getValue().toString():"%")+"' " +
				"and vSectionID like '"+sectionName+"' and vOccasion='"+cmbOccasion.getValue().toString()+"' order " +
			...
			try
			{
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					detailQuery[0]=" select vEmployeeCode,vProximityID,vEmployeeName,vDesignationName,CAST(mGrossSalary as FLOAT) mGrossSalary," +
							...
							"where YEAR(dBonusDate)=YEAR('"+dFormat.format(dYear.getValue())+"') and vDepartmentID like '"+(cmbDepartment.getValue()!=null?cmbDepartment.getValue().toString():"%")+"' " +
							"and vSectionID like '"+sectionName+"' and vOccasion='"+cmbOccasion.getValue().toString()+"' order by vDepartmentName,vSectionName," +
							"vEmployeeCode";
					rowWidth=13;
