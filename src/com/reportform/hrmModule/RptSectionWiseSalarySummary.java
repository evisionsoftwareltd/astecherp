...
public class RptSectionWiseSalarySummary extends Window {
	...
	public RptSectionWiseSalarySummary(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("SECTION WISE SALARY SUMMARY :: "+sessionBean.getCompany());
	...
	public void cmbDepartmentAddData()
	{
		...
		try
		{
			String query="select distinct vDepartmentId,vDepartmentName from tbSalary where " +
					"Month(dDate) = Month('"+dFormat.format(dSalaryMonth.getValue())+"') and vDepartmentId !="+CHO+" order by vDepartmentName";
	...
	public void cmbSectionAddData()
	{
		...
		try
		{
			String query="select distinct SectionId,Section from tbSalary where " +
					...
					"Month(dDate) = Month('"+dFormat.format(dSalaryMonth.getValue())+"')  and vDepartmentId !="+CHO+"  order by Section";
			List <?> list=session.createSQLQuery(query).list();
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
	...
	private void reportShow()
	{
		...
		try
		{
			...
			if(cmbEmpType.getValue().toString().equalsIgnoreCase("Casual") || cmbEmpType.getValue().toString().equalsIgnoreCase("Temporary"))
			{
				query="select distinct vDepartmentName,Section,empType,Sum(Gross)Gross,sum(perDay)perDay," +
						"Sum(AttBonus)AttBonus,sum(advanceSalary)advanceSalary,Sum(Adjust)Adjust," +
						"Sum(Less)Less,Sum(FridayAllowance)FridayAllowance,sum(iExtraOT-iLessOT)ExtraOThrs,SUM(iTotalOTMin)iTotalOTMin,SUM(itotalOTHour)itotalOTHour,Sum(otRate)otRate," +
						"SUM(iTotalNormalOTHour)iTotalNormalOTHour,SUM(itotalFridayOTHour)itotalFridayOTHour," +
						"SUM(Friday)Friday,Sum((Gross-round(absentDay*perDay,0)))as salaryAmt," +
						"Sum(round(itotalOTHour*otRate+(otRate/60*iTotalOTMin),0)) as otAmt," +
						"Sum(mRevenueStamp)mRevenueStamp from tbSalary where" +
						" vMonthName= '"+dSalaryMonthFormat.format(dSalaryMonth.getValue())+"' and" +
						" year='"+dYearFormat.format(dSalaryMonth.getValue())+"' and vDepartmentID" +
						" like'"+Department+"' and SectionId like '"+section+"' and empType='Casual'  and vDepartmentId !="+CHO+" " +
						" group by empType,vDepartmentName,Section";
				rptName="rptMonthlySalarySummaryCasual.jasper";
			}
			...
			else
			{
				...
				query="select distinct vDepartmentName,Section,empType, Sum(Gross) Gross,SUM(basicSalary)Basic,SUM(houseRent)houserent,SUM(Conveyance)conveyance,SUM(Medical)medical," +
						"SUM(AttBonus)AttBonus,SUM(FridayAllowance)FridayAllowance,SUM(advanceSalary)AdvanceSalary,sum(incomeTax)Incometax," +
						"SUM(Insurance)Insurance,SUM(ProvidentFund)ProvidentFund,sum((absentDay*perDay))absAmount, SUM(Adjust)Adjust,SUM(Less)Less,SUM(mRevenueStamp)Revenue from tbSalary where " +
						"SectionID like '"+section+"' and vDepartmentID Like '"+Department+"'  and vDepartmentId !="+CHO+" and empType='permanent' and vMonthName='"+dSalaryMonthFormat.format(dSalaryMonth.getValue())+"' and Year='"+dYearFormat.format(dSalaryMonth.getValue())+"' " +
						"group by empType,vDepartmentName,Section";
				rptName="rptSectionWiseMonthlySalarySummary.jasper";
			}
