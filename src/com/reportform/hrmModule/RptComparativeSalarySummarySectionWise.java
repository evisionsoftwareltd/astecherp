...
public class RptComparativeSalarySummarySectionWise extends Window 
{
	...
	public RptComparativeSalarySummarySectionWise(SessionBean sessionBean) 
	{
	...
	public void addDepartmentName()
	{
		...
		try
		{
			List <?> list = session.createSQLQuery(" SELECT vDepartmentID,vDepartmentName from tbSalary where " +
					"Month(dDate) = Month('"+dateFormat.format(dMonth.getValue())+"') and " +
					"YEAR(dDate) = Year('"+dateFormat.format(dMonth.getValue())+"') and vDepartmentName!='CHO'  order by vDepartmentName ").list();
	...
	public void addSectionName()
	{
		...
		try
		{
			List <?> list = session.createSQLQuery(" SELECT SectionID,vDepartmentName,Section from tbSalary " +
					...
					"YEAR(dDate) = Year('"+dateFormat.format(dMonth.getValue())+"') and Section!='CHO' order by vDepartmentName,Section").list();
