...
public class RptSectionWiseInTimePosition extends Window 
{
	...
	public RptSectionWiseInTimePosition(SessionBean sessionBean) 
	{
		this.sessionBean=sessionBean;
		this.setCaption("SHIFT WISE IN TIME REPORT :: "+sessionBean.getCompany());
	...
	public void addSectionName(String DepartmentID)
	{
		...
		try
		{
			String query="select distinct ein.vSectionId,sein.vDepartmentName,sein.SectionName from tbSectionInfo sein inner join " +
					"tbEmployeeInfo ein on sein.vSectionId=ein.vSectionId where sein.vDepartmentID like '"+DepartmentID+"' and ein.vDepartmentId!="+CHO+" " +
					"order by sein.vDepartmentName,sein.SectionName";
			List <?> list = session.createSQLQuery(query).list();
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
	...
	private void reportShow(Object departmentId,Object sectionId)
	{
		...
		try
		{
			...
			if(cmbSection.getValue()!=null)
			{
				Sect=cmbSection.getValue().toString();
			}
			else
			{
				Sect="%";
			...
			String query="select dDate,vDepartmentId,vDepartmentName,vSectionId,vSectionName,totalEmployeeNo,presentEmployeeNo," +
					"absentEmployeeNo,leaveEmployeeNo,tourEmployeeNo from funShiftWiseInTime('"+dfYMD.format(dDate.getValue())+"'," +
					"'"+Dept+"'," +
					"'"+Sect+"') where vDepartmentId!="+CHO+" ";
			...
			if(queryValueCheck(query))
			{
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					String fname = "Shift_wise_in_time_report.xls";
					String url = getWindow().getApplication().getURL()+"VAADIN/themes/temp/attendanceFolder/"+fname;
					
					String header[]=new String[1];
					header [0] = "Date : "+dfYMD.format(dDate.getValue());
					String reportName = "SHIFT WISE IN TIME REPORT";
					String detailQuery[]=new String[1];
					String GroupQuery[]=new String[1];
					String [] signatureOption = new String [0];
					
					detailQuery[0]="select vDepartmentName,vSectionName,totalEmployeeNo,"
							+ "convert(varchar(50),presentEmployeeNo)+'------'+convert( varchar(50),"
							+ "(cast(presentEmployeeNo as float)/cast(totalEmployeeNo as float)*100))+'%'"
							+ " presentEmployeeNo,convert(varchar(50),absentEmployeeNo)+'------'+convert"
							+ "( varchar(50),(cast(absentEmployeeNo as float)/cast(totalEmployeeNo as float)*100))+"
							+ "'%' absentEmployeeNo,convert(varchar(50),leaveEmployeeNo)+'------'+convert"
							+ "( varchar(50),(cast(leaveEmployeeNo as float)/cast(totalEmployeeNo as float)*100))+"
							+ "'%' as leaveEmployeeNo ,convert(varchar(50),tourEmployeeNo)+'------'+convert"
							+ "( varchar(50),(cast(tourEmployeeNo as float)/cast(totalEmployeeNo as float)*100))+'%' "
							+ "as tourEmployeeNo from funShiftWiseInTime('"+dfYMD.format(dDate.getValue())+"'," +
					"'"+Dept+"'," +
					"'"+Sect+"') where vDepartmentId!="+CHO+" ";

					File inFile;
					inFile=new File("D://Tomcat 7.0/webapps/report/astecherp/hrmReportExl/Shift_wise_in_time_report.xls");

					int rowWidth=3;
					new SalaryExcelReport(sessionBean, loc, url, fname, header, inFile, "Shift_Wise_in_time_report", 
				...
				else
				{
					...
					Window win = new ReportViewer(hm,"report/account/hrmModule/rptShiftWiseInTime.jasper",
