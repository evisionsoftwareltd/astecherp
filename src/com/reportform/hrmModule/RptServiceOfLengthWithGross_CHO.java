...
public class RptServiceOfLengthWithGross_CHO extends Window {
	...
	public RptServiceOfLengthWithGross_CHO(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("Length of Service With Gross(CHO) :: "+sessionBean.getCompany());
	...
	public void cmbDepartmentAddData()
	{
		...
		try
		{
			List <?> list=session.createSQLQuery(" SELECT vDepartmentID,vDepartmentName from tbDepartmentInfo " +
					"where vDepartmentName = 'CHO' order by vDepartmentName ").list();
	...
	public void cmbSectionAddData()
	{
		...
		try
		{
			List <?> list=session.createSQLQuery(" SELECT vSectionID,vDepartmentName,SectionName from tbSectionInfo " +
					" where vDepartmentId='"+cmbDepartment.getValue().toString()+"' order by vDepartmentName,SectionName ").list();
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
	...
	public void cmbEmployeeIDData(String Section)
	{
		...
		try
		{
			String query = "select vEmployeeId,employeeCode from tbEmployeeInfo where vDepartmentID like " +
					...
					"'"+cmbSection.getValue()+"' order by employeeCode";
		...
		try
		{
			...
			if(cmbLengthOfMeasure.getValue().toString().equals("Confirmation Date"))
			{
				query = " SELECT dept.vDepartmentName,b.SectionName," +
						...
						" dConfirmationDate confDate,((DATEDIFF(DAY,dConfirmationDate,'"+asOnDate+"'))/365) as year," +
						" (((DATEDIFF(DAY,dConfirmationDate,'"+asOnDate+"'))%365)/30) as month," +
						" ((((DATEDIFF(DAY,dConfirmationDate,'"+asOnDate+"'))%365)%30)) as day,a.iStatus,a.mOthersAllowance from tbEmployeeInfo" +
						...
						" vEmployeeId like '"+employeeValue+"'and isnull(dConfirmationDate,'')!='' and dJoiningDate<='"+asOnDate+"' order by vDepartmentName," +
			...
			else if(cmbLengthOfMeasure.getValue().toString().equals("Joining Date"))
			{
				query = " SELECT dept.vDepartmentName,b.SectionName," +
						" vEmployeeType,vEmployeeName,employeecode,c.designationName,dJoiningDate,dInterviewDate,convert(date,dConfirmationDate) dConfirmationDate ," +
						" dConfirmationDate confDate,((DATEDIFF(DAY,dJoiningDate,'"+asOnDate+"'))/365) as year," +
						" (((DATEDIFF(DAY,dJoiningDate,'"+asOnDate+"'))%365)/30) as month," +
						" ((((DATEDIFF(DAY,dJoiningDate,'"+asOnDate+"'))%365)%30)) as day,a.iStatus,a.mOthersAllowance from tbEmployeeInfo" +
						" as a inner join tbDepartmentInfo as dept on dept.vDepartmentID=a.vDepartmentID inner join " +
						" tbSectionInfo as b on a.vSectionId=b.vSectionId inner join tbDesignationInfo as" +
						" c on a.vDesignationId=c.designationId where a.iStatus like '"+activeFlag+"' and a.vDepartmentID='"+cmbDepartment.getValue().toString()+"' " +
						" and a.vSectionId like '"+sectionValue+"' and  vEmployeeType like '"+categoryValue+"' and vEmployeeId " +
						" like '"+employeeValue+"' and isnull(dJoiningDate,'')!='' and dJoiningDate<='"+asOnDate+"' order by vDepartmentName,SectionName,a.iStatus," +
			...
			else
			{
				query = " SELECT dept.vDepartmentName,b.SectionName," +
						" vEmployeeType,vEmployeeName,employeecode,c.designationName,dJoiningDate,dInterviewDate,convert(date,dConfirmationDate) dConfirmationDate ," +
						" dConfirmationDate confDate,((DATEDIFF(DAY,dInterviewDate,'"+asOnDate+"'))/365) as year," +
						" (((DATEDIFF(DAY,dInterviewDate,'"+asOnDate+"'))%365)/30) as month," +
						" ((((DATEDIFF(DAY,dInterviewDate,'"+asOnDate+"'))%365)%30)) as day,a.iStatus,a.mOthersAllowance from tbEmployeeInfo" +
						" as a inner join tbDepartmentInfo as dept on dept.vDepartmentID=a.vDepartmentID inner join " +
						" tbSectionInfo as b on a.vSectionId=b.vSectionId inner join tbDesignationInfo as" +
						" c on a.vDesignationId=c.designationId where a.iStatus like '"+activeFlag+"' and a.vDepartmentID='"+cmbDepartment.getValue().toString()+"' " +
						" and a.vSectionId like '"+sectionValue+"' and  vEmployeeType like '"+categoryValue+"' and vEmployeeId " +
						" like '"+employeeValue+"' and isnull(dInterviewDate,'')!='' and dJoiningDate<='"+asOnDate+"' order by vDepartmentName,SectionName,a.iStatus," +
						"vEmployeeType,employeeCode";
			...
			if(queryValueCheck(query))
			{
				try
				{
					if(RadioBtnGroup.getValue()=="Excel")
					{
						...
						String strColName[]={"SL#","Employee ID","Employee Name","Designation","Interview Date","Joining Date","Confirmation Date","Year","Month","Day","Gross"};
						String Header="";
						...
						String GroupQuery=" select distinct d.vDepartmentId,d.vDepartmentName,s.vSectionID,s.SectionName,a.vEmployeeType from tbEmployeeInfo " +
								...
								"  a.vEmployeeType like '"+categoryValue+"' order by d.vDepartmentName,s.SectionName,a.vEmployeeType";
						...
						for(Iterator <?> itr1=lst1.iterator();itr1.hasNext();)
						{
							...
							if(cmbLengthOfMeasure.getValue()=="Confirmation Date")
							{
								detailQuery[countInd] = "select a.employeeCode,a.vEmployeeName,b.designationName,a.dInterviewDate,a.dJoiningDate," +
										...
										" ((DATEDIFF(DAY,dConfirmationDate,'"+asOnDate+"'))/365) as year,(((DATEDIFF(DAY,dConfirmationDate,'"+asOnDate+"'))%365)/30) as month," +
										" ((((DATEDIFF(DAY,dConfirmationDate,'"+asOnDate+"'))%365)%30)) as day,cast(a.mOthersAllowance as float) mOthersAllowance from tbEmployeeInfo as a inner join tbDesignationInfo " +
										...
										" and (dConfirmationDate<='"+asOnDate+"' and ISNULL(dConfirmationDate,'')!='') order by " +
							...
							else if(cmbLengthOfMeasure.getValue()=="Joining Date")
							{
								detailQuery[countInd] = " select a.employeeCode,a.vEmployeeName,b.designationName,a.dInterviewDate,a.dJoiningDate," +
										...
										" (((DATEDIFF(DAY,dJoiningDate,'"+asOnDate+"'))%365)/30) as month," +
										" ((((DATEDIFF(DAY,dJoiningDate,'"+asOnDate+"'))%365)%30)) as day,cast(a.mOthersAllowance as float) mOthersAllowance from " +
										...
										" and (dJoiningDate<='"+asOnDate+"' and ISNULL(dJoiningDate,'')!='') order by a.iStatus,a.employeeCode,vEmployeeType," +
										" dJoiningDate";
							...
							else 
							{
								detailQuery[countInd] ="select  a.employeeCode,a.vEmployeeName,b.designationName,a.dInterviewDate,a.dJoiningDate," +
										" (case when (isnull(dConfirmationDate,'')!='' and  year(CONVERT(date,dConfirmationDate))!=1900) then SUBSTRING(a.dConfirmationDate,9,2)+'-'+SUBSTRING(dConfirmationDate,6,2)+'-'+SUBSTRING(dConfirmationDate,1,4) else '' end ) dConfirmationDate," +
										" ((DATEDIFF(DAY,dInterviewDate,'"+asOnDate+"'))/365) as year," +
										" (((DATEDIFF(DAY,dInterviewDate,'"+asOnDate+"'))%365)/30) as month," +
										" ((((DATEDIFF(DAY,dInterviewDate,'"+asOnDate+"'))%365)%30)) as day,cast(a.mOthersAllowance as float) mOthersAllowance from " +
										...
										" and (dInterviewDate<='"+asOnDate+"' and YEAR(dInterviewDate)!='1900') order by a.iStatus,a.employeeCode,vEmployeeType,dJoiningDate";
							}
							countInd++;
						}
						new GenerateExcelReport(sessionBean, loc, url, fname, "length_of_sertvice_report","LENGTH OF SERVICE WITH GROSS",
					...
					else
					{
						...
						Window win = new ReportViewer(hm,"report/account/hrmModule/RptService_lenghthWithGross.jasper",
