...
public class RptMonthlyPaySlip extends Window
{
	...
	public RptMonthlyPaySlip(SessionBean sessionBean)
	{
	...
	private void reportShow()
	{
		...
		try
		{
			String query="select S.year,s.vMonthName,s.empId,s.empCode,s.empName,s.shiftId,s.shiftName,s.empType,s.designation," +
					...
					+ "fridayAmount "
					+ "from tbSalary S "
					+ "inner join tbEmployeeInfo ei on s.autoEmployeeID=ei.vEmployeeId "
					...
					+ "left join tbLoanRecoveryInfo lri on lri.vAutoEmployeeId=ei.vEmployeeId "
					+ "and DATENAME(MM,lri.dRecoveryDate)=s.vMonthName "+
