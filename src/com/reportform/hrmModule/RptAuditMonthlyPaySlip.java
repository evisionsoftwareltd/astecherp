...
public class RptAuditMonthlyPaySlip extends Window
{
	...
	public RptAuditMonthlyPaySlip(SessionBean sessionBean)
	{
	...
	public void cmbDepartmentAddData()
	{
		...
		try
		{
			String query=" select distinct vDepartmentID,vDepartmentName from tbAuditSalary " +
	...
	public void employeeSetData(String sectionId)
	{
		...
		try
		{
			String query = " select vEmployeeId,employeeCode from tbEmployeeInfo where vDepartmentID = '"+cmbDepartment.getValue()+"' and vSectionId='"+sectionId+"' order by employeeCode ";
			lblEmployeeName.setValue("Employee ID :");
			
			if(opgEmployee.getValue()=="Employee Name")
			{
				query = " select vEmployeeId,vEmployeeName,employeeCode from tbEmployeeInfo where vDepartmentID = '"+cmbDepartment.getValue()+"' and vSectionId='"+sectionId+"' order by employeeCode ";
				lblEmployeeName.setValue("Employee Name :");
			}
			
			else if(opgEmployee.getValue()=="Proximity ID")
			{
				query = " select vEmployeeId,vProximityID,employeeCode from tbEmployeeInfo where vDepartmentID = '"+cmbDepartment.getValue()+"' and vSectionId='"+sectionId+"' order by employeeCode";
	...
	private void reportShow()
	{
		...
		try
		{
			String query="select S.iyear,s.vMonthName,s.vEmployeeID,s.vEmployeeCode,s.vEmployeeName,s.vEmployeeType,s.vDesignationID,s.vDesignationName," +
					"s.vDepartmentID,s.vDepartmentName,s.vSectionID,s.vSectionName,s.dJoinDate," +
					"s.iTotalWorkingDay,s.iPresentDays,s.iAbsentDays,s.iLeaveDays,s.iHolidays,s.mGross," +
					"s.mBasic,s.mHouseRent,s.mConveyance,s.mMedicalAllowance,s.mPerDaySalary," +
					"s.mAttendanceBonus,s.mAdvanceSalary,s.mIncomeTax,s.mInsurance,s.mAdjust," +
					"((s.mBasic+s.mHouseRent+s.mConveyance+s.mMedicalAllowance+s.mAttendanceBonus)-(s.mAdvanceSalary+s.mIncomeTax+s.mInsurance))as PayableAmount," +
					"s.iTotalOTHour,s.iTotalOTMin,s.mOTRate," +
					"CONVERT(date,ei.dConfirmationDate)dConfirmationDate,ei.dConfirmationDate as conDate," +
					"(ISNULL(Lb.iClyBalance,0)+ISNULL(Lb.iClOpening,0))as totalCL,ISNULL(Lb.iClEnjoyed,0)" +
					" iClEnjoyed,(ISNULL(Lb.iSlyBalance,0)+ISNULL(Lb.iSlOpening,0))as " +
					"totalSL,ISNULL(Lb.iSlEnjoyed,0) iSlEnjoyed," +
					"(ISNULL(Lb.iAlyBalance,0)+ISNULL(Lb.iAlOpening,0))as totalAL," +
					"ISNULL(Lb.iAlEnjoyed,0) iAlEnjoyed,(ISNULL(Lb.iMlyBalance,0)+ISNULL(Lb.iMlOpening,0))as totalML," +
					"ISNULL(Lb.iMlEnjoyed,0) iMlEnjoyed,ISNULL(lri.mLoanBalance,0) mLoanBalance," +
					"ISNULL(lri.mRecoveryAmount,0) mRecoveryAmount from tbAuditSalary S inner join" +
					" tbEmployeeInfo ei on S.vEmployeeID=ei.vEmployeeId left join tbLeaveBalanceNew Lb " +
					"on Lb.vAutoEmployeeId=ei.vEmployeeId and lb.iflag=1 left join tbLoanRecoveryInfo lri on " +
					"lri.vAutoEmployeeId=ei.vEmployeeId and DATENAME(MM,lri.dRecoveryDate)=s.vMonthName and year(lri.dRecoveryDate)=s.iYear " +
					"where iYear='"+fYear.format(dSalaryMonth.getValue())+"' and s.vEmployeeID like '"+EmployeeId+"' and vMonthName='"+fMonth.format(dSalaryMonth.getValue())+"' and s.vDepartmentID='"+cmbDepartment.getValue()+"'" +
							" and s.vSectionID like '"+SectionId+"' order by s.vDepartmentName,s.vSectionName,s.vEmployeeID";
			...
			if(queryValueCheck(query))
			{
				...
				{
					report="report/account/hrmModule/rptauditbanglapayslip.jasper";
				}
				else if(RadioBtnGroup1.getValue().toString().equals("English"))
				{
					report="report/account/hrmModule/rptauditpayslip.jasper";
				}
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		RadioBtnGroup1.setValue("English");
		mainLayout.addComponent(RadioBtnGroup1, "top:20.0px;left:150.0px;");
		...
		mainLayout.addComponent(RadioBtnGroup, "top:200.0px;left:150.0px;");

		mainLayout.addComponent(new Label("_______________________________________________________________________________"), "top:210.0px;left:20.0px;right:20.0px;");
