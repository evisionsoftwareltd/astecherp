...
public class RptAuditMonthlyAttendance extends Window 
{
	...
	public RptAuditMonthlyAttendance(SessionBean sessionBean) 
	{
		this.sessionBean=sessionBean;
		this.setCaption("MONTHLY_EMPLOYEE_ATTENDANCE :: "+sessionBean.getCompany());
	...
	public void addDepartmentName()
	{
		...
		try
		{
			String query="select distinct vDepartmentId,vDepartmentName from tbEmployeeMainAttendance " +
					...
					"year(dDate)=year('"+dFormat.format(dMonth.getValue())+"') order by vDepartmentName";
	...
	public void addemployeeName()
	{
		...
		try
		{
			String query = "SELECT distinct vEmployeeId,employeeCode from tbEmployeeMainAttendance " +
					...
					"year(dDate)=year('"+dFormat.format(dMonth.getValue())+"') order by employeeCode";
			lblEmployee.setValue("Employee ID :");
			
			if(opgEmployee.getValue()=="Employee Name")
			{
				query = "SELECT distinct vEmployeeId,vEmployeeName,employeeCode from tbEmployeeMainAttendance " +
						"where vDepartmentID='"+cmbDepartment.getValue().toString()+"' and " +
						"vSectionId='"+cmbsection.getValue().toString().trim()+"' and " +
						"Month(dDate)=Month('"+dFormat.format(dMonth.getValue())+"') and " +
						"year(dDate)=year('"+dFormat.format(dMonth.getValue())+"') order by employeeCode";
			...
			{
				query = "SELECT distinct vEmployeeId,vProximityID,employeeCode from tbEmployeeMainAttendance " +
						"where vDepartmentID='"+cmbDepartment.getValue().toString()+"' and " +
						"vSectionId='"+cmbsection.getValue().toString().trim()+"' and " +
						"Month(dDate)=Month('"+dFormat.format(dMonth.getValue())+"') and " +
						"year(dDate)=year('"+dFormat.format(dMonth.getValue())+"') order by employeeCode";
	...
	private void reportShow(String sectionName,String employeeName)
	{
		...
		try
		{
			...
			query = "select * from funAuditMonthlyEmployeeAttendance('"+dFormat.format(dMonth.getValue())+"','"+employeeName+"','"+cmbDepartment.getValue()+"','"+sectionName+"') order by vEmployeeCode";
			if(queryValueCheck(query))
			{
				hm.put("sql", query);

				Window win = new ReportViewer(hm,"report/account/hrmModule/rptAuditMonthlyAttendence1.jasper",
