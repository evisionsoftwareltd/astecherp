...
public class RptDateBetweenSectionWiseAttendance extends Window {
	...
	public RptDateBetweenSectionWiseAttendance(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("SECTION WISE DATE BETWEEN ATTENDANCE :: "+sessionBean.getCompany());
	...
	public void cmbDepartmentAddData()
	{
		...
		try
		{
			String query="select distinct vDepartmentId,vDepartmentName from tbEmployeeMainAttendance where " +
					"dDate between '"+dFormat.format(dFrom.getValue())+"' and '"+dFormat.format(dTo.getValue())+"' and vDepartmentId!="+CHO+" " +
	...
	public void cmbSectionAddData(String DepartmentID)
	{
		...
		try
		{
			String query="select distinct vSectionId,vDepartmentName,vSectionName from tbEmployeeMainAttendance where " +
					...
					"and '"+dFormat.format(dTo.getValue())+"'  and vDepartmentId!="+CHO+" " +
	...
	private void EmployeeDataAdd(String DepartmentID,String SectionID)
	{
		...
		try
		{
			String query = "select vEmployeeId,employeeCode from tbEmployeeMainAttendance where " +
					...
					"dateDiff(hh,dInTimeOne,dOutTimeOne)>=7  and vDepartmentId!="+CHO+"  order by vDepartmentName,vSectionName,employeeCode";
	...
	private void reportShow()
	{
		...
		try
		{
			...
			query="select * from funDailyEmployeeAttendance('"+dFormat.format(dFrom.getValue())+"','"+dFormat.format(dTo.getValue())+"','"+employee+"','"+Department+"','"+section+"')" +
					" where ISNULL(dInTimeOne,'')!='' and ISNULL(dOutTimeOne,'')!=''  and vDepartmentId!="+CHO+"  and dTotalWorkingHour>='07:00:00' order by vDepartmentName,vSectionName,vEmployeeCode";
			...
			if(queryValueCheck(query))
			{
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					String query1="select distinct EMA.employeeCode,EMA.vProximityId,EMA.vEmployeeName,ein.dJoiningDate,EMA.vDesignationName, " +
							...
							"EMA.vDepartmentID like '"+Department+"'  and EMA.vDepartmentId!="+CHO+"  and EMA.vSectionID like '"+section+"' and EMA.vEmployeeID like '"+employee+"'";
					...
					for(Iterator <?> itr1=lst1.iterator();itr1.hasNext();)
					{
						...
						detailQuery[countInd]="select Date,vShiftName,CONVERT(date,dInTimeOne) inDate,ISNULL(subString(convert(varchar(50)," +
								"CONVERT(time,dInTimeTwo)),1,8)+', ','')+ISNULL(subString(convert(varchar(50),CONVERT(time,dInTimeOne)),1,8),'') inTime," +
								"ISNULL(CONVERT(varchar(20),CONVERT(date,dOutTimeTwo),105)+', ','')+ISNULL(CONVERT(varchar(20),CONVERT(date,dOutTimeOne),105),'') " +
								"outDate,ISNULL(subString(convert(varchar(50),CONVERT(time,dOutTimeTwo)),1,8)+', ','')+ISNULL(subString(convert(varchar(50)," +
								"CONVERT(time,dOutTimeOne)),1,8),'') outTime,subString(convert(varchar(50),dTotalWorkingHour),1,8) totalHrs,subString(Convert(varchar(50),dOtHour),1,8) " +
								"OTHrs from funDailyEmployeeAttendance('"+dFormat.format(dFrom.getValue())+"'," +
								"'"+dFormat.format(dTo.getValue())+"','"+element[7].toString()+"','"+element[8].toString()+"','"+element[9].toString()+"') where " +
								"ISNULL(dInTimeOne,'')!='' and ISNULL(dOutTimeOne,'')!='' and dTotalWorkingHour>='07:00:00' and vDepartmentId!="+CHO+" " +
					...
					new GenerateExcelReport(sessionBean, loc, url, fname, "Date Btn Attendance", "SECTION WISE DATE BETWEEN ATTENDANCE", Header, strColName,
							5,groupItem, GroupColName, GroupElement, 2, detailQuery, 0, 0,"A4","Portrait",signatureOption);
				...
				else
				{
					...
					Window win = new ReportViewer(hm,"report/account/hrmModule/rptDatebetweenAttendence.jasper",
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(cButton,"top:225.opx; left:160.0px");
		return mainLayout;
	}
}