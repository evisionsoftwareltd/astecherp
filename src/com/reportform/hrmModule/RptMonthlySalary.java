...
public class RptMonthlySalary extends Window {
	...
	public RptMonthlySalary(SessionBean sessionBean)
	{
		...
		try
		{
			if(cmbEmpType.getValue().toString().equalsIgnoreCase("Casual") || cmbEmpType.getValue().toString().equalsIgnoreCase("Temporary"))
			{
				query="select year,vMonthName,empId,empCode,empName,empType,designation,vDepartmentID,vDepartmentName,SectionID,Section,joinDate,totalWorkingDay," +
						...
						"and empType='"+cmbEmpType.getValue().toString()+"'  and vDepartmentId !="+CHO+" order by vDepartmentName,Section,empID";
			...
			else
			{
				query="select year,vMonthName,empId,empCode,empName,empType,designation,vDepartmentID,vDepartmentName,"
						...
						+ "empType='"+cmbEmpType.getValue().toString()+"'  and vDepartmentId !="+CHO+" order by vDepartmentName,Section,empID";
				...
				if(RadioBtnGroup.getValue()=="Excel")
				{
					...
					{	
						...
						detailQuery[0]="select empId,empName,designation,cast(Gross as float) Gross,joinDate,present,absentDay," +
								...
								"cast((round(absentDay*perDay,0)+advanceSalary+incomeTax+insurance+mRevenueStamp) as float) totalDeduction," +
								"cast((Gross-round(absentDay*perDay,0))+" +
								...
								"empType='"+cmbEmpType.getValue().toString()+"' and vDepartmentID!="+CHO+" order by Section,empID";
						rowWidth=23;
						...
						System.out.println("xls Casual & Temporary: "+detailQuery[0]);
					}
					...
					else
					{
						...
						detailQuery[0]="select empId,empName,designation,cast(Gross as float) Gross,joinDate,present,absentDay,leaveDay," +
								...
								"cast(round(absentDay*perDay,0) as float) as absAmt,cast(advanceSalary as float) advanceSalary," +
								"cast(incomeTax as float) incomeTax,cast(insurance as float) insurance," +
								"cast((round(absentDay*perDay,0)+advanceSalary+incomeTax+insurance+mRevenueStamp) as float) totalDeduction," +
								"cast((Adjust-Less) as float) adjustment," +
						...
						System.out.println("imranp"+detailQuery[0]);
					}
