...
public class RptInactiveIDMonthlyAttendance extends Window 
{
	...
	public RptInactiveIDMonthlyAttendance(SessionBean sessionBean) 
	{
		this.sessionBean=sessionBean;
		this.setCaption("INACTIVE ID WITH MONTHLY ATTENDANCE:: "+sessionBean.getCompany());
	...
	public void addDepartmentName()
	{
		...
		try
		{
			String query="select distinct vDepartmentId,(select vDepartmentName from tbDepartmentInfo d where d.vDepartmentId=e.vDepartmentID)" +
					"vDepartmentName from tbEmployeeMainAttendance e where Month(dDate)=Month('"+dFormat.format(dMonth.getValue())+"') and " +
	...
	public void addemployeeName()
	{
		...
		try
		{
			String query = "select e.vEmployeeID,e.employeeCode from tbEmployeeInfo d" +
					" inner join tbEmployeeMainAttendance e on d.vEmployeeId=e.vEmployeeID where d.iStatus='0' " +
					" and MONTH(e.dDate)=Month('"+dFormat.format(dMonth.getValue())+"')" +
					"  and YEAR(e.dDate)=year('"+dFormat.format(dMonth.getValue())+"') and d.vDepartmentId!="+CHO+" order by d.employeeCode " ;
			
			lblEmployee.setValue("Employee ID :");

			if(opgEmployee.getValue()=="Employee Name")
			{
				query = "select e.vEmployeeID,e.vEmployeeName from tbEmployeeInfo d" +
					" inner join tbEmployeeMainAttendance e on d.vEmployeeId=e.vEmployeeID where d.iStatus='0' " +
					" and MONTH(e.dDate)=Month('"+dFormat.format(dMonth.getValue())+"')" +
					"  and YEAR(e.dDate)=year('"+dFormat.format(dMonth.getValue())+"') and d.vDepartmentId!="+CHO+" order by d.employeeCode ";
			...
			{
				query = "select  e.vEmployeeID,e.vProximityID from tbEmployeeInfo d" +
					" inner join tbEmployeeMainAttendance e on d.vEmployeeId=e.vEmployeeID where d.iStatus='0' " +
					" and MONTH(e.dDate)=Month('"+dFormat.format(dMonth.getValue())+"')" +
					"  and YEAR(e.dDate)=year('"+dFormat.format(dMonth.getValue())+"') and d.vDepartmentId!="+CHO+" order by d.employeeCode ";
	...
	private void reportShow(String sectionName,String employeeName)
	{
		...
		try
		{
			...
			query = "select * from funMonthlyEmployeeAttendanceInactive('"+dFormat.format(dMonth.getValue())+"','"+employeeName+"','"+cmbDepartment.getValue()+"','"+sectionName+"') where vDepartmentId!="+CHO+"";
			if(queryValueCheck(query))
			{
				hm.put("sql", query);

				Window win = new ReportViewer(hm,"report/account/hrmModule/rptMonthlyAttendenceInactiveID.jasper",
