...
public class RptIncrementProposalCHO extends Window {
		...
		public RptIncrementProposalCHO(SessionBean sessionBean)
		{
			this.sessionBean=sessionBean;
			this.setCaption("INCREMENT PROPOSAL CHO :: "+sessionBean.getCompany());
		...
		public void cmbSectionAddData()
		{
			...
			try
			{
				String query="select distinct vSectionId,vSectionName from tbSalaryIncrement where "
						+ " YEAR(dDate) = '"+yearFormat.format(dYear.getValue())+"' "
		...
		private void reportShow()
		{
			...
			try
			{
				String query = "select sinc.employeeCode,sinc.vEmployeeName,ein.vGender,sinc.vDesignationName,sinc.vDepartmentName,sinc.vSectionName,"
						...
						+ " sinc.vEmployeeType,sinc.mGross,sinc.vIncrementPercentage,sinc.mIncrementAmount,sinc.mNewGross from tbSalaryIncrement sinc "
						...
						+ " and sinc.vEmployeeType = '"+cmbCategory.getValue()+"' order by sinc.vDepartmentName,sinc.vSectionName,sinc.employeeCode";
				...
				if(queryValueCheck(query))
				{
					if(RadioBtnGroup.getValue()=="Excel")
					{
						...
						String Groupuery="SELECT distinct vDepartmentID,vDepartmentName,vSectionID,vSectionName from tbSalaryIncrement "
								...
								+ " and vEmployeeType = '"+cmbCategory.getValue()+"' order by vDepartmentName,vSectionName";
						...
						for(Iterator <?> itr1=lst1.iterator();itr1.hasNext();)
						{
							...
							detailQuery[countInd]="select sinc.employeeCode,sinc.vEmployeeName,ein.vGender,sinc.vDesignationName,ein.dJoiningDate,"
									+ " sinc.vEmployeeType,cast(sinc.mGross as float) mGross,CONVERT(varchar,DateDiff(DD,ein.dInterviewDate,sinc.dDate)/365) +'y '+CONVERT(varchar,DateDiff(DD,ein.dInterviewDate,sinc.dDate)%365/30)+'m' vLengthOfService,"
									...
									+ " and sinc.vEmployeeType = '"+cmbCategory.getValue()+"' order by sinc.employeeCode";
							countInd++;
						}
		...
		private AbsoluteLayout buildMainLayout()
		{
			...
			// lblCategory
			lblCategory = new Label("Employee Type :");
			lblCategory.setImmediate(false);
			lblCategory.setWidth("100.0%");
			lblCategory.setHeight("-1px");
			mainLayout.addComponent(lblCategory,"top:95.0px; left:20.0px;");
			...
			mainLayout.addComponent(cmbCategory, "top:93.0px; left:130.0px;");
