...
public class RptSectionWiseDailyAbsent extends Window 
{
	...
	public RptSectionWiseDailyAbsent(SessionBean sessionBean) 
	{
		...
		this.setCaption("SECTION WISE DAILY ABSENT :: "+sessionBean.getCompany());
	...
	public void addDepartmentName()
	{
		...
		try
		{
			String query="select distinct ein.vDepartmentId,dept.vDepartmentName "
					+ "from tbDepartmentInfo dept "
					+ "inner join tbEmployeeInfo ein on dept.vDepartmentId=ein.vDepartmentId "
					+ "and dept.vDepartmentName!='CHO' "
	...
	public void addSectionName(String DepartmentID)
	{
		...
		try
		{
			String query="select distinct ein.vSectionId,sein.vDepartmentName,sein.SectionName from tbSectionInfo sein inner join " +
					"tbEmployeeInfo ein on sein.vSectionId=ein.vSectionId where ein.vDepartmentID like '"+DepartmentID+"' order by sein.SectionName";
			List <?> list = session.createSQLQuery(query).list();
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
	...
	private void reportShow(Object DepartmentID,Object sectionId)
	{
		...
		try
		{
			...
			String query="select * from tbEmployeeMainAttendance where dDate='"+dfYMD.format(dDate.getValue())+"'";
			if(queryValueCheck(query))
			{
				...
				if(!queryValueCheck(query))
				{
					query="select vProximityId,employeeCode,vEmployeeName,din.designationName,dept.vDepartmentName," +
							...
							"where ei.vDepartmentId like '"+DepartmentID+"' and ei.vDepartmentId!='DEPT10' and ei.vSectionId like '"+sectionId+"' and " +
							"((ei.vEmployeeID not in (select vEmployeeId from tbEmployeeMainAttendance where " +
							"dDate='"+dfYMD.format(dDate.getValue())+"')) and (ei.vEmployeeId not in (select vEmployeeId from " +
							"tbTempTourApplication where dTourDate ='"+dfYMD.format(dDate.getValue())+"'))) and ei.iStatus='"+status+"' and ISNULL(ei.vProximityID,'')!='' " +
							"order by sinf.SectionName,employeeCode";
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(RadioBtnStatus, "top:110.0px;left:150.0px;");
		
		RadioBtnGroup = new OptionGroup("",type1);
		RadioBtnGroup.setImmediate(true);
		RadioBtnGroup.setStyleName("horizontal");
		RadioBtnGroup.setValue("PDF");
		mainLayout.addComponent(RadioBtnGroup, "top:130.0px;left:150.0px;");

		mainLayout.addComponent(new Label("__________________________________________________________________________________"), "top:145.0px;left:20.0px;right:20.0px;");
