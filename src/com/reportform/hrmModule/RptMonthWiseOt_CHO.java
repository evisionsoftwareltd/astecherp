...
public class RptMonthWiseOt_CHO extends Window {
		...
		public RptMonthWiseOt_CHO(SessionBean sessionBean)
		{
		...
		private void reportShow(Object Section)
		{
			...
			try
			{
				String query="select ts.year,ts.vMonthName,ts.empId,ts.empCode,ts.empName,ts.designation,ts.Gross,ts.FridayAllowance," +
						...
						"'"+cmbDepartment.getValue()+"' and ts.SectionID='"+cmbSection.getValue()+"' order by ts.empId";
				...
				if(queryValueCheck(query))
				{
					if(RadioBtnGroup.getValue()=="Excel")
					{
						...
						inFile=new File("D://Tomcat 7.0/webapps/report/astecherp/hrmReportExl/MonthWiseOTCHO.xls");

						detailQuery[0]=	" select ts.empId,ts.empCode,ts.empName,ts.designation,cast(ts.Gross as float) Gross," +
								...
								" as varchar) totalNormalOT,cast(ts.otRate as varchar) otRate,round(cast((iTotalNormalOTHour*otRate)" +
								...
								" ts.SectionID='"+Section+"' order by ts.empId";
