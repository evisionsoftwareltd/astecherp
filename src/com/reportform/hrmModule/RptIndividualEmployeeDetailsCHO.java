...
public class RptIndividualEmployeeDetailsCHO extends Window {
			...
			public RptIndividualEmployeeDetailsCHO(SessionBean sessionBean)
			{
				this.sessionBean=sessionBean;
				this.setCaption("INDIVIDUAL EMPLOYEE DETAILS CHO:: "+sessionBean.getCompany());
...
*/
				...
				try
				{
					...
					else
					{
						query= " select vEmployeeId,iFingerID,vProximityId,vEmployeeName,employeeCode from tbEmployeeInfo " +
								" where vSectionId = '"+(cmbSection.getValue()!=null?cmbSection.getValue().toString():"")+"' and iStatus like '"+status+"' and vDepartmentId ='DEPT10' ";
					}
		...
		/*		if(chkSection.booleanValue()==true)
			...
			private void cmbSectionData()
			{
				...
				try
				{
					/*String sql="select distinct tei.vSectionId,tsi.SectionName from tbEmployeeInfo tei "
							+ "inner join  tbSectionInfo as tsi on tei.vSectionID=tsi.vSectionID where tsi.SectionName='CHO' ";*/
					
					String sql="select distinct tei.vSectionId,tsi.SectionName from tbEmployeeInfo tei "
							+ "inner join  tbSectionInfo as tsi on tei.vSectionID=tsi.vSectionID where tei.vDepartmentID='DEPT10'";
			...
			private void reportView()
			{
				...
				try
				{
					...
					query="select tei.vEmployeeId,employeeCode,iFingerID,vProximityId,vEmployeeName,vEmployeeNameBan,vReligion,"
							...
							+ "mHouseRent,mMedicalAllowance,mClinical,mOthersAllowance,mSpecial,mDearnessAllowance,mFireAllowance,"
							+ "mRoomCharge,mProvidentFund,mKFund,mKhichuri,dApplicationDate,dInterviewDate,dJoiningDate,dConfirmationDate,"
							+ "tedi.vNameOfExam,tedi.vGroup_Subject,tedi.vNameOfInstitution,tedi.vYearOfPassing,tedi.vDivision_Class_Grade,"
							...
							+ "left join tbshiftInformation as tsfi on tsfi.vShiftId=tei.vFloor "
							+ "where tei.vEmployeeId like '"+employee+"' and tei.vSectionId like '"+section+"' "
							+ "order by tdi.designationSerial,tei.vEmployeeId";
