...
public class RptDataEntryStatus extends Window
{
	...
	private static final List<String> optionDate = Arrays.asList(new String[]{"As On","Between Date"});
	...
	public RptDataEntryStatus(SessionBean sessionBean)
	{
		this.sessionBean = sessionBean;
		//this.menuId = menuId;
		cm = new CommonMethod(sessionBean);
		this.setCaption("TRANSACTION STATUS :: "+this.sessionBean.getCompany());
	...
	private void btnAction()
	{
		...
		RadioBtnDate.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(RadioBtnDate.getValue().toString().equalsIgnoreCase("As On"))
				{
					lblFromdate.setVisible(false);
					dFromDate.setVisible(false);
					lblTodate.setValue("As on : ");
	...
	public void cmbAddGroupData()
	{
		...
		Transaction tx=null;
		String sql="";
		//"select distinct vUserId,vUserName from funDataEntryStatusDetailsAsOnInventory('%','%','"+dtfYMD.format(dToDate.getValue())+"') ";
		if(RadioBtnType.getValue().toString().equals("Summary"))
		{
			if(RadioBtnDate.getValue().toString().equals("As On"))
			{
				sql="select * from funDataEntryStatusAsOnInventory('%','%','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
			}
			else if(RadioBtnDate.getValue().toString().equals("Between Date"))
			{
				sql="select * from funDataEntryStatusInventory('%','%','"+sessionBean.dfDb.format(dFromDate.getValue())+"','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
			}
			//rptName="report/account/setupReport/RptDataEntrySummary.jasper";
		}
		else
		{
			if(RadioBtnDate.getValue().toString().equals("As On"))
			{
				sql="select * from funDataEntryStatusDetailsAsOnInventory('%','%','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
			}
			else if(RadioBtnDate.getValue().toString().equals("Between Date"))
			{
				sql="select * from funDataEntryStatusDetailsInventory('%','%','"+sessionBean.dfDb.format(dFromDate.getValue())+"','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
			}
			//rptName="report/account/setupReport/RptDataEntryStatusDetails.jasper";
		}
		try
		{
...
/*	private void setCombo()
	{
		try
		{
			...
			List<?> lst = session.createSQLQuery(" select vBuyerOrderNoId,vBuyerOrderNo from tbLcOpeningInfo order by vBuyerOrderNo ").list();
			for (Iterator<?> iter = lst.iterator();iter.hasNext();) 
			{
				Object[] element = (Object[]) iter.next();
				cmbExpLcNo.addItem(element[1]);
				cmbExpLcNo.setItemCaption(element[1], element[1].toString());
			}
		}
		catch (Exception ex) 
		{
	...
	private void previewBtnAction()
	{
		...
		try
		{
			...
			if(RadioBtnType.getValue().toString().equals("Summary"))
			{
				if(RadioBtnDate.getValue().toString().equals("As On"))
				{
					query="select * from funDataEntryStatusAsOnInventory('"+userId+"','"+userName+"','"+sessionBean.dfDb.format(dToDate.getValue())+"')  order by iSerial";
				}
				else if(RadioBtnDate.getValue().toString().equals("Between Date"))
				{
					query="select * from funDataEntryStatusInventory('"+userId+"','"+userName+"','"+sessionBean.dfDb.format(dFromDate.getValue())+"','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
				}
				rptName="report/account/setupReport/RptDataEntrySummary.jasper";
			}
			else
			{
				if(RadioBtnDate.getValue().toString().equals("As On"))
				{
					query="select * from funDataEntryStatusDetailsAsOnInventory('"+userId+"','"+userName+"','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
				}
				else if(RadioBtnDate.getValue().toString().equals("Between Date"))
				{
					query="select * from funDataEntryStatusDetailsInventory('"+userId+"','"+userName+"','"+sessionBean.dfDb.format(dFromDate.getValue())+"','"+sessionBean.dfDb.format(dToDate.getValue())+"') order by iSerial";
				}
				rptName="report/account/setupReport/RptDataEntryStatusDetails.jasper";
			}
			...
			if(queryCheckValue(query))
			{
				...
				hm.put("developer","Evision Software Ltd.");
				...
				if(RadioBtnType.getValue().toString().equals("Details"))
				{
					...
					else if(RadioBtnDate.getValue().toString().equals("As On"))
					{
				...
				else
				{
					...
					if(RadioBtnDate.getValue().toString().equals("As On"))
					{
						hm.put("dataEntryName", "As On :"+sessionBean.dfBd.format(dFromDate.getValue()));
					}
				}
