...
import acc.menuform.menu.AccountingMenu;
import acc.menuform.menu.AdminMenu;
import acc.menuform.menu.CrashingMenu;
import acc.menuform.menu.DOSalesMenu;
import acc.menuform.menu.FinishedGoodsMenu;
import acc.menuform.menu.FixedAssetMenu;
import acc.menuform.menu.HrmMenu;
import acc.menuform.menu.LcMenu;
import acc.menuform.menu.ProductionMenu;
import acc.menuform.menu.RawMaterialMenu;
import acc.menuform.menu.SparePartsMenu;
import acc.menuform.menu.ThirdPartMenu;
import acc.menuform.menu.TransportMenu;
...
public class UserAutentication extends Window
{
	...
	private boolean isSetup = false, isRawMeterials = false,isSpareParts=false;
	...
	{	
		...
		try
		{
			...
			if(!lst.isEmpty())
			{
				for (Iterator iter = lst.iterator(); iter.hasNext();) 
				{
					...
					if(Integer.parseInt(element[0].toString())==11)
					{	isSpareParts = true;	}
					
					/*if(Integer.parseInt(element[0].toString())==12)
					{	isThirdPartyRm = true;	}*/
					...
					if(Integer.parseInt(element[0].toString())==2)
					{	isProduction = true;	}

					if(Integer.parseInt(element[0].toString())==3)
					{	isFinishedGoods = true;	}

					if(Integer.parseInt(element[0].toString())==4)
					{	isDoSales = true;	}

					if(Integer.parseInt(element[0].toString())==5)
					{	isAccounts = true;	}

					if(Integer.parseInt(element[0].toString())==6)
					{	isFixedAsset = true;	}
					
					if(Integer.parseInt(element[0].toString())==9)
					{	isLc = true;	}//lc is last module that's why it's module id is 9

					if(Integer.parseInt(element[0].toString())==7)
					{	isHrm = true;	}

					if(Integer.parseInt(element[0].toString())==8)
					{	isTransport = true;	}
		...
		Object setupModule = null,rawMeterials = null,spareParts = null,productionModule = null;
		...
		Object thirdpartyModule = null,crashingModule = null;
		...
		if(isRawMeterials)
		{
			rawMeterials = addCaptionedItem("INVENTORY MODULE", null);
		}
		if(isSpareParts)
		{
			spareParts = addCaptionedItem("SPARE PARTS MODULE", null);
		}
		if(isProduction)
		{
		...
		if(isFinishedGoods)
		{
			finishedGoodsModule = addCaptionedItem("FINISH GOODS MODULE", null);
		}
		if(isDoSales)
		{
			doSalesModule = addCaptionedItem("PO & SALES MODULE", null);
		}
		if(isAccounts)
		{
		...
		if(isFixedAsset)
		{
			fixedAssetModule = addCaptionedItem("FIXED ASSET MODULE", null);
		}
		if(isHrm)
		{
		...
		/*if(isThirdPartyRm)
		{
			thirdpartyModule = addCaptionedItem("THIRD PARTY R/M MODULE", null);
		}*/
		...
		addChild(setupModule, rawMeterials, spareParts,productionModule,finishedGoodsModule,
				doSalesModule,accountsModule,fixedAssetModule,hrmModule,transportModule,lcModule,thirdpartyModule,crashingModule);
	}

	void addChild(Object setupModule, Object rawMeterials,Object sparePartsModule, Object productionModule, Object finishedGoodsModule,
			Object doSalesModule, Object accountsModule, Object fixedAssetModule, Object hrmModule,
			Object transportModule,Object lcModule,Object thirdpartyModule,Object crashingModule)
	{
		if(isSetup)
		{
		...
		if(isRawMeterials)
		{
			new RawMaterialMenu(rawMeterials,menuTree,sessionBean,panelUp);
		}
		if(isSpareParts)
		{
			new SparePartsMenu(sparePartsModule,menuTree,sessionBean,panelUp);
		}
		if(isProduction)
		{
			new ProductionMenu(productionModule,menuTree,sessionBean,panelUp);
		}
		if(isFinishedGoods)
		{
			new FinishedGoodsMenu(finishedGoodsModule,menuTree,sessionBean,panelUp);
		}
		if(isDoSales)
		{
			new DOSalesMenu(doSalesModule,menuTree,sessionBean,panelUp);
		}
		if(isAccounts)
		{
			new AccountingMenu(accountsModule,menuTree,sessionBean,panelUp);
		}
		if(isFixedAsset)
		{
			new FixedAssetMenu(fixedAssetModule,menuTree,sessionBean,panelUp);
		}
		if(isLc)
		{
			new LcMenu(fixedAssetModule,menuTree,sessionBean,panelUp);
		}
		if(isHrm)
		{
		...
		if(isTransport)
		{
			new TransportMenu(transportModule,menuTree,sessionBean,panelUp);
		}
		
		/*if(isThirdPartyRm)
		{
			new ThirdPartMenu(thirdpartyModule,menuTree,sessionBean,panelUp);
		}*/
		if(isCrashing)
		{
			new CrashingMenu(crashingModule,menuTree,sessionBean,panelUp);
		}
	}
