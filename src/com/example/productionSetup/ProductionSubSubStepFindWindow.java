...
public class ProductionSubSubStepFindWindow extends Window
{
	...
	private Label lblProductionSubStep;
	private ComboBox cmbProductionSubStep;
	...
	public ProductionSubSubStepFindWindow(SessionBean sessionBean,TextField txtReceiptSupplierId)
	{
		this.txtReceiptSupplierId = txtReceiptSupplierId;
		this.sessionBean=sessionBean;
		this.setCaption("FIND PRODUCTION PROCESS SUB SUB STEP :: "+sessionBean.getCompany());
		this.center();
	...
	public void cmbSubStepLoadData()
	{
		cmbProductionSubStep.removeAllItems();
		Transaction tx = null;
		try{
			Session session = SessionFactoryUtil.getInstance().getCurrentSession();
			tx = session.beginTransaction();
			List list = session.createSQLQuery("select SubStepId,SubStepName from tbProductionSubStep where productionTypeId='"+cmbProductionType.getValue()+"' and productionStepId='"+cmbProductionStep.getValue()+"'").list();

			for(Iterator iter=list.iterator();iter.hasNext();)
			{
				Object[] element = (Object[]) iter.next();
				cmbProductionSubStep.addItem(element[0]);
				cmbProductionSubStep.setItemCaption(element[0], (String) element[1]);
			}
		}
		catch(Exception exp)
		{
	...
	public void setEventAction()
	{
		...
		cmbProductionSubStep.addListener(new ValueChangeListener() {

			public void valueChange(ValueChangeEvent event) {

				if(cmbProductionSubStep.getValue()!=null){
					tableDataAdding();
				}
			}
		});
	...
	private void tableDataAdding()
	{
		try{
			...
			String query="select SubSubStepId,SubSubStepName from tbProductionSubSubStep " +
					"where productionTypeId='"+cmbProductionType.getValue()+"' and productionStepId like '"+cmbProductionStep.getValue()+"' and productionSubStepId like '"+cmbProductionSubStep.getValue()+"'";
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		lblProductionSubStep = new Label("Production Sub Step : ");
		lblProductionSubStep.setImmediate(false);
		lblProductionSubStep.setWidth("-1px");
		lblProductionSubStep.setHeight("-1px");

		cmbProductionSubStep = new ComboBox();
		cmbProductionSubStep.setImmediate(true);
		cmbProductionSubStep.setWidth("220px");
		cmbProductionSubStep.setHeight("-1px");
		...
		mainLayout.addComponent(cmbProductionType, "top:20px; left:150px;");

		mainLayout.addComponent(lblProductionStep, "top:50px; left:20px;");
		mainLayout.addComponent(cmbProductionStep, "top:50px; left:150px;");

		mainLayout.addComponent(lblProductionSubStep, "top:80px; left:20px;");
		mainLayout.addComponent(cmbProductionSubStep, "top:80px; left:150px;");

		mainLayout.addComponent(table, "top:110px; left:150px;");
