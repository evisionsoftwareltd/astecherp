...
public class AdjustmentEntryFind extends Window
{
	...
	private ArrayList<Label> tblblAdjustmentNo = new ArrayList<Label>();
	private ArrayList<Label> tblblAdjustmentDate = new ArrayList<Label>();
	...
	public AdjustmentEntryFind(SessionBean sessionBean,TextField txtProductionNo)
	{
		this.txtProductionNo=txtProductionNo;
		this.sessionBean=sessionBean;
		this.setResizable(false);
		this.setCaption("ADJUSTMENT ENTRY FIND :: "+sessionBean.getCompany());
	...
	private void ProductionStepLoad() {
		...
		Iterator iter=dbService("select 0,productionStep from tbAdjustmentInfo" +
				" where adjustmentDate between '"+dateFormat.format(dFromDate.getValue())+"' " +
	...
	public void btnAction()
	{
		...
		table.addListener(new ItemClickListener() 
		{
			public void itemClick(ItemClickEvent event) 
			{
				...
				{
					String receiptAreaId = tblblAdjustmentNo.get(Integer.valueOf(event.getItemId().toString())).getValue().toString();
	...
	private void tableDataLoad() {
		...
		Iterator iter=dbService("select productionStep,batchNo,AdjustmentNo,convert(varchar(10),adjustmentDate,103)adjustmentDate from tbAdjustmentInfo  " +
				"where adjustmentDate between '"+dateFormat.format(dFromDate.getValue())+"' and '"+dateFormat.format(dToDate.getValue())+"'" +
				" and productionStep like '"+productionStep+"' order by AdjustmentNo");
		int a=0;
		if(iter.hasNext()){
			while(iter.hasNext()){
				Object element[]=(Object[])iter.next();
				tblblProductionStep.get(a).setValue(element[0]);
				tblblBatchNo.get(a).setValue(element[1]);
				tblblAdjustmentNo.get(a).setValue(element[2]);
				tblblAdjustmentDate.get(a).setValue(element[3]);
	...
	public void tableRowAdd(final int ar)
	{
		try{
			...
			tblblAdjustmentNo.add(ar, new Label());
			tblblAdjustmentNo.get(ar).setImmediate(true);
			tblblAdjustmentNo.get(ar).setWidth("100%");
			tblblAdjustmentNo.get(ar).setHeight("-1px");


			tblblAdjustmentDate.add(ar, new Label());
			tblblAdjustmentDate.get(ar).setImmediate(true);
			tblblAdjustmentDate.get(ar).setWidth("100%");
			tblblAdjustmentDate.get(ar).setHeight("-1px");	


			table.addItem(new Object[]{tbLblSl.get(ar),	tblblProductionStep.get(ar),tblblBatchNo.get(ar),tblblAdjustmentNo.get(ar),tblblAdjustmentDate.get(ar)},ar);
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		table.setColumnWidth("Proudction Step",90);

		table.addContainerProperty("Batch No",  Label.class , new  Label(),null,null,Table.ALIGN_CENTER);
		table.setColumnWidth("Batch No",70);
		
		table.addContainerProperty("Adjustment No",  Label.class , new  Label(),null,null,Table.ALIGN_CENTER);
		table.setColumnWidth("Adjustment No",85);

		table.addContainerProperty("Adjustment Date",  Label.class , new  Label(),null,null,Table.ALIGN_CENTER);
		table.setColumnWidth("Adjustment Date",90);
