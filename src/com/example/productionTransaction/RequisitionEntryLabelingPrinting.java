...
public class RequisitionEntryLabelingPrinting extends Window 
{
	...
	private ArrayList<TextRead> tblblStockQty = new ArrayList<TextRead>();
	...
	public RequisitionEntryLabelingPrinting(SessionBean sessionBean, String string, int i)
	{
		this.setResizable(false);
		this.setCaption("REQUISITION ENTRY [LABELING/PRINTING/CAP FOLDING/SHRINK] :: "+sessionBean.getCompany());
	...
	private void tbCmbDataSBMLoad(int ar){
		tbCmbItemName.get(ar).removeAllItems();
		Iterator iter=dbService("Select semiFgSubId,semiFgSubName+'#'+color from tbSemiFgSubInformation where productionStepName like 'Stretch Blow Molding'");
		while(iter.hasNext()){
	...
	private void sectionToLoadData() {
		...
		while(iter.hasNext()){
			...
			cmbTo.addItem("Stretch Blow Molding");
		}
	}
	private String getTransactionNo(){

		Iterator iter=dbService("select 0,isNull(max(cast(SUBSTRING(batchNo,CHARINDEX('-',batchNo)+1,len(batchNo)-CHARINDEX('-',batchNo))as int)),0)+1 id from tbRequisitionEntryLabelingPrintingInfo");
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
	...
	private String getReqNo()
	{
		...
		Iterator iter=dbService("select 0,isNull(max(cast(SUBSTRING(reqNo,CHARINDEX('-',reqNo)+1,len(reqNo)-CHARINDEX('-',reqNo))as int)),0)+1 id from tbRequisitionEntryLabelingPrintingInfo");
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
			return element[1].toString();
		}
		...
		cmbFrom.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(cmbFrom.getValue()!=null){
					...
					if(cmbFrom.getValue().toString().equalsIgnoreCase("Dry Offset Printing")){
						txtBatchNo.setValue("DOP-"+getTransactionNo());
					}
					...
					else if(cmbFrom.getValue().toString().equalsIgnoreCase("Heat Trasfer Label")){
						txtBatchNo.setValue("HTL-"+getTransactionNo());
					}
					else if(cmbFrom.getValue().toString().equalsIgnoreCase("Manual Printing")){
						txtBatchNo.setValue("MP-"+getTransactionNo());
					}
					
					else if(cmbFrom.getValue().toString().equalsIgnoreCase("SHRINK")){
						txtBatchNo.setValue("SHR-"+getTransactionNo());
					}
			...
			public void valueChange(ValueChangeEvent event) 
			{
				if(cmbTo.getValue()!=null)
				{
					for(int a=0;a<tblblCode.size();a++){	
						if(cmbTo.getValue().toString().equalsIgnoreCase("Stretch Blow Molding")){
	...
	public boolean deleteData(Session session, Transaction tx) 
	{
		try 
		{
			String sqlUd="insert into tbUdRequisitionEntryLabelingPrintingInfo "
					+ "select  reqNo, batchNo, ReqFrom, ReqTo, ReqDate, userIp, userName, entryTime,"
					+ "'Update' vUdFlag  from tbRequisitionEntryLabelingPrintingInfo where batchNo='"+txtBatchNo.getValue()+ "'";

			String sqlUd1="insert into tbUdRequisitionEntryLabelingPrintingDetails "
					+ "select  batchNo, reqNo, productId, productName, unit, color, reqQty ,"
					+ "'Update' vUdFlag from tbRequisitionEntryLabelingPrintingDetails where batchNo='"+txtBatchNo.getValue()+ "'";

			session.createSQLQuery(sqlUd).executeUpdate();
			session.createSQLQuery(sqlUd1).executeUpdate();
			session.createSQLQuery("delete from tbRequisitionEntryLabelingPrintingInfo where batchNo='"+txtBatchNo.getValue()+ "'").executeUpdate();
			///System.out.println("delete tbLabelProductionInfo where ProductionNo='"+txtProductionNo.getValue()+ "' ");
			session.createSQLQuery("delete from tbRequisitionEntryLabelingPrintingDetails where batchNo='"+txtBatchNo.getValue()+ "'").executeUpdate();
	...
	private void insertData(){
		...
		tx = session.beginTransaction();
		try{
			String sqlInfo="insert into tbRequisitionEntryLabelingPrintingInfo(reqNo,batchNo,ReqFrom,ReqTo,ReqDate,userIp,userName,entryTime) "+
			...
			if(isUpdate){
				String sqlInfoUd="insert into tbUdRequisitionEntryLabelingPrintingInfo(reqNo,batchNo,ReqFrom,ReqTo,ReqDate,userIp,userName,entryTime,vUdFlag) "+
						" values('"+txtreqNo.getValue()+"','"+txtBatchNo.getValue()+"','"+cmbFrom.getValue()+"'," +
						"'"+cmbTo.getValue()+"','"+dateFormat.format(dreqDate.getValue())+"'," +
			...
			for(int a=0;a<tblblCode.size();a++){
				if(tbCmbItemName.get(a).getValue()!=null&&Double.parseDouble("0"+tblblReqQty.get(a).getValue().toString())>0){
					String sqlDetails="insert into tbRequisitionEntryLabelingPrintingDetails(batchNo,ReqNo,productId," +
					...
					if(isUpdate){
						String sqlDetailsUd="insert into tbUdRequisitionEntryLabelingPrintingDetails(batchNo,ReqNo,productId," +
	...
	{
		Window win = new RequisitionLabelingPrintingFindWindow(sessionBean,txtBatchNoFind);
	...
	private void tbCmbAction(int ar){
		...
		if(cmbTo.getValue().toString().equalsIgnoreCase("Stretch Blow Molding")){
			...
			sql="select unit,color,StoreStock,SectionStock"
				+ " from funLebelPrintEntrySBMStock('"+tbCmbItemName.get(ar).getValue()+"',"
						+ "'"+dateFormat.format(dreqDate.getValue())+"') order by semiFgId	";
		}
		else
		{
			sql="select unit,color,semiFgStock,isnull( (select semiFgSectionStock from " +
		...
		Iterator iter=dbService(sql);
			...
			while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
			tblblUnit.get(ar).setValue(element[0]);
			tblblColor.get(ar).setValue(element[1]);
			tblblStockQty.get(ar).setValue(df.format(element[2]));
			tblblSectionStockQty.get(ar).setValue(df.format(element[3]));
		}
	}
	...
	public void tableRowAdd(final int ar)
	{
		try{
			...
			tblblStockQty.add(ar, new TextRead(1));
			tblblStockQty.get(ar).setImmediate(true);
			tblblStockQty.get(ar).setWidth("100%");
			tblblStockQty.get(ar).setHeight("-1px");
			...
			tblblReqQty.get(ar).addListener(new ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					if(!isUpdate){
						double stock=Double.parseDouble(tblblStockQty.get(ar).getValue().toString().isEmpty()?"0.0":tblblStockQty.get(ar).getValue().toString());
			...
			table.addItem(new Object[]{tbLblSl.get(ar),	tbCmbItemName.get(ar),tblblCode.get(ar),tblblUnit.get(ar),tblblColor.get(ar),
					tblblStockQty.get(ar),tblblSectionStockQty.get(ar),tblblReqQty.get(ar)},ar);
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(dreqDate, "top:44.0px;left:720.0px;");
		...
		table.addContainerProperty("Store Stock",  TextRead.class , new TextRead(),null,null,Table.ALIGN_CENTER);
		table.setColumnWidth("Store Stock",90);
