...
public class IssueToAssemble extends Window {
	...
	public IssueToAssemble(SessionBean sessionBean,String caption,int a)
	{
		this.sessionBean=sessionBean;
		this.setResizable(false);
		this.setCaption("ISSUE TO ASSEMBLE :: "+sessionBean.getCompany());
		buildMainLayout();
	...
	private void cmbBatchNoLoad() {
		String sql="select 0,batchNo from tbRequisitionEntryAssembleInfo where ReqFrom like '"+cmbIssueTo.getValue()+"' and ReqTo " +
				" in('Lacqure', 'Dry Offset Printing','Screen Printing','Heat Trasfer Label','Manual Printing') " +
				"and batchNo not in(select batchNo from tbIssueToAssembleInfo where issueTo='Assemble')";
		if(isFind){
			sql="select 0,batchNo from tbRequisitionEntryAssembleInfo where ReqFrom like '"+cmbIssueTo.getValue()+"' and ReqTo in('Lacqure', "+
					" 'Dry Offset Printing','Screen Printing','Heat Trasfer Label','Manual Printing')";
		}
		cmbBatchNo.removeAllItems();
	...
	private void tbCmbAction(int ar){
		Iterator iter=dbService("select sectionStock,assembleStock,reqQty,issuedQty,(reqQty-issuedQty)remainQty from( "+
				...
				" from funAssembleStock('"+tbCmbItemName.get(ar).getValue()+"',CURRENT_TIMESTAMP) "+
				" ) a  ");
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
	...
	private void tableRowAdd(final int rq)
	{
		...
		table.addItem(new Object[]{tbSl.get(rq),tbCmbItemName.get(rq),tbUnit.get(rq),tbColor.get(rq),tbSectionStock.get(rq),
				tbAssembleStock.get(rq),tbReqQty.get(rq),tbIssuedQty.get(rq),tbRemainQty.get(rq),tbIssueQty.get(rq),tbRemarks.get(rq)},rq);

	}
	private void findButtonEvent(){
		Window win = new IssueToAssembleFind(sessionBean, txtItemID);
	...
	private void findInitialise(String issueNo){
		...
		while(iter.hasNext()){
			...
			double stock=0;
			stock=Double.parseDouble(tbSectionStock.get(a).getValue().toString().isEmpty()?"0.00" :tbSectionStock.get(a).getValue().toString())+ Double.parseDouble(element[10].toString())  ;
			tbSectionStock.get(a).setValue(df.format(stock));
