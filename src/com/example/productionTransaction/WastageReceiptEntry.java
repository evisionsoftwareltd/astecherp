...
public class WastageReceiptEntry extends Window 
{
	...
	public WastageReceiptEntry(SessionBean sessionBean) 
	{
	...
	private void tableFindDataLoad(){
		...
		String query=null;
		try{
			query=	" select jobNo, jobDate from tbWastageInfo" +
	...
	private void findInitialise(String findJobNo) 
	{
		...
		try 
		{
			...
			String sql2 = "";
			sql2 = "select itemCode, unit, stockQty, receivedQty  from tbWastageDetails where jobNo='"+findJobNo+ "'";
	...
	private void tableDataAdding(int i)
	{
		...
		try{
			...
			String query ="select vRawItemCode, vRawItemName, vUnitName from tbRawItemInfo"; 
	...
	private void insertData()
	{
		...
		try
		{
			...
			String sql =" insert into tbWastageInfo" +
			...
			String sqlUd =" insert into tbUdWastageInfo" +
			...
			for (int i = 0; i < cmbRawItem.size(); i++)
			{
				...
				if (cmbRawItem.get(i).getValue()!=null)
				{
					...
					String query = 	"insert into tbWastageDetails" +
					...
					String queryUd = 	"insert into tbUdWastageDetails" +
					...
					System.out.println(queryUd);
					
					
					session.createSQLQuery(query).executeUpdate();
					if(isUpdate){
						session.createSQLQuery(queryUd).executeUpdate();
					}
					if(i==0){
	...
	public boolean deleteData(Session session, Transaction tx) 
	{
		try 
		{
			String sql1="insert into tbUdWastageInfo "
					+ "select  jobNo, jobDate, userId, userIp, entryTime ,'Update' vUdFlag "
					+ " from tbWastageInfo where jobNo = '"+txtJobNo.getValue()+ "' ";
			
			session.createSQLQuery(sql1).executeUpdate();
			
			String sql2="insert into tbUdWastageDetails "
					+ "select  jobNo, itemCode, unit, stockQty, receivedQty,'Update' vUdFlag "
					+ " from tbWastageDetails where jobNo = '"+txtJobNo.getValue()+ "' ";
			
			session.createSQLQuery(sql2).executeUpdate();
			
			session.createSQLQuery("delete tbWastageInfo where jobNo='"+txtJobNo.getValue()+ "' ").executeUpdate();
			System.out.println("delete tbWastageInfo where jobNo='"+txtJobNo.getValue()+ "' ");

			session.createSQLQuery("delete tbWastageDetails where jobNo='"+txtJobNo.getValue()+ "' ").executeUpdate();
			System.out.println("delete tbWastageDetails where jobNo='"+txtJobNo.getValue()+ "' ");
	...
	private String autoJobNo()
	{
		...
		try
		{
			...
			Iterator iter = session.createSQLQuery("Select cast(isnull(max(cast(replace(jobNo, '', '')as int))+1, 1)as varchar) from tbWastageInfo").list().iterator();
