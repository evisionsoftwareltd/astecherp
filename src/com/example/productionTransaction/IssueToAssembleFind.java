...
public class IssueToAssembleFind extends Window
{
	...
	public IssueToAssembleFind(SessionBean sessionBean,TextField txtIssueNo)
	{
	...
	private void tableDataLoad() {
		...
		Iterator iter=dbService("select issueNo,issueTo,ReqNo,batchNo,convert(varchar(103),issueDate,103)issueDate from tbIssueToAssembleInfo "+ 
				...
				"and issueTo like '"+issueTo+"' and reqNo  in( "+
				"select reqNo from tbRequisitionEntryAssembleInfo where ReqTo  in('Lacqure','Dry Offset Printing', "+
				"'Screen Printing','Heat Trasfer Label','Manual Printing')) order by issueNo ");
		int a=0;
		if(iter.hasNext()){
			while(iter.hasNext()){
				Object element[]=(Object[])iter.next();
