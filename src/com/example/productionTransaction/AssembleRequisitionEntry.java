...
public class AssembleRequisitionEntry extends Window 
{
	...
	private ArrayList<TextRead> tbLblAssembleStock = new ArrayList<TextRead>();
	...
	public AssembleRequisitionEntry(SessionBean sessionBean, String string, int i)
	{
		this.setResizable(false);
		this.setCaption("REQUISITION ENTRY [ASSEMBLE] :: "+sessionBean.getCompany());
	...
	private void sectionToLoadData() {
		...
		cmbTo.addItem("Lacqure");
		cmbTo.addItem("Dry Offset Printing");
		cmbTo.addItem("Screen Printing");
		cmbTo.addItem("Heat Trasfer Label");
		cmbTo.addItem("Manual Printing");
		cmbTo.addItem("Shrink");
	}
	private void tableClear() {
		for(int a=0;a<tbCmbItemName.size();a++){
			...
			tbLblAssembleStock.get(a).setValue("");
	...
	private void tbCmbDataLoad(String step)
	{
		for(int a=0;a<tbCmbItemName.size();a++)
		{
		...
		Iterator iter=dbService("select FgId,FgName from funAssembleStockNew('%','"+dateFormat.format(dreqDate.getValue())+"') where productionStep like '"+step+"' order by FgName");
		while(iter.hasNext())
		{
			Object element[]=(Object[])iter.next();
	...
	private String getTransactionNo(){
		...
		Iterator iter=dbService("select 0,isNull(max(cast(SUBSTRING(batchNo,CHARINDEX('-',batchNo)+1,len(batchNo)-CHARINDEX('-',batchNo))as int)),0)+1 id from tbRequisitionEntryAssembleInfo");
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
	...
	private String getReqNo(){
		...
		Iterator iter=dbService("select 0,isNull(max(cast(SUBSTRING(reqNo,CHARINDEX('-',reqNo)+1,len(reqNo)-CHARINDEX('-',reqNo))as int)),0)+1 id from tbRequisitionEntryAssembleInfo");
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
			return element[1].toString();
		}
		...
		cmbFrom.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(cmbFrom.getValue()!=null){
					txtreqNo.setValue("Assemble Req-"+getReqNo());
					if(cmbFrom.getValue().toString().equalsIgnoreCase("Assemble")){
						txtBatchNo.setValue("Assemble-"+getTransactionNo());
					}
		...
		cmbTo.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(cmbTo.getValue()!=null){
					...
					if((cmbTo.getValue().toString().equalsIgnoreCase("Dry Offset Printing")||
							cmbTo.getValue().toString().equalsIgnoreCase("Screen Printing")||
							cmbTo.getValue().toString().equalsIgnoreCase("Heat Trasfer Label")||
							cmbTo.getValue().toString().equalsIgnoreCase("Manual Printing"))){
						productionStep="Non Lacqure";
					}
					else if(cmbTo.getValue().toString().equalsIgnoreCase("Lacqure"))
					{
						productionStep="Lacqure";
					}
					else{
						productionStep="Store";
					}
					/*tableClear();
					for(int a=0;a<tbCmbItemName.size();a++)
					{
						tbCmbDataLoad(a,productionStep);
					}*/
					
					tbCmbDataLoad(productionStep);
				}
			}
		});
	...
	public boolean deleteData(Session session, Transaction tx) 
	{
		try 
		{
			String sqlUd="insert into tbUdRequisitionEntryAssembleInfo select  reqNo, batchNo, ReqFrom, "
					+ "ReqTo, ReqDate, userIp, userName, entryTime,'Update' vUdFlag "
					+ "from tbRequisitionEntryAssembleInfo  where batchNo='"+txtBatchNo.getValue()+ "'";
			session.createSQLQuery(sqlUd).executeUpdate();
			
			String sqlUd1="insert into tbUdRequisitionEntryAssembleDetails "
					+ "select  batchNo, reqNo, productId, productName, unit, color, reqQty,'Update' "
					+ "from tbRequisitionEntryAssembleDetails where batchNo='"+txtBatchNo.getValue()+ "'";
			session.createSQLQuery(sqlUd1).executeUpdate();
			
			session.createSQLQuery("delete from tbRequisitionEntryAssembleInfo where batchNo='"+txtBatchNo.getValue()+ "'").executeUpdate();
			///System.out.println("delete tbLabelProductionInfo where ProductionNo='"+txtProductionNo.getValue()+ "' ");

			session.createSQLQuery("delete from tbRequisitionEntryAssembleDetails where batchNo='"+txtBatchNo.getValue()+ "'").executeUpdate();
	...
	private void insertData(){
		...
		tx = session.beginTransaction();
		try{
			String sqlInfo="insert into tbRequisitionEntryAssembleInfo(reqNo,batchNo,ReqFrom,ReqTo,ReqDate,userIp,userName,entryTime) "+
			...
			if(isUpdate){
				String sqlUdInfo="insert into tbUdRequisitionEntryAssembleInfo(reqNo,batchNo,ReqFrom,ReqTo,ReqDate,userIp,userName,entryTime,vUdFlag) "+
						" values('"+txtreqNo.getValue()+"','"+txtBatchNo.getValue()+"','"+cmbFrom.getValue()+"'," +
						"'"+cmbTo.getValue()+"','"+dateFormat.format(dreqDate.getValue())+"'," +
			...
			if(!(cmbTo.getValue().toString().equalsIgnoreCase("Dry Offset Printing")||
					...
					cmbTo.getValue().toString().equalsIgnoreCase("Heat Trasfer Label")||
					cmbTo.getValue().toString().equalsIgnoreCase("Manual Printing")||
					cmbTo.getValue().toString().equalsIgnoreCase("Lacqure")))
			{
				String sqlInfo1="insert into tbRequisitionEntryLabelingPrintingInfo(reqNo,batchNo,ReqFrom,ReqTo,ReqDate,userIp,userName,entryTime) "+
						" values('"+txtreqNo.getValue()+"','"+txtBatchNo.getValue()+"','"+cmbFrom.getValue()+"'," +
						"'"+cmbTo.getValue()+"','"+dateFormat.format(dreqDate.getValue())+"'," +
						"'"+sessionBean.getUserIp()+"','"+sessionBean.getUserName()+"',CURRENT_TIMESTAMP)";
			...
			for(int a=0;a<tblblCode.size();a++){
				if(tbCmbItemName.get(a).getValue()!=null&&Double.parseDouble("0"+tblblReqQty.get(a).getValue().toString())>0){
					...
					String sqlDetails="insert into tbRequisitionEntryAssembleDetails(batchNo,ReqNo,productId," +
					...
					if(isUpdate){
						String sqlUdDetails="insert into tbUdRequisitionEntryAssembleDetails(batchNo,ReqNo,productId," +
					...
					if(!(cmbTo.getValue().toString().equalsIgnoreCase("Dry Offset Printing")||
							...
							cmbTo.getValue().toString().equalsIgnoreCase("Lacqure")))
					{
						String sqlDetails1="insert into tbRequisitionEntryLabelingPrintingDetails(batchNo,ReqNo,productId," +
								"productName,unit,color,reqQty) "+
								" values('"+txtBatchNo.getValue()+"','"+txtreqNo.getValue()+"'," +
								"'"+tbCmbItemName.get(a).getValue()+"','"+tbCmbItemName.get(a).getItemCaption(tbCmbItemName.get(a).getValue())+"'," +
								"'"+tblblUnit.get(a).getValue()+"','"+tblblColor.get(a).getValue()+"','"+tblblReqQty.get(a).getValue()+"')";
	...
	{
		Window win = new AssembleRequisitionEntryFind(sessionBean,txtBatchNoFind);
	...
	private void tbCmbAction(int ar){
		...
		Iterator iter=dbService("select sectionStock,assembleStock from funAssembleStockNew" +
				"('"+tbCmbItemName.get(ar).getValue()+"','"+dateFormat.format(dreqDate.getValue())+"')");
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
			tbLblSectionStock.get(ar).setValue(df.format(element[0]));
			tbLblAssembleStock.get(ar).setValue(df.format(element[1]));
		}
		tblblReqQty.get(ar).focus();
	}
	...
	public void tableRowAdd(final int ar)
	{
		try{
			...
			tbLblAssembleStock.add(ar, new TextRead(1));
			tbLblAssembleStock.get(ar).setImmediate(true);
			tbLblAssembleStock.get(ar).setWidth("100%");
			tbLblAssembleStock.get(ar).setHeight("-1px");
			...
			table.addItem(new Object[]{tbLblSl.get(ar),	tbCmbItemName.get(ar),tblblCode.get(ar),tblblUnit.get(ar),tblblColor.get(ar),
					tbLblSectionStock.get(ar),tbLblAssembleStock.get(ar),tblblReqQty.get(ar)},ar);
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		table.addContainerProperty("Assemble Stock",  TextRead.class , new TextRead(),null,null,Table.ALIGN_CENTER);
		table.setColumnWidth("Assemble Stock",90);
