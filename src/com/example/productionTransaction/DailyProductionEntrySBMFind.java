...
public class DailyProductionEntrySBMFind extends Window
{
	...
	public DailyProductionEntrySBMFind(SessionBean sessionBean,TextField txtProductionNo)
	{
		this.txtProductionNo=txtProductionNo;
		this.sessionBean=sessionBean;
		this.setResizable(false);
		this.setCaption("SBM DAILY PRODUCTION ENTRY FIND :: "+sessionBean.getCompany());
	...
	private void tableDataLoad() {
		...
		Iterator iter=dbService("select productionStep,productionNo,convert(varchar(10),productionDate,103)productionDate from tbSBMDailyProductionInfo  "
				+ "where convert(date,productionDate,105) between '"+dateFormat.format(dFromDate.getValue())+"' and '"+dateFormat.format(dToDate.getValue())+"'  "
						+ "and productionStep like 'Stretch Blow Molding' order by productionNo");
		int a=0;
		if(iter.hasNext()){
			while(iter.hasNext()){
				Object element[]=(Object[])iter.next();
