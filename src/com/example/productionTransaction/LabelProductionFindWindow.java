...
public class LabelProductionFindWindow extends Window
{
	...
	public LabelProductionFindWindow(SessionBean sessionBean,TextField txtProdNo)
	{
		this.txtProdNo = txtProdNo;
		this.sessionBean=sessionBean;
		this.setCaption("FIND LABEL PRODUCTION ENTRY INFO :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("420px");
	...
	private void cmbJobOrderData()
	{
		...
		try
		{
			...
			sql = "select distinct jobOrderNo,0 from tbLabelProductionInfo";
	...
	public void tableRowAdd(final int ar)
	{
		...
		table.addItem(new Object[]{lblProductionNo.get(ar),lblProductionDate.get(ar),lblIssueNo.get(ar)/*,lblRawName.get(ar)*/},ar);
	}
	...
	private void tableDataAdding()
	{
		...
		try{
			...
			String query ="select ProductionNo, ProductionDate, IssueNo from tbLabelProductionInfo where Stepid like " +
					"'"+productionStep+"' and convert(date,ProductionDate,105) between '"+dFormat.format(dFromDate.getValue())+"'" +
					" and '"+dFormat.format(dToDate.getValue())+"' and joborderNo like '"+cmbJobOrderNo.getValue()+"' order by convert(date,ProductionDate,105)";
