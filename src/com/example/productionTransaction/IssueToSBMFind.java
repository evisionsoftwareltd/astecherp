...
public class IssueToSBMFind extends Window
{
	...
	public IssueToSBMFind(SessionBean sessionBean,TextField txtIssueNo)
	{
		this.txtIssueNo=txtIssueNo;
		this.sessionBean=sessionBean;
		this.setResizable(false);
		this.setCaption("ISSUE TO SBM FIND :: "+sessionBean.getCompany());
	...
	private void IssueToLoad() {
		...
		Iterator iter=dbService("select Distinct 0,issueTo from tbIssueToSBMInfo  ");
		while(iter.hasNext()){
	...
	private void tableDataLoad() {
		...
		Iterator iter=dbService("select issueNo,ReqNo,batchNo,"
				+ "convert(varchar(103),issueDate,103)issueDate from tbIssueToSBMInfo "
				+ "where issueDate  between "
				+ "'"+dateFormat.format(dFromDate.getValue())+"' and "
				+ "'"+dateFormat.format(dToDate.getValue())+"' "
				+ "and issueTo like '"+issueTo+"' order by issueNo ");
		int a=0;
		if(iter.hasNext()){
			while(iter.hasNext()){
				Object element[]=(Object[])iter.next();
				tblblIssueNo.get(a).setValue(element[0]);
				tblblReqNo.get(a).setValue(element[1]);
				tblblBatchNo.get(a).setValue(element[2]);
				tblblIssueDate.get(a).setValue(element[3]);
				a++;
	...
	public void tableRowAdd(final int ar)
	{
		try{
			...
			table.addItem(new Object[]{tbLblSl.get(ar),	tblblIssueNo.get(ar),tblblReqNo.get(ar),
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(table, "top:134.0px;left:15.0px;");
		tableInitialise();
