...
/*
	...
	private void tableDataAdding()
	{
		...
		//category = cmbCategoryName.getValue().toString();
				...
				query= " select vRawItemCode,replace(vRawItemName,'~','''') vRawItemName,vGroupName,vSubGroupName,vUnitName,vSubSubCategoryname from tbRawItemInfo where  vCategoryType = '"+cmbCategoryType.getValue().toString()+"' and vGroupId like '"+category+"' and  vSubGroupId like '"+subCategory+"' order by iAutoId "; 
	...
	/*	cmbsubsubCategory.setNullSelectionAllowed(true);
		...
		table.setColumnWidth("Product Name",280);

		table.addContainerProperty("Category Name", Label.class, new Label());
		table.setColumnWidth("Category Name",250);
