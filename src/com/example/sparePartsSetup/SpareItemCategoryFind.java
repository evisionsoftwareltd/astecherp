...
public class SpareItemCategoryFind extends Window
{
	...
	public SpareItemCategoryFind(SessionBean sessionBean,TextField txtReceiptSupplierId)
	{
		this.txtReceiptSupplierId = txtReceiptSupplierId;
		this.sessionBean=sessionBean;
		this.setCaption("FIND SPARE CATEGORY INFO :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("520px");
		this.setHeight("320");
	...
	public void cmbAddCategoryData()
	{
		...
		try
		{
			...
			List list=session.createSQLQuery("Select iCategoryCode, vCategoryName,vCategoryType  from  tbRawItemCategory where vCategoryType  like '%Spare Parts%' and vFlag='New' order by vCategoryName").list();
	...
	private void tableDataAdding()
	{
		try
		{
			...
			String query ="Select iCategoryCode,vCategoryName FROM tbRawItemCategory Where  iCategoryCode like '"+cmbCategoryName.getValue()+"' and vCategoryType='Spare Parts' and vFlag='New' order by iAutoID desc";
