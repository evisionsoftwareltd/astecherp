	...
	private void insertData()
	{
		...
		try{
			...
			String sql= " insert into tbRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,vItemType,dReqDate) "
					...
					+" '"+vocherId+"','"+voucharType+"','"+cmbBatchNo.getValue()+"', 1, (select  vCategoryType from tbRawItemInfo where vRawItemCode='"+tbCmbItemName.get(0).getValue().toString()+"'),'"+dateFormat.format(dReqDate.getValue())+"') ";
			...
			String sqludInfo= " insert into tbudRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,transactionNo,udflag,vItemType) "
					...
					+" 'PT-8','Assemble' ,'' ,"
					+" '"+txtchallanNo.getValue().toString()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"',CURRENT_TIMESTAMP,  "
					+" '"+vocherId+"','"+voucharType+"','"+cmbBatchNo.getValue()+"',1,'"+autoUdTransactionNo()+"','"+udFlag+"',(select  vCategoryType from tbRawItemInfo where vRawItemCode='"+tbCmbItemName.get(0).getValue().toString()+"')) ";
			...
			for (int i = 0; i < tbrate.size(); i++)
			{
				...
				if (temp != null && !tbCmbItemName.get(i).getValue().toString().isEmpty())
				{
					...
					String purchasevoucherquery=" insert into "+voucher+"(Voucher_No,Date,Ledger_Id,Narration,DrAmount,CrAmount,vouchertype,costId,chqClear,userId,userIp,entryTime,auditapproveflag,companyId,attachBill,TransactionWith,sourceForm) "
							...
							+" '2', '"+sessionBean.getCompanyId()+"' ,'','"+cmbsection.getItemCaption(cmbsection.getValue())+"','IssueToAssembleImported') ";
