...
public class RawMaterialsIssueSpare extends Window {
	...
	public RawMaterialsIssueSpare(SessionBean sessionBean) {
	...
	private void reqNoLoadData(String type)
	{
		...
		for(Iterator iter=list.iterator();iter.hasNext();) {
			Object element[]=(Object[]) iter.next();
			cmbReqNo.addItem(element[0]);
			cmbReqNo.setItemCaption(element[0], element[0]+" Date: [ "+element[1]+" ]");
		}
	}
	...
	private AbsoluteLayout buildMainLayout(){
		...
		table.addContainerProperty("Balance Qty(KG)", TextRead.class , new TextRead());
		table.setColumnWidth("Balance Qty(KG)",80);
		...
		mainLayout1.addComponent(lblFromDate,"top:10.0px;left:720.0px;");
		mainLayout1.addComponent(dFromDate,"top:8.0px;left:790.0px;");	
	...
	public void setEventAction(){
		...
		cmbItemType.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				{
                    for(int i=0;i<cmbItemType.size();i++)
                    {
                    	productDataload(i);	
                    }
	...
	private void requisitionEntryLoad(){
		...
		/*ProductionRequistion pr=new ProductionRequistion(sessionBean);
		pr.findInitialise(cmbReqNo.getValue().toString());
		win.addListener(new Window.CloseListener() 
		{
			public void windowClose(CloseEvent e) 
			{
	...
	//	String sql ="select vRawItemCode,vRawItemName from tbRawItemInfo where vCategoryType like '"+cmbItemType.getValue().toString()+"' order by vRawItemName"; 
	...
	private void findInitialise(){
		...
		try
		{
			...
			List led = session.createSQLQuery(" select IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep," +
					"finishedGoods,challanNo,VoucherNo,issueRef,isnull(vItemType,'') vItemType from tbRawIssueInfo where IssueNo " +
					"like '"+txtReceiptId.getValue().toString()+"' ").list();
			...
			String query="select ProductID,Qty,Rate,remarks,storeID,a.groupId from tbRawIssueDetails a " +
	...
	private void insertData(Session session,Transaction tx){
		...
		try
		{
			...
			String sql= " insert into tbRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,vItemType) "
					...
					+" '"+vocherId+"','"+voucharType+"','"+reqNo+"',1,'"+itemtype+"') ";
			...
			for (int i = 0; i < cmbProduct.size(); i++)
			{
				...
				if (temp != null && !amount.get(i).getValue().toString().isEmpty())
				{
					...
					String purchasevoucherquery=" insert into "+voucher+"(Voucher_No,Date,Ledger_Id,Narration,DrAmount,CrAmount,vouchertype,costId,chqClear,userId,userIp,entryTime,auditapproveflag,companyId,attachBill,TransactionWith) "
							+" values('"+vocherId+"','"+new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(dateField.getValue())+"','"+ProductLedeger+"', "  
							+" '"+naration+"','0' , "
