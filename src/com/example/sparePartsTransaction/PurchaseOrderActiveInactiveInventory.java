...
public class PurchaseOrderActiveInactiveInventory extends Window
{
	...
	private Label lblParty = new Label("Supplier Name: ");
	...
	public PurchaseOrderActiveInactiveInventory(SessionBean sessionBean)
	{
	...
	private void findButtonEvent()
	{
		...
		try
		{
			...
			String sql = "select RPOI.poNo,CONVERT(date,RPOI.poDate) poDate,SUM(RPOD.qty) poQty, "
					+ "(select isnull(sum(RPD.qty),0) qty from tbRawPurchaseInfo RP inner join tbRawPurchaseDetails RPD on RP.ReceiptNo=RPD.ReceiptNo inner join  tbRawPurchaseOrderInfo RPO on RP.poNo=RPO.poNo where RPO.poNo like RPOI.poNo) PurchaseReceipt,"
					+ "(SUM(RPOD.qty)-(select isnull(sum(RPD.qty),0) qty from tbRawPurchaseInfo RP inner join tbRawPurchaseDetails RPD on RP.ReceiptNo=RPD.ReceiptNo inner join  tbRawPurchaseOrderInfo RPO on RP.poNo=RPO.poNo where RPO.poNo like RPOI.poNo)) balance "
					+ "from tbRawPurchaseOrderInfo RPOI inner join tbRawPurchaseOrderDetails RPOD on RPOI.poNo=RPOD.poNO  where RPOI.supplierId like '"+suppId+"' and CONVERT(date,RPOI.poDate) between '"+dateF.format(dFromDate.getValue())+"' and '"+dateF.format(dToDate.getValue())+"' and RPOI.vStatus not like 'Inactive' "
					+ "group by RPOI.poNo,CONVERT(date,RPOI.poDate) having "
					+ "(SUM(RPOD.qty)-(select isnull(sum(RPD.qty),0) qty from tbRawPurchaseInfo RP inner join tbRawPurchaseDetails RPD on RP.ReceiptNo=RPD.ReceiptNo inner join  tbRawPurchaseOrderInfo RPO on RP.poNo=RPO.poNo where RPO.poNo like RPOI.poNo))>0 "
					+ "order by CONVERT(date,RPOI.poDate)";
	...
	private void findCancelledData()
	{
		...
		try
		{
			...
			sql = "SElect vPoNo,dPoDate,mTotalQty,mDeliveredQty,mBalanceQty from tbPurchaseOrderCancelInventory "
	...
	private void updateData()
	{
		...
		for(int i = 0; i<tbpoNo.size(); i++)
		{
			if(tbselect.get(i).booleanValue() && !tbpoNo.get(i).getValue().toString().isEmpty())
			{
				...
				try
				{
					insertQuery = "Insert into tbUdPurchaseOrderCancelInventory(vCancelId,vPartyId,vPartyName,"+
					...
					select  = "select vCancelId,vPartyId,vPartyName, vPoNo,dPoDate,mTotalQty,mDeliveredQty,mBalanceQty,vReason,vProductId, "
							...
							+ "from tbPurchaseOrderCancelInventory where vPono like "  
					...
					String 	deleteQuery = "delete from  tbPurchaseOrderCancelInventory where  vPoNo='"+tbpoNo.get(i).getValue().toString()+"'";
					...
					String updateFindQuery="update tbRawPurchaseOrderInfo set vStatus = 'Active',dInactivedate='1900-01-01',vRemark='"+tbReason.get(i).getValue().toString()+"'  where poNo = '"+tbpoNo.get(i).getValue().toString()+"'  and vStatus = 'Inactive' ";
	...
	private void insertData()
	{
		...
		for(int i = 0; i<tbpoNo.size(); i++)
		{
			if(tbselect.get(i).booleanValue() && !tbpoNo.get(i).getValue().toString().isEmpty())
			{
				...
				{
					/*//	if(isFind==false){
					String sql="select productId,(select vRawItemName from tbRawItemInfo where vRawItemCode like tbRawPurchaseOrderDetails.productId),(select vUnitName from tbRawItemInfo where vRawItemCode like tbRawPurchaseOrderDetails.productId) from tbRawPurchaseOrderDetails where poNo like 'ASTECH/264/2019'";
					List<?> lst = session.createSQLQuery(sql).list();
					Iterator<?> iter = lst.iterator(); 
					while(iter.hasNext()){
					Object[] element = (Object[]) iter.next();
					...
					insertQuery = "Insert into tbPurchaseOrderCancelInventory(vCancelId,vPartyId,vPartyName,"+
							" vPoNo,dPoDate,mTotalQty,mDeliveredQty,mBalanceQty,vReason,vProductId,vProductName,"+
							" vProductUnit,vUserIp,vUserName,dEntryTime) "
	...
	private String referenceNo()
	{
		...
		Session session = SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		String sql = "Select isnull(max(cast(SUBSTRING(vCancelId,2,50) as int)),0)+1 from tbPurchaseOrderCancelInventory";
		Iterator<?> iter = session.createSQLQuery(sql).list().iterator();
		if(iter.hasNext())
		{
	...
	private void compInit()
	{
		...
		table.addContainerProperty("Receipt Qty", Label.class, null);
		table.setColumnWidth("Receipt Qty", 100);
