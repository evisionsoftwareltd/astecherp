...
public class FindLoanRcvIssue extends Window{
	...
	public FindLoanRcvIssue(SessionBean sessionBean,TextField txtReceiptId,String frmName)
	{
		...
		try{
			...
			if(frmName.equals("raw"))
			{
				String sql = "select * from " 
						...
						+"where supplierId=a.supplierId)supplier from tbRawPurchaseInfo a where vouchertype like  "
						+"'RDP' and convert(date,Date,105)  between '"+dateformat.format(fromDate.getValue())+"' and '"+dateformat.format(toDate.getValue())+"' and SupplierId like '"+cmbSupplierName.getValue().toString()+"' "
						+"and  purchaseType not like '%loan%' and a.SupplierId not like '%AL%' and flag='0'  "    
								  ...
								  +"'RDP' and convert(date,Date,105) between '"+dateformat.format(fromDate.getValue())+"' and '"+dateformat.format(toDate.getValue())+"' and  SupplierId like '"+cmbSupplierName.getValue().toString()+"' "
								  +"and  purchaseType not like '%loan%' and a.SupplierId  like '%AL%' and flag='0'  " 
			...
			if(frmName.equals("spareIssueLoan"))
			{
				String query= "select IssueNo,IssuedTo,CONVERT(date,Date,105)as Date,(select supplierName from tbSupplierInfo where supplierId like IssuedTo) from tbRawIssueInfo "
						+"where CONVERT(date,Date,105) between '"+dateformat.format(fromDate.getValue())+"' and  '"+dateformat.format(toDate.getValue())+"' and IssuedTo like '"+cmbSupplierName.getValue().toString()+"' and issuetype like '%loan%' order by Date";
				System.out.println(query);
				lst= session.createSQLQuery(query).list();
			}
			...
			if(frmName.equals("MixtureIssue")){
				...
				String sql="select  convert(date,issueDate,105),issueNo,productionTypeName from tbMixtureIssueEntryInfo where issueDate between '"+dateformat.format(fromDate.getValue())+" 00:00:00"+"' and '"+dateformat.format(toDate.getValue())+" 23:59:59.999"+"' order by issueDate";
				lst=session.createSQLQuery(sql).list();
			}
	...
	public void supplierNameData()
	{
		...
		try {
			...
			List<?> list = session.createSQLQuery("select a.SupplierId, b.supplierName from tbRawPurchaseInfo a "
																		+" inner join tbSupplierInfo b on a.SupplierId= b.supplierId "
																		+ "where a.SupplierId not like '%AL%' and transactionType like '%Loan%' order by b.supplierName ").list();
	...
	private void compInit(){
		...
		//mainLayout.addComponent(cmbSupplierName, "top:20.0px; left:10.0px");

	//	mainLayout.addComponent(cButton,"top:18.0px; left:310.0px;");
