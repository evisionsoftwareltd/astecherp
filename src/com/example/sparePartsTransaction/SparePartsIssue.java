...
public class SparePartsIssue extends Window {
	...
	private ComboBox cmbItemType,cmbRackName,cmbSubRackName;
	...
	public SparePartsIssue(SessionBean sessionBean) {
	...
	private void itemTypeDataLoad(){
		...
		List list=dbService("select distinct  0, vCategoryType from tbRawItemInfo where vCategoryType  like '%Spare Parts%' and vflag='New'");
	...
	private void tbCmbProductLoad(){
		...
		for(int a=0;a<cmbProduct.size();a++){
			List list=dbService("select vRawItemCode ,vRawItemname, SUBSTRING(vSubGroupName,CHARINDEX('-',vSubGroupName)+1,LEN(vSubGroupName) )subGroup, "+
					...
					" from  tbRawItemInfo  where vCategoryType like '%Spare Parts%' and vflag='New' order by vGroupId,vSubGroupId,vsubsubCategoryId");
...
*/
		...
		mainLayout1.addComponent((new Label("Rack Name:")), "top:130.0px;left:10.0px;");
		mainLayout1.addComponent(cmbRackName, "top:130.0px;left:122.0px;");
		...
		mainLayout1.addComponent((new Label("SubRack Name:")), "top:130.0px;left:400.0px;");
		mainLayout1.addComponent(cmbSubRackName, "top:130.0px;left:500.0px;");
	...
	private void cmbGroupData() {
		...
		try 
		{
			...
			List list = session.createSQLQuery("select 0,vCategoryType from tbRawItemCategory where vCategoryType like '%Spare Parts%' and vflag='New'").list();
	...
	private void cmbIssueData(){
		...
		try
		{
			...
			List<?>  list = session.createSQLQuery("select * from tbRawIssueInfo  where vItemType='Spare Parts' and vflag='New'").list();
	...
	private void cmbProductAdd(){
		...
		try 
		{
			...
			String sql= "select a.vRawItemCode ,a.vRawItemname, SUBSTRING(b.vSubGroupName,CHARINDEX('-',b.vSubGroupName)+1,LEN(b.vSubGroupName) ) as category  from  tbStandardFinishedDetails a "
					+"inner join "
					+"tbRawItemInfo b "
					+" on  a.vRawItemCode=b.vRawItemCode  where b.vItemType='Spare Parts' and b.vflag='New'";
	...
	{ 		
       ...
       /* if(!isFind)
        {*/
       	  sql ="select vRawItemCode,vRawItemName from tbRawItemInfo where vCategoryType like '%Spare Parts%' and vflag='New' order by vRawItemName"; 
        
       /* else
        {
       	 sql ="select vRawItemCode,vRawItemName from tbRawItemInfo where vCategoryType like '%Spare Parts%'  order by vRawItemName";	 
        }*/
	...
	public void dataload(int a )
	{
		...
		try{
			...
			List list=dbService("select vRawItemCode ,vRawItemname, SUBSTRING(vSubGroupName,CHARINDEX('-',vSubGroupName)+1,LEN(vSubGroupName) )subGroup, "+
					" SUBSTRING(vSubSubCategoryName,CHARINDEX('-',vSubSubCategoryName)+1,LEN(vSubSubCategoryName) )SubSubGroup  "+ 
					" from  tbRawItemInfo  where vCategoryType like '%Spare Parts%' and vflag='New' and vRackId like '"+rackid+"' and vSubRackId like '"+subrackId+"' order by vGroupId,vSubGroupId,vsubsubCategoryId");
	...
	public void cmbSubSubCategoryData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			List list=session.createSQLQuery("select iSubSubCategoryID,vSubSubCategoryName from  tbRawItemsubSubCategory where SubGroupid like '"+subCategory+"' and vflag='New' order by iSubSubCategoryID").list();
	...
	public void cmbSubCategoryData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			List list=session.createSQLQuery(" SELECT SubGroup_Id,vSubCategoryName FROM tbRawItemSubCategory where Group_Id like '"+category+"' and vflag='New' order by vSubCategoryName ").list();
	...
	public void cmbAddRackData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
		String query="select distinct vRackId, vRackName  from  tbRawPurchaseInfo where  purchaseType like '%Spare Parts%' and vflag='New' order by vRackName";
			List list=session.createSQLQuery(query).list();
	...
	public void cmbAddSubRackData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			List list=session.createSQLQuery("select distinct vSubRackId, vSubRackName  from  tbRawPurchaseInfo where vRackId like '"+rackId+"' and vflag='New'").list();
	...
	public void cmbCategoryData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			List list=session.createSQLQuery("select  Group_Id, vCategoryName from tbRawItemCategory  where vCategoryType like '"+categoryType+"' and vflag='New' order by vCategoryName").list();
	...
	private void findButtonEvent(){
		...
		Window win=new FindWindowSP(sessionBean,txtReceiptId,"rawIssue");
		win.addListener(new Window.CloseListener() 
		{
			public void windowClose(CloseEvent e) 
			{
	...
	private void findInitialise(){
		...
		try
		{
			...
			String query= "select IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep," +
					"finishedGoods,challanNo,VoucherNo,issueRef,isnull(vItemType,'') vItemType ,convert(varchar(10),Date,105)date,vRackId,vSubRackId from tbRawIssueInfo where IssueNo " +
			...
			if(led.iterator().hasNext()){
				...
				cmbItemType.setValue("Spare Parts");
				cmbRackName.setValue(element[12].toString());
				cmbSubRackName.setValue(element[13].toString());
	...
	private void saveButtonEvent() {
		...
		if(!issueNo.getValue().toString().trim().isEmpty()){
			...
			showNotification("Warning !","Please Select Product .", Notification.TYPE_WARNING_MESSAGE);	
	}
	...
	private void insertData(Session session,Transaction tx){
		try
		{
			...
			String itemtype="",rackid="",subrackid="",rackName="",subrackName="";
			...
			String sql= " insert into tbRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,vItemType,dReqDate,vRackId,vRackName,vSubRackId,vSubRackName,vflag) "
					...
					+" '"+vocherId+"','"+voucharType+"','"+reqNo+"', 1, '"+itemtype+"','"+reqDate+"','"+rackid+"','"+rackName+"','"+subrackid+"','"+subrackName+"','New') ";
			
			String sqludInfo= " insert into tbudRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,transactionNo,udflag,vItemType,vRackId,vRackName,vSubRackId,vSubRackName,dReqDate,vflag) "
					+" values('"+issueNo.getValue().toString()+"','"+cmbissueTo.getValue().toString()+"', "
					+" '"+dateformat.format(dateField.getValue())+"','"+totalField.getValue().toString()+"', "
					+" '"+cmbProductionType.getValue()+"','' ,'' ,"
					+" '"+txtChallanNo.getValue().toString()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"',CURRENT_TIMESTAMP,  "
					+" '"+vocherId+"','"+voucharType+"','"+reqNo+"',1,'"+autoUdTransactionNo()+"','"+udFlag+"','"+itemtype+"','"+rackid+"','"+rackName+"','"+subrackid+"','"+subrackName+"','"+reqDate+"','New') ";
				...
				if (temp != null && !amount.get(i).getValue().toString().isEmpty())
				{
					...
					/*if(i==0)
					{
						String SupplierVoucherquery=" insert into "+voucher+"(Voucher_No,Date,Ledger_Id,Narration,DrAmount,CrAmount,vouchertype,costId,chqClear,userId,userIp,entryTime,auditapproveflag,companyId,attachBill,TransactionWith,sourceForm) "
								...
								+" '2', '"+sessionBean.getCompanyId()+"','' ,'"+cmbissueTo.getItemCaption(cmbissueTo.getValue())+"','issue') ";
