...
public class StockSearch extends Window
{
	...
	public StockSearch(SessionBean sessionBean)
	{
	...
	public void productNameLoad(String Id)
	{
		cmbProductName.removeAllItems();
		try {
			String sql="select vRawItemCode,vRawItemName from tbRawItemInfo where vGroupId like '"+Id+"'  "
	...
	public void subCateGoryDataAdd(String catId)
	{
		...
		try {
			...
			String sql="select  iSubCategoryID,vSubCategoryName from  tbRawItemSubCategory where Group_Id like '"+catId+"' ";
			Iterator<?>iter=dbService(sql);
			while(iter.hasNext())
			{
				Object[] element = (Object[]) iter.next();
	...
	private void cmbCategoryLoad(){
		cmbCategoryName.removeAllItems();
		String sql="select  iCategoryCode,vCategoryName from  tbRawItemCategory where vCategoryType like '"+cmbParentType.getValue().toString()+"'";
		Iterator<?>iter=dbService(sql);
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
	...
	private void cmbParenTypeDataLoad(){
		...
		String sql="select distinct 0,vCategoryType  from tbRawItemCategory where vCategoryType not like '%Spare Parts%'";
		Iterator<?>iter=dbService(sql);
		while(iter.hasNext()){
			Object element[]=(Object[])iter.next();
