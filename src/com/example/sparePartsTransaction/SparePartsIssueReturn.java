...
public class SparePartsIssueReturn extends Window {
	...
	public SparePartsIssueReturn(SessionBean sessionBean) {
	...
	private AbsoluteLayout buildMainLayout(){
		...
		mainLayout1.addComponent((new Label("Rack Name:")), "top:20.0px;left:705.0px;");
		mainLayout1.addComponent(cmbRackName, "top:18.0px;left:795.0px;");
		...
		mainLayout1.addComponent((new Label("SubRack Name:")), "top:46.0px;left:705.0px;");
		mainLayout1.addComponent(cmbSubRackName, "top:44.0px;left:795.0px;");
...
/*	private void cmbGroupData() {
	...
	public void cmbAddRackData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
		String query="select distinct vRackId, vRackName  from  tbRawIssueInfo where  vItemType like '%Spare Parts%' and vflag='New' order by vRackName";
	...
	public void cmbAddSubRackData()
	{
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			List list=session.createSQLQuery("select distinct vSubRackId, vSubRackName  from  tbRawIssueInfo where vRackId like '"+rackId+"'  and vflag='New'").list();
	...
	private void issueTo(){
		...
		try{
			...
			List list=session.createSQLQuery("select distinct IssuedTo,ISNULL((select SectionName from tbSectionInfo where cast(AutoID as varchar)=IssuedTo),IssuedTo)SectionName from tbRawIssueInfo").list();
		...
		try{
			...
			if(!isFind)
			{
				query=  "select distinct 0, a.IssueNo from tbRawIssueInfo a "
						...
						+"on "
						+"a.IssueNo=b.IssueNo "
						+"where b.returnFlag like '1' and a.IssuedTo like '"+cmbrteturnFrom.getValue().toString()+"'  and vflag='New'";	
			}
			...
			if(isFind)
			{
				query=  "select distinct 0, a.IssueNo from tbRawIssueInfo a "
						...
						+"on "
						+"a.IssueNo=b.IssueNo "
						+"where  a.IssuedTo like '"+cmbrteturnFrom.getValue().toString()+"' and vflag='New'";		
			}
...
/*			String sql="select vRawItemCode,vRawItemName,subString(vSubGroupName,CHARINDEX('-',vSubGroupName)+1,LEN(vSubGroupName))as category,"
			...
			*/
			...
			String sql= "select a.vRawItemCode ,a.vRawItemname, SUBSTRING(b.vSubGroupName,CHARINDEX('-',b.vSubGroupName)+1,LEN(b.vSubGroupName) ) as category  from  tbStandardFinishedDetails a "
					    +"inner join "
					    +"tbRawItemInfo b "
					    +" on  a.vRawItemCode=b.vRawItemCode and b.vflag='New'";
...
/*			
	...
	private void findButtonEvent(){
		...
		Window win=new IssueReturnFindSP(sessionBean,txtReceiptId,"rawIssue");
	...
	private void findInitialise(){
		...
		try
		{
			...
			txtClear();
			String sql= "Select transactionNo,transactionDate,challanNo,issueNo,VoucherNo,total,returnForm ,vRackId,vSubRackId from tbIssueReturnInfo  "
					    +" where transactionNo like '"+txtReceiptId.getValue().toString()+"' and vflag='New'";
			...
			if(led.iterator().hasNext()){
				...
				cmbRackName.setValue(element[7].toString());
				cmbSubRackName.setValue(element[7].toString());
	...
	private void insertData(Session session,Transaction tx){
		try
		{
			...
			String rackid="",subrackid="",rackName="",subrackName="";
			if(cmbRackName.getValue()!=null)
			{
			...
			String sql= "  insert into tbIssueReturnInfo(transactionNo,transactionDate,challanNo,issueNo,returnForm,userid,userIp,entryTime,VoucherNo,VoucherType,total,vRackId,vRackName,vSubRackId,vSubRackName,vflag)   "
					  + " values ('"+txttransactionNo.getValue().toString()+"','"+datef.format(dreturnDate.getValue()) +"', "
					  +"  '"+challanNo+"','"+issueno+"','"+sectionm+"','"+sessionBean.getUserId()+"','"+sessionBean.getUserIp()+"',getdate(),'"+vocherId+"','"+voucharType+"','"+total+"','"+rackid+"','"+rackName+"','"+subrackid+"','"+subrackName+"','New')   ";
					...
					if(!txtissueRturnQty.get(ar).getValue().toString().isEmpty())
					{
						...
						if(!isFind)
						{
							query=  " select 0, ISNULL(SUM(qty),0) as amount  from tbIssueReturnDetails a "
									...
									+"where b.issueNo like '"+cmbIssueNo.getValue().toString()+"' and a.productId like '"+cmbProduct.get(ar).getValue().toString()+"'  and b.vflag='New'";	
						}
						...
						if(isFind)
						{
							...
							query=  " select 0, ISNULL(SUM(qty),0) as amount  from tbIssueReturnDetails a "
									...
									+"on "
									+"b.transactionNo=a.transactionNo "
									+"where b.issueNo like '"+cmbIssueNo.getValue().toString()+"' and a.productId like '"+cmbProduct.get(ar).getValue().toString()+"' and a.transactionNo not like '"+txttransactionNo.getValue().toString()+"' and b.vflag='New'";
						}
...

					{
						...
						if (fla )
						{
							...
							query=  " select  ProductID,b.vRawItemName,Qty,Rate,b.vUnitName from  tbRawIssueDetails a "
									...
									+"where a.IssueNo like '"+cmbIssueNo.getValue().toString()+"' and a.ProductID like '"+cmbProduct.get(ar).getValue().toString()+"' and b.vflag='New'" ;
