...
public class RptSemiFgProductionDetailsAsOnDate extends Window{
	...
	public RptSemiFgProductionDetailsAsOnDate(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("SEMI FG PRODUCTION DETAILS AS ON DATE::"+sessionBean.getCompany());
	...
	private void cmbSemiFgLoad() {
		...
		try
		{
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			String sql="select semiFgCode,semiFgName,color from tbSemiFgInfo where productionTypeId like '"+cmbType.getValue()+"' order by semiFgName";
			List list=session.createSQLQuery(sql).list();
			for(Iterator iter=list.iterator();iter.hasNext();){
				Object element[]=(Object[]) iter.next();
	...
	{	
		...
		try{
			...
			query="select * from funcSemiFgWiseProductionAsOnDate('"+SemiFg+"','"+datef.format(formDate.getValue())+"')";
			hm.put("sql", query);

			String subQuery="select a.semiFgName,a.unit,a.color,sum(a.totalProduction)totalProduciton,sum(a.totalRejection)totalRejection " +
					"from( select * from funcSemiFgWiseProductionAsOnDate('"+SemiFg+"','"+datef.format(formDate.getValue())+"') )a group by a.semiFgName,a.unit,a.color";
			hm.put("subQuery", subQuery);
			hm.put("SUBREPORT_DIR", "./report/production/");

			List list=session.createSQLQuery(query).list();
			if(!list.isEmpty()){
				Window win = new ReportViewerNew(hm,"report/production/SemiFgProductionAsOnDate.jasper",
	...
	private AbsoluteLayout buildMainLayout() {
		...
		mainLayout.addComponent(lblProductionType, "top:22.0px;left:20.0px;");		
		...
		mainLayout.addComponent( cmbType, "top:20.0px;left:140.0px;");
		
		chkAll.setImmediate(true);
		chkAll.setWidth("-1px");
		chkAll.setHeight("-1px");
		mainLayout.addComponent(chkAll, "top:47.0px;left:450.0px;");
		...
		mainLayout.addComponent(lblFDate, "top:72.0px;left:20.0px;");
		...
		mainLayout.addComponent( formDate, "top:70.0px;left:140.0px;");
