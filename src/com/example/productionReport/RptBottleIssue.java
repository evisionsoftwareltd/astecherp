...
public class RptBottleIssue extends Window{
	...
	public RptBottleIssue(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("MOULDING PRODUCTION ISSUE::"+sessionBean.getCompany());
	...
	private void challandataload() {
		...
		try{
			...
			Session session=SessionFactoryUtil.getInstance().getCurrentSession();
			tx=session.beginTransaction();
			query="select distinct  0,ChallanNo from tbMouldingIssueInfo where IssueTo like '"+cmbTo.getValue()+"'  ";
	...
	{	
		...
		/*	query = " select  a.jobDate,a.jobNo,a.ChallanNo, b.fgName ,a.issueQty,a.itemCode ," +
			...
			query="select b.Fgid,c.vProductName,c.vUnitName,c.vCategoryId,c.vCategoryName,a.challanNo,a.jobNo,a.jobDate from  " +
					...
					"like  '"+cmbFrom.getValue()+"' and a.IssueTo like '"+cmbTo.getValue()+"'  and a.challanNo like '"+cmbchallan.getValue()+"' ";
