...
public class RptFgOpening extends Window{
	...
	public RptFgOpening(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("FG OPENING STOCK ::"+sessionBean.getCompany());
	...
	private void cmbSemiFgLoad() {
		...
		try
		{
			...
			String sql="select distinct fgCode,fgName from tbFgOpening where CONVERT(date,entryTime,105)>'2020-01-31' order by fgName";
	...
	{	
		...
		try{
			...
			if(cmbSemiFgName.getValue()!=null){
				...
				query="select openingYear,productionStepName,fgName,openingQty,rate,amount from tbFgOpening where fgCode like '"+semiFg+"' and CONVERT(date,entryTime,105)>'2020-01-31' order by productionStepName,fgName";
				hm.put("sql", query);
				List list=session.createSQLQuery(query).list();
				if(!list.isEmpty()){
					Window win = new ReportViewerNew(hm,"report/production/RptFinishedGoodsOpeninggStock.jasper",
							this.getWindow().getApplication().getContext().getBaseDirectory()+"".replace("\\","/")+"/VAADIN/rpttmp",
							this.getWindow().getApplication().getURL()+"VAADIN/rpttmp",false,
							this.getWindow().getApplication().getURL()+"VAADIN/applet",type);
					win.setCaption("Project Report");
	...
	/*	lblProductionType = new Label();
		...
		mainLayout.addComponent(chklayout, "top:90.0px; left:140.0px");
		...
		mainLayout.addComponent(exitButton,"top:160.opx; left:250.0px");
