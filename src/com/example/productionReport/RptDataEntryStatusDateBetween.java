...
public class RptDataEntryStatusDateBetween extends Window{
	...
	public RptDataEntryStatusDateBetween(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("PRODUCTION DATA ENTRY STATUS::"+sessionBean.getCompany());
	...
	{	
		...
		try{
			...
			query="select date,Convert(varchar,DATENAME(WEEKDAY,date)) dayName,blowMoulding,injectionMoulding,injectionBlowMoulding,stretchBlowMoulding,labeling,capFolding,dop,screenPrinting,htl,manualPrinting,lacqure,assemble,shrink "
					+ "from funcProductionDataEntry('"+datef.format(formDate.getValue())+"','"+datef.format(toDate.getValue())+"','"+productionType+"')";
			...
			if(!list.isEmpty()){
				Window win = new ReportViewerNew(hm,"report/production/RptEntryStatus.jasper",
