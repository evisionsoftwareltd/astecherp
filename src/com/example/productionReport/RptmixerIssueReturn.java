...
public class RptmixerIssueReturn extends Window
{
	...
	public RptmixerIssueReturn(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("Mixer Issue Return::"+sessionBean.getCompany());
	...
	private void productionTypeDataLoad()
	{
		try
		{
			...
			String sql=	
					"select distinct productionTypeId,productionTypeName from tbMixtureIssueReturnEntryInfo " 
	...
	private void Mouldata(String semifg) 
	{
		...
		try
		{
			...
			String sql= "select distinct b.mouldId,b.mouldName from tbMixtureIssueReturnEntryInfo a inner join  "
					    +"tbMixtureIssueReturnEntryDetailsEntry b on a.returnNo=b.returnNo  "
	...
	private void machineDataload(String mould) 
	{
		...
		try
		{
			...
			String sql=
					"select distinct b.machineId,b.machineName from tbMixtureIssueReturnEntryInfo a inner join " 
					+"tbMixtureIssueReturnEntryDetailsEntry b on a.returnNo=b.returnNo " 
	...
	private void semiFgDataLoad() 
	{
		...
		try
		{
			...
			String sql="select distinct b.semiFgId,b.semiFgName,(select color from tbSemiFgInfo where semiFgCode=b.semiFgId)" +
					" from tbMixtureIssueReturnEntryInfo a inner join "
					   +"tbMixtureIssueReturnEntryDetailsEntry b on a.returnNo=b.returnNo "
	...
	{	
		...
		try{
			...
			hm.put("type",cmbType.getItemCaption(cmbType.getValue()) );
			 ...
			 {
				System.out.println("Segrecate Array Name is :"+name1[index]); 
			 }
			 
			 for(int index=0;index<data.size();index++)
			 {
				System.out.println("Segrecate ArrayList Name is :"+data.get(index)); 
			 }
			 
			 

			 query=  "select a.returnDate, a.productionTypeName,b.semiFgName,b.semiFgUnit,b.mouldName,b.machineName,ReturnQty,ReturnBag from tbMixtureIssueReturnEntryInfo a "
					 +"inner join tbMixtureIssueReturnEntryDetailsEntry b on a.returnNo=b.returnNo "
					 ...
					 +"and convert(date,a.returnDate,105)  between '"+datef.format(formDate.getValue())+"' and '"+datef.format(toDate.getValue())+"' order by productionTypeName,semiFgName,mouldName,machineName ";
			...
			if(!list.isEmpty()){
				Window win = new ReportViewerNew(hm,"report/production/RptMixtureIssueReturn.jasper",
