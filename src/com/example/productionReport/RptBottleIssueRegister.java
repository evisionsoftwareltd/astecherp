...
public class RptBottleIssueRegister extends Window{
	...
	public RptBottleIssueRegister(SessionBean sessionBean,String s){
	...
	{	
		...
		try{
			...
			query= "select b.Fgid,c.vProductName,c.vUnitName,c.vCategoryId,c.vCategoryName,a.challanNo,"
			+"a.jobNo,a.jobDate from  tbMouldingIssueInfo as a inner join tbMouldingFgIssueDetails"
			+" b on a.jobNo=b.jobNo inner join tbFinishedProductInfo c on c.vProductId=b.Fgid where" 
			+" a.IssueFrom like '"+cmbFrom.getValue()+"' and a.IssueTo like '"+cmbTo.getValue()+"' and CONVERT(date,a.jobDate,105) between"
			+" '"+datef.format(fromDate.getValue())+"' and  '"+datef.format(toDate.getValue())+"'";
			
			Subquery= "select b.productId,c.vRawItemName,c.vUnitName,a.challanNo,a.jobNo,a.jobDate,b.sectionissue," +
					"b.wastageissue from  tbMouldingIssueInfo as a inner join tbMouldingRawIssueDetails b on " +
					"a.jobNo=b.jobNo inner join tbRawItemInfo c on c.vRawItemCode=b.productId where a.issueFrom " +
					"like '"+cmbFrom.getValue()+"' and a.IssueTo like '"+cmbTo.getValue()+"' and CONVERT(date,a.jobDate,105) between '"+datef.format(fromDate.getValue())+"' and " +
					" '"+datef.format(toDate.getValue())+"'";
			...
			List list=session.createSQLQuery(query).list();
			if(!list.isEmpty()){
				Window win = new ReportViewerNew(hm,"report/production/rptMoudingIssueDateBetween.jasper",
