...
public class RptLacqureStock extends Window{
...
//	private static final List<String> OptionValue=Arrays.asList(new String[]{"With Value","All FG"});
	...
	public RptLacqureStock(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("LACQURE STOCK ::"+sessionBean.getCompany());
	...
	private void cmbSemiFgLoad() {
		...
		try
		{
			...
			String sql="select distinct productId,productName from tbIssueToLacqureDetails ";
	...
	{	
		...
		try{
			...
			//{
			    query= "select * from funcLacqureStockDateBetween('"+semiFg+"','"+datef.format(formDate.getValue())+"','"+datef.format(toDate.getValue())+"')order by productName ";
				...
				List list=session.createSQLQuery(query).list();
				if(!list.isEmpty()){
					Window win = new ReportViewerNew(hm,"report/production/rptLacqureStock.jasper",
							this.getWindow().getApplication().getContext().getBaseDirectory()+"".replace("\\","/")+"/VAADIN/rpttmp",
							this.getWindow().getApplication().getURL()+"VAADIN/rpttmp",false,
							this.getWindow().getApplication().getURL()+"VAADIN/applet",type);
					win.setCaption("Project Report");
