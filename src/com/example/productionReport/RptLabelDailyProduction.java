...
public class RptLabelDailyProduction extends Window{
	...
	public RptLabelDailyProduction(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("LABEL DAILY PRODUCTION REPORT::"+sessionBean.getCompany());
...
//			hm.put("todate", new SimpleDateFormat("dd-MM-yyyy").format(toDate.getValue()));
			...
			query = "select c.vProductName, ISNULL(c.Dia,'') as mDia,b.ShiftASqm,b.ShiftAQty,b.ShiftBSqm,b.ShiftBQty ,(b.ShiftASqm+b.ShiftBSqm)as totalsqm,(b.ShiftAQty+b.ShiftBQty)as totalqty ,b.WastageSqm,b.WastageQty,b.WastagePercent  from tbLabelProductionInfo a inner join tbLabelProductionDetails b on a.ProductionNo=b.ProductionNo inner join tbFinishedProductInfo c on c.vProductId=b.FinishedProduct  where a.Stepid like '"+cmbStep.getValue()+"' and CONVERT(date,a.ProductionDate,105) like '"+datef.format(formDate.getValue())+"' ";

			String sqlConsumption = "";
			sqlConsumption = "select a.rawItemCode,e.vRawItemName,  ISNULL(sum((b.ShiftASqm+b.ShiftBSqm+b.WastageSqm)),0) as totalsqm, ISNULL(sum((b.ShiftAQty+b.ShiftBQty+b.WastageQty)),0)  as totalqty  from tbLabelProductionInfo a inner join tbLabelProductionDetails b on a.ProductionNo=b.ProductionNo inner join tbFinishedProductInfo c on c.vProductId=b.FinishedProduct  inner join tbRawItemInfo e on e.vRawItemCode=a.rawItemCode where a.Stepid like '"+cmbStep.getValue()+"' and CONVERT(date,a.ProductionDate,105) like '"+datef.format(formDate.getValue())+"' group by  a.rawItemCode,e.vRawItemName" ;
