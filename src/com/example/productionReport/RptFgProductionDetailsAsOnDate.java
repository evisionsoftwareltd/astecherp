...
public class RptFgProductionDetailsAsOnDate extends Window{
	...
	public RptFgProductionDetailsAsOnDate(SessionBean sessionBean,String s){

		this.sessionBean=sessionBean;
		this.setCaption("FG PRODUCTION DETAILS AS ON DATE::"+sessionBean.getCompany());
	...
	{	
		...
		try{
			...
			query="select * from funcFgProductionAsOnDate('"+SemiFg+"','"+datef.format(formDate.getValue())+"')";
			hm.put("sql", query);
			...
			String subQuery="select a.semiFgName,a.unit,a.color,sum(a.totalProduction)totalProduciton,sum(a.totalRejection)totalRejection " +
					"from( select * from funcFgProductionAsOnDate('"+SemiFg+"','"+datef.format(formDate.getValue())+"') )a group by a.semiFgName,a.unit,a.color";
			hm.put("subQuery", subQuery);
			hm.put("SUBREPORT_DIR", "./report/production/");

			List list=session.createSQLQuery(query).list();
			if(!list.isEmpty()){
				Window win = new ReportViewerNew(hm,"report/production/FgProductionAsOnDate.jasper",
	...
	private AbsoluteLayout buildMainLayout() {
		...
		mainLayout.addComponent(lblFgName, "top:47.0px;left:20.0px;");
		...
		mainLayout.addComponent( cmbFgName, "top:45.0px;left:140.0px;");
