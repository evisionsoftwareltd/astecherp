...
public class GoodsReceiveNote extends Window {
		...
		try {
			...
			if(caption.equalsIgnoreCase("Local Credit/Cash Purchase"))
			{
				sql ="select distinct '1' as type ,a.supplierId,b.supplierName from tbRawPurchaseInfo a "
				 		+ "inner join tbSupplierInfo b on a.supplierId=b.supplierId where a.SupplierId not like '%AL%' and a.vflag='New'";	
			}
			if(caption.equalsIgnoreCase("L / C"))
			{
				 sql = "select distinct '2' as type,  a.supplierId,(select Ledger_Name from tbLedger  "
				 		+ "where Ledger_Id=a.supplierId) from tbRawPurchaseInfo a  where a.SupplierId  like '%AL%' and a.vflag='New' order by type ";
			}
	...
	private void addChallanNo()
	{
		...
		String query="select  distinct 0, MrrNo  from tbRawPurchaseInfo where SupplierId like '"+cmbSupplier.getValue()+"' and vflag='New'";
