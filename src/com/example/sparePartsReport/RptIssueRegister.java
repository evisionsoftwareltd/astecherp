...
public class RptIssueRegister extends Window {
	...
	public  RptIssueRegister(SessionBean sessionBean,String str) {
		this.sessionBean = sessionBean;
		buildMainLayout();
		setContent(mainLayout);
		this.setResizable(false);
		this.setCaption("ITEM ISSUE REGISTER::  "+ sessionBean.getCompany());
	...
	//ISNULL(Replace(CONVERT(char(10),Reqdate,103),'/','-'),'1900-01-01') ReqDate,
			String sql= 
					   "select * from"
					   + " ( select  Reqdate, b.ProductID, c.vRawItemName,c.vUnitName,a.IssueNo,a.Date,a.IssuedTo,(SElect SectionName from tbSectionInfo where  CAST(AutoID as varchar)=a.IssuedTo) SectionName, "
					   ...
					   +"c.vProductType,e.productTypeName,f.StepName,g.vProductName,c.vCategoryType, ISNULL(rq.ReqQty,0)ReqQty from tbRawIssueInfo a "
					   +"inner join tbRawIssueDetails b on a.IssueNo=b.IssueNo inner join tbRawItemInfo c on c.vRawItemCode=b.ProductID "
					   + "left join tbProductionType e on e.productTypeId=a.ProductionType left join tbProductionStep f "
					   +"on f.StepId=a.productionStep left join tbFinishedProductInfo g on g.vProductId=a.finishedGoods "
					   +"left join tbProductionRmDetails rq  on rq.ReqNo=a.IssueRef and rq.rawItemCode=b.ProductID  "
					   +" left join tbProductionReqInfo pr on rq.ReqNo=pr.ReqNo "
					   +"where CONVERT(Date,a.Date,105)  between '"+new SimpleDateFormat("yyyy-MM-dd").format(pdffromdate.getValue())+"' and '"+new SimpleDateFormat("yyyy-MM-dd").format(pdftodate.getValue())+"' and c.vCategoryType like "
					   +" '"+categirytype+"'  and c.vGroupId like '"+categoryID+"' and b.ProductID like '"+productID+"' and a.IssueType is null "
					   +"and c.vSubGroupId like '"+subcategoryID+"'    union all " 
					   +"select ReqDate, b.ProductID, c.vRawItemName,c.vUnitName,a.IssueNo, "
					   +"a.Date,a.IssuedTo,d.supplierName,  a.ProductionType,a.productionStep,a.finishedGoods,a.challanNo, "
					   +"b.Qty,b.Rate,(b.Qty*b.Rate) as amount,  c.vGroupId,c.vGroupName,c.vSubGroupId,c.vSubGroupName , "
					   +"c.vsubsubCategoryId,c.vSubSubCategoryName,  c.vProductType,e.productTypeName,f.StepName, "
					   +"g.vProductName,c.vCategoryType, ISNULL(rq.ReqQty,0)ReqQty   from tbRawIssueInfo a inner join tbRawIssueDetails b on "
					   +"a.IssueNo=b.IssueNo  inner join tbRawItemInfo c  on c.vRawItemCode=b.ProductID  inner join "
					   +"tbSupplierInfo d  on d.supplierId=a.IssuedTo  left join tbProductionType e "
					   +"on e.productTypeId=a.ProductionType  left join tbProductionStep f "
					   +"on f.StepId=a.productionStep  left join tbFinishedProductInfo g " 
					   +"on g.vProductId=a.finishedGoods left join tbProductionRmDetails rq on rq.ReqNo=a.IssueRef and rq.rawItemCode=b.ProductID "
					   + " left join tbProductionReqInfo pr on rq.ReqNo=pr.ReqNo "
					   + "  where CONVERT(Date,a.Date,105) "
					   +"between '"+new SimpleDateFormat("yyyy-MM-dd").format(pdffromdate.getValue())+"' and '"+new SimpleDateFormat("yyyy-MM-dd").format(pdftodate.getValue())+"'  and c.vCategoryType like '"+categirytype+"' "
					   +"and c.vGroupId like '"+categoryID+"' and b.ProductID like '"+productID+"' and a.IssueType is not null " 
					   +"and c.vSubGroupId like '"+subcategoryID+"'  ) "
					   		+ "as temp order by temp.Date "; 
			...
			if(!lst.isEmpty())
			{
				...
				Window win = new ReportViewer(hm,"report/raw/rptIssueRegister.jasper",
