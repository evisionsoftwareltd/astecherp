...
public class ItemWisePurcaseRegisterSPRpt extends Window{
	...
	public ItemWisePurcaseRegisterSPRpt(SessionBean sessionBean,String s){
		...
		cmbparentype.setValue("Spare Parts");
		cmbparentype.setEnabled(false);
		cmbItemDataLoad("Spare Parts");
		cmbItem.focus();
	}
	...
	private void cmbItemDataLoad(String parenttype) 
	{
		...
		try{
			...
			String sql=		"select distinct b.ProductID,c.vRawItemName from tbRawPurchaseInfo a "
					...
					+"where c.vCategoryType like '"+parenttype+"' and a.vflag='New' ";
		...
		try{
			...
			*/
			...
			query= 	"select rpi.Date,rpi.ReceiptNo,rpi.purchaseType,rpi.poNo,rpi.ChallanNo,rpi.challanDate,  si.supplierName,rpd.Qty,rpd.Rate, "
			        ...
			        +"like '"+parentype+"' and rpi.vflag='New' and transactionType  not in ('L / C','Cash Purchase') and rpi.SupplierId not like 'AL%'"
			     ...
			     +"left join tbRawPurchaseOrderInfo poi on rpi.poNo=poi.poNo where CONVERT(Date, rpi.Date,105)  "
			     +"between '"+datef.format(formDate.getValue())+"' and '"+datef.format(toDate.getValue())+"' and pi.vRawItemCode like '"+productId+"' and  PI.vCategoryType  "
			     +"like '"+parentype+"' and rpi.vflag='New' and transactionType  in ('L / C','Cash Purchase') and rpi.SupplierId  like 'AL%'   order by PI.vCategoryType , pi.vRawItemCode,rpi.Date, rpi.ReceiptNo " ;
