...
public class ThirdPartyItemFind extends Window
{
	...
	public  ThirdPartyItemFind(SessionBean sessionBean,TextField txtReceiptSupplierId,String frmName)
	{
		this.txtReceiptSupplierId = txtReceiptSupplierId;
		this.sessionBean=sessionBean;
		this.setCaption("THIRD PARTY  ITEM FIND :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("910px");
	...
	private void tableDataAdding()
	{
		...
		try
		{
			...
			if(isFind)
			{
				...
				query= "Select vCode,replace(vItemName,'~','''') vRawItemName,vGroupName,vSubGroupName,vUnitName,imageLoc from tbThirdPartyItemInfo where  vCategoryType = '"+cmbCategoryType.getValue().toString()+"' and vGroupId like '"+category+"' and  vSubGroupId like '"+subCategory+"' order by iAutoId "; 
			...
			if(!list.isEmpty())
			{
				...
				{						  
					...
					if(!element[5].toString().equals("0")&&!element[6].toString().equals("null")){
						ImageforPopup(element[5].toString(),i);
					}
					else{
						btnPopupView.get(i).setVisible(false);
					}
