...
/*			
	...
	private void insertData(Session session,Transaction tx){
		try
		{
			...
			String sql= "  insert into tbIssueReturnInfo(transactionNo,transactionDate,challanNo,issueNo,returnForm,userIp,entryTime,VoucherNo,VoucherType,total,userid,vflag)   "
					  + " values ('"+txttransactionNo.getValue().toString()+"','"+datef.format(dreturnDate.getValue()) +"', "
					  +"  '"+challanNo+"','"+issueno+"','"+sectionm+"','"+sessionBean.getUserIp()+"',getdate(),'"+vocherId+"','"+voucharType+"','"+total+"','"+sessionBean.getUserName()+"','OLD')   ";
				...
				if (cmbProduct.get(i).getValue()!=null && ! txtissueRturnQty.get(i).getValue().toString().isEmpty())
				{
					...
					if(i==0)
					{
						String SupplierVoucherquery=" insert into "+voucher+"(Voucher_No,Date,Ledger_Id,Narration,DrAmount,CrAmount,vouchertype,costId,chqClear,userId,userIp,entryTime,auditapproveflag,companyId,attachBill,TransactionWith,sourceForm) "
								...
								+" '2', '"+sessionBean.getCompanyId()+"','' ,'"+cmbrteturnFrom.getItemCaption(cmbrteturnFrom.getValue())+"','issueReturn') ";
