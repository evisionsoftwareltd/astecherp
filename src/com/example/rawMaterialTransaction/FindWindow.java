...
public class FindWindow extends Window{
	...
	public FindWindow(SessionBean sessionBean,TextField txtReceiptId,String frmName)
	{
		...
		try{
			...
			if(frmName.equals("raw"))
			{
				String sql = "select * from " 
						...
						+"and  purchaseType not like '%loan%' and a.SupplierId not like '%AL%' and VoucherNo='' "    
								  ...
								  +"'RDP' and convert(date,Date,105) between '"+dateformat.format(fromDate.getValue())+"' and '"+dateformat.format(toDate.getValue())+"' "
								  +"and  purchaseType not like '%loan%' and a.SupplierId  like '%AL%' and VoucherNo=''  " 
			...
			if(frmName.equals("ThirdPartyIssue"))
				lst= session.createSQLQuery("select 'I. Date'=convert(varchar(2),day(Date)) + '-' + convert(varchar(2),month(Date)) + '-' + convert(varchar(4),year(Date)), 'Issue No' = IssueNo, 'Issued To'=IssuedTo," +
						"'Section Name' = (select SectionName from tbSectionInfo where AutoID = IssuedTo),'ReqNo' = IssueRef " +
						" from tbThirPartyIssueInfo where Date between '"+dateformat.format(fromDate.getValue())+"' and '"+dateformat.format(toDate.getValue())+"'     order by date").list();

			if(frmName.equals("RejectedItemIssue"))
			{
				String sql= "select a.issueNo,CONVERT(varchar(120),dIssueDate,110)issueDate, "
						   +"(select SectionName from tbSectionInfo where AutoID=a.issueTo) "
						   +"SectionName,a.challanNo,CONVERT(varchar(120),challandate,110)challanDate  from tbRejectedIssueInfo a inner join tbRejectedIssueDetails b "
						   +"on a.issueNo=b.issueNo where CONVERT(date,dIssueDate,105) "
			...
			if(frmName.equals("crashingIssue"))
			{
				String sql= "select a.issueNo,CONVERT(varchar(120),dIssueDate,110)issueDate, "
						   +"(select SectionName from tbSectionInfo where AutoID=a.issueTo) "
						   +"SectionName,a.challanNo,CONVERT(varchar(120),challandate,110)challanDate  from tbRecycledItemIssueInfo a inner join tbRecycledIssueDetails b "
						   +"on a.issueNo=b.issueNo where CONVERT(date,dIssueDate,105) "
						   +"between '"+dateformat.format(fromDate.getValue())+"' and '"+dateformat.format(toDate.getValue())+"' ";
			...
			if(!lst.isEmpty())
				for (Iterator iter = lst.iterator(); iter.hasNext();)
				{
					...
					if(frmName.equals("RejectedItemIssue"))
					{
						...
						table.addItem(new Object[] {lblAutoID.get(i),element[1],  element[2],element[3],element[4]}, new Integer(i));
						lblAutoID.get(i).setValue(element[0].toString().trim());
					}
	...
	private void compInit(){
		...
		if(frmName.equals("RejectedItemIssue"))
		{
			...
			table.setColumnWidth("Challan Date",80);
		}
