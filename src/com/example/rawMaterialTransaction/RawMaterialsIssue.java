...
*/
	...
	private void issueTo(){
		...
		try{
			...
			String sql=
					"select distinct  IssuedTo,(select SectionName from tbSectionInfo where CAST(AutoID as varchar(120)) =CAST(tbRawIssueInfo.IssuedTo as varchar(120)) )sectionName from tbRawIssueInfo "
					+"where CAST(IssuedTo as varchar(120))  in (select CAST(tbSectionInfo.AutoID  as varchar(120))  from tbSectionInfo) "
					+"union "
					+"select AutoID,SectionName from tbSectionInfo where vDepartmentName like '%Production%' ";
	...
	private void insertData(Session session,Transaction tx){
		try
		{
			...
			String sql= " insert into tbRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,vItemType,dReqDate,vflag) "
					...
					+" '"+vocherId+"','"+voucharType+"','"+reqNo+"', 1, '"+itemtype+"','"+reqDate+"','OLD') ";
			...
			String sqludInfo= " insert into tbudRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo,UserId,userIp,EntryTime,VoucherNo,VoucherType,issueRef,isActive,transactionNo,udflag,vItemType,vflag) "
					...
					+" '"+cmbProductionType.getValue()+"','' ,'' ,"
					+" '"+txtChallanNo.getValue().toString()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"',CURRENT_TIMESTAMP,  "
					+" '"+vocherId+"','"+voucharType+"','"+reqNo+"',1,'"+autoUdTransactionNo()+"','"+udFlag+"','"+itemtype+"','OLD') ";
