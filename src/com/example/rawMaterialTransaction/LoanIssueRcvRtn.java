...
public class LoanIssueRcvRtn extends Window {
	...
	private void  insertData()
	{
		...
		try
		{
			...
			sql="insert  into tbRawIssueInfo(IssueNo,IssuedTo,Date,TotalAmount,ProductionType,productionStep,finishedGoods,challanNo," +
					"UserId,userIp,EntryTime,IssueType,VoucherNo,VoucherType,IssueRef,isActive,vloanRefNo,vflag) " 
					+"values('"+IssueNo+"','"+IssueTo+"','"+date+"' ,'"+totalAmount+"','','','','"+ChallanNo+"','"+sessionBean.getUserName()+"',"
					+" '"+sessionBean.getUserIp()+"',getdate(),'"+IssueType+"' ,'"+txtvoucherNo.getValue().toString()+"','','',0,'"+txtreferenceNo.getValue().toString()+"','OLD')";
	...
	public void tableRowAdd(final int ar)
	{
		...
		try
		{
			...
			List lst1 = dbService("select distinct 0,vCategoryType from tbRawItemInfo");
