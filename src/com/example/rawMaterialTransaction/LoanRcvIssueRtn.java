...
public class LoanRcvIssueRtn extends Window 
{
	...
	public LoanRcvIssueRtn(SessionBean sessionBean) 
	{
	...
	public void tableRowAdd(final int ar)
	{
		...
		try{
			...
			List lst1 = session.createSQLQuery("select distinct 0,vCategoryType from tbRawItemInfo").list();
			...
			rate.get(ar).addListener(new ValueChangeListener() 
			{
				public void valueChange(ValueChangeEvent event) 
				{
					...
					amount=rate1*qty1;
					LoanRcvIssueRtn.this.amount.get(ar).setValue(amount);
					
					for(int i=0;i<rate.size();i++)
					{
						
						if(!LoanRcvIssueRtn.this.amount.get(i).getValue().toString().isEmpty())
						{
							total=total+Double.parseDouble(LoanRcvIssueRtn.this.amount.get(i).getValue().toString()); 	
	...
	private void insertData(Session session,Transaction tx) 
	{
		...
		try
		{
			...
			String sql ="insert into tbRawPurchaseInfo(ReceiptNo,Date,SupplierId,TotalAmount,ChallanNo,VoucherNo,VoucherType,StoreLocation,username,userIp,entryDate,purchaseType,transactionType,MrrType,MrrNo,flag ,vloanRefNo,vflag)" +
					    ...
					    " '"+voucharType+"','"+cmbStoreLocation.get(i).getValue()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"',getdate(),'"+Loantype.getValue().toString()+"','"+Loantype.getValue().toString()+"','','"+txtmrrNo.getValue().toString()+"','0','"+txtreference.getValue().toString()+"','OLD') ";
