...
public class DeptWiseIssueSummary extends Window {
	...
	boolean type=false;
	public DeptWiseIssueSummary(SessionBean sessionBean,String str) {
		this.sessionBean = sessionBean;
		
		setContent(buildMainLayout());
		this.setResizable(false);
		this.setCaption("Section WISE ISSUE SUMMARY ::  "+ sessionBean.getCompany());
		cmbcategorytype.focus();
		Component ob[]={cmbcategorytype,cmbCategoryName, cmbSectionName,cmbitem,fromDate,toDate,previewButton};
	...
	private void reportShow()
	{
		...
		String sectionId= cmbSectionName.getValue().toString();
		...
		try{
			...
			hm.put("toDate",new SimpleDateFormat("dd-MM-yyyy").format(toDate.getValue()));
			hm.put("section",cmbSectionName.getItemCaption(cmbSectionName.getValue().toString()));


			query= "select * from funcSectionWiseIssueSumm('"+categoryID+"','%','"+new SimpleDateFormat("yyyy-MM-dd").format(fromDate.getValue())+"','"+new SimpleDateFormat("yyyy-MM-dd").format(toDate.getValue())+"','"+categirytype+"','"+item+"','"+sectionId+"') where qty>0";
			...
			if(!lst.isEmpty())
			{
				Window win = new ReportViewer(hm,"report/raw/RptSectionWiseIssueSummary.jasper",
	...
	public void addSectionName(String category)
	{
		...
		try{
			...
			/*String query=  
					 "select distinct  a.IssuedTo,d.SectionName from tbRawIssueInfo a "
					 +"inner join " 
					 +"tbRawIssueDetails b  "
				...
				*/
			String query=  
			 "select distinct  a.IssuedTo,(select SectionName from tbSectionInfo where CAST(AutoID as varchar)=a.IssuedTo) SectionName,(select vDepartmentName from tbSectionInfo where AutoID=a.IssuedTo)dept from tbRawIssueInfo a "
			 +"inner join " 
			 +"tbRawIssueDetails b  "
			 +" on a.IssueNo=b.IssueNo " 
			 +"inner join " 
			 ...
			 +"is null "
			 +"union all "
			 +"select distinct  a.IssuedTo,d.supplierName,ISNULL((select vDepartmentName from tbSectionInfo where AutoID=a.IssuedTo),'') dept  from tbRawIssueInfo a " 
			 +"inner join " 
			 +"tbRawIssueDetails b "
			 +"on a.IssueNo=b.IssueNo "
			 +"inner join " 
			...
			for(Iterator iter=list.iterator();iter.hasNext();)
			{
				...
				cmbSectionName.setItemCaption(element[0].toString(), element[2].toString()+"-"+element[1].toString());
			}
		}catch(Exception exp){
			this.getParent().showNotification("Error",exp+"",Notification.TYPE_ERROR_MESSAGE);
		}
