			...
			int a= Integer.parseInt((String) hmnew.get(cmbSupplier.getValue().toString()));
			 ...
			 if(a==1)
			 {
				 query="select rpi.MrrNo,rpi.Date,rpi.ReceiptNo,rpi.purchaseType,rpi.poNo,rpi.ChallanNo,rpi.challanDate,"
							...
							+" where CONVERT(Date, rpi.Date,105) between '"+datef.format(formDate.getValue())+"' and '"+datef.format(toDate.getValue())+"'  and  si.supplierId like '"+cmbSupplier.getValue().toString()+"' and rpi.supplierId not like '%AL%'  order by rpi.Date";
			 ...
			 else
			 {
			    query=  "select rpi.MrrNo,rpi.Date,rpi.ReceiptNo,rpi.purchaseType,rpi.poNo,rpi.ChallanNo,rpi.challanDate, "
			    		...
			    		+"inner join tbRawPurchaseDetails rpd on rpi.ReceiptNo=rpd.ReceiptNo inner join tbRawItemInfo pi " 
			    		+"on rpd.ProductID=pi.vRawItemCode left join tbRawPurchaseOrderInfo poi on rpi.poNo=poi.poNo " 
			    		+"where CONVERT(Date, rpi.Date,105) between '"+datef.format(formDate.getValue())+"' and '"+datef.format(toDate.getValue())+"'   and  si.Ledger_Id like '"+cmbSupplier.getValue().toString()+"' and " 
